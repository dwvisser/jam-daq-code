...
public class JamCommand extends MouseAdapter implements ActionListener,
ItemListener {
     ...
     */
    public JamCommand(JamMain jamMain, Display display, Broadcaster broadcaster,
    JamConsole console ) {
        ...
        runControl=new RunControl(jamMain, histogramControl, scalerControl, (VMECommunication)frontEnd, hdfio, console);
        ...
        //setup classes
        setupSortOn=new SetupSortOn(jamMain, runControl, displayCounters, frontEnd, console);
        setupSortOff=new SetupSortOff(jamMain, sortControl, displayCounters, broadcaster, console);
    ...
    public synchronized void actionPerformed(ActionEvent e){
        ...
        try {
            if(incommand=="overlay"){
                ...
                impExpASCII.saveFile ( Histogram.getHistogram(JamStatus.getCurrentHistogramName() ));
            } else if(incommand=="savespe"){
                impExpSPE.saveFile (Histogram.getHistogram(JamStatus.getCurrentHistogramName()));
            } else if(incommand=="saveornl"){
                impExpORNL.saveFile( Histogram.getHistogram(JamStatus.getCurrentHistogramName()));
            } else if(incommand=="savexsys"){  //FIXME not implemented
                console.errorOutln("Save xsys file NOT implemented");
                impExpXSYS.saveFile (Histogram.getHistogram(JamStatus.getCurrentHistogramName()));
                ...
                if(JamStatus.isOnLine()) {
                    ...
                    console.errorOutln("Error: start command given when not in Online mode.");
                }
            } else if (incommand=="stop"){
                if(JamStatus.isOnLine()) {
                    runControl.stopAcq();
                } else {
                    console.errorOutln("Error: stop command given when not in Online mode.");
                }
            } else {
                console.errorOutln(getClass().getName()+": Error unrecognized command \""+
                incommand+"\"");
            }
            ...
            console.errorOutln(getClass().getName()+".actionPerformed(\""+incommand+"\"): "+
    ...
    public void itemStateChanged(ItemEvent ie){
        ...
        if (text=="Ignore zero channel on autoscale") {
            ...
            JamProperties.setProperty(JamProperties.FRONTEND_VERBOSE,item.isSelected());
        } else if (text=="Debug front end") {
            frontEnd.debug(item.isSelected());
            JamProperties.setProperty(JamProperties.FRONTEND_DEBUG,item.isSelected());
    ...
    void selectHistogram(Histogram hist) throws GlobalException {
        if (hist != null) {
                        ...
                        try {
                            SwingUtilities.invokeAndWait(new Runnable() {
                                public void run() {
                                    ...
                                    display.displayHistogram(Histogram.getHistogram(
            ...
            System.err.println(getClass().getName()+".selectHistogram(Histogram): null argument");
        }
    }
        ...
        if (gateObject instanceof Gate) {
            ...
            try {
                ...
                if(gate.getType()==Gate.ONE_DIMENSION) {
                    area=gate.getArea();
                    centroid=(double)( (int)(gate.getCentroid()*100.0) )/100.0;
                    lowerLimit=gate.getLimits1d()[0];
                    upperLimit=gate.getLimits1d()[1];
                    msgHandler.messageOut("Gate: "+gate.getName()+ ", Ch. "+lowerLimit+" to "
                    +upperLimit , MessageHandler.NEW);
                    msgHandler.messageOut("  Area = "+area+", Centroid = "+centroid ,
                    MessageHandler.END);
                } else {
                    area=gate.getArea();
                    msgHandler.messageOut("Gate "+gate.getName(), MessageHandler.NEW);
            ...
            //error not a Gate
            System.err.println("Error: JamCommand.selectGate(Object): Object instanceof "+
            gateObject.getClass().getName()+", String rep=\""+gateObject+"\"");
        }
    }
    ...
    public void printHistogram() {
        ...
        //cancel pjob=null might not work on windows 95 so try graphics object
        if (gpage==null) {
            return;
        }
        pageSize=pjob.getPageDimension();
        pagedpi=pjob.getPageResolution();
        System.err.println(getClass().getName()+".printHistogram(): printPlot pageSize "+pageSize);
        System.err.println(getClass().getName()+".printHistogram(): printPlot pagedpi "+pagedpi);
        msgHandler.messageOut("Printing histogram: "+JamStatus.getCurrentHistogramName(), MessageHandler.NEW);
