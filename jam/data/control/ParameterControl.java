 ...
 *
 */
 public class ParameterControl extends DataControl implements ActionListener, ItemListener {
    ...
    public ParameterControl(Frame frame, Broadcaster broadcaster, MessageHandler messageHandler){
		super();    
		this.frame=frame;
	...
	try {
		...
		System.err.println("Error Unregonized command [ParameterControl]");
	...
	*/
    ...
    public void setup(){    
		...
		if (numberParameters!=0) {	
	    	...
	    	enumParameter=DataParameter.getParameterList().elements();	    
	    	count=0;	    
	    	while(enumParameter.hasMoreElements()) {
				currentParameter=(DataParameter)enumParameter.nextElement();
				pParam[count]=new JPanel(new FlowLayout(FlowLayout.RIGHT,5,5));			    
		    	cddisp.add(pParam[count]);		    
				labelParam[count]=new JLabel(currentParameter.getName(), JLabel.RIGHT);
	...
	try {	    		    
		...
		if(textValue.equals("")) {
		    ...
		    currentParameter.setValue( (new Double(textValue).doubleValue() ));
		}		    
		count++;
	    }
	    read();	    
		...
		messageHandler.errorOutln("Not a valid number, parameter "+currentParameter.getName()+" [ParameterControl]");
	    } else {
		messageHandler.errorOutln("Not a valid number, null parameter [ParameterControl]");	    		
	    }		
	}				
