...
public class PeakFinder {
	...
	static PeakFinder instance = new PeakFinder();
	...
	private List<Multiplet> peakFind() {
		final Multiplet peaks = new Multiplet();
		...
		for (int i = filterLimit + 1; i < spectrum.length - filterLimit - 1; i++) {
			if (sum1[i] > sensitivity * Math.sqrt(sum2[i])
				...
				peaks.add(new Peak(posn, sum1[i], width));
			}
		}
			...
			for (int i = 0; i < len; i++) {
				...
				if (npeaks > 1) {
					for (int j = 0; j < npeaks; j++) {
						mult_i.set(i, correctPeak(mult_i, j));
					}
				}
			}
		}
