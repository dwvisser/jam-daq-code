...
public class Gate implements Serializable {
    ...
    public Gate(String name, Histogram hist){
        ...
        if(name.length()>NAME_LENGTH){
            System.err.println("Gate name '"+name+"' too long, truncated to '"+name2+"'.");
        }
        name=name2;
    ...
    public static List getGateList(int type)  {
        ...
        for (int i=0;i<out.size(); i++){
            if (((Gate)(out.elementAt(i))).getType() != type) {
                ...
                i--;//so that i++ in next loop iteration will actually access the next element
            }
        }
        return out;
    }
    ...
    public int [] getLimits1d() throws DataException{
        ...
        if (type != ONE_DIMENSION) throw new DataException("getLimits1d(): can only be called for gates" +
    ...
    public boolean [][] getLimits2d() throws DataException{
        if (type != TWO_DIMENSION) throw new DataException("getLimits2d(): can only be called for gates" +
    ...
    public Polygon getBananaGate() throws DataException{
        if (type != TWO_DIMENSION) throw new DataException("getBananaGate can only be called for gates" +
        " of type TWO_DIMENSION");
        return bananaGate;
    }
    ...
    public void setLimits(int ll, int ul) throws DataException{
        if (type != ONE_DIMENSION) throw new DataException("setLimits(int,int): can only be called for gates" +
        " of type ONE_DIMENSION");
        if (ll<=ul){
            lowerLimit=ll;
            upperLimit=ul;
    ...
    public void setLimits(List gatePoints) throws DataException{
        ...
        if (type != TWO_DIMENSION) throw new DataException("setLimits(Vector): can only be called for gates" +
        " of type TWO_DIMENSION");
    ...
    public void setLimits(Polygon gatePoly) throws DataException{
        ...
        if (type != TWO_DIMENSION) throw new DataException("setLimits(Polygon): can only be called for gates" +
        " of type TWO_DIMENSION");
    ...
    public boolean inGate(int channel) throws DataException{
        if (type != ONE_DIMENSION) throw new DataException("inGate(int): can only be called for gates" +
        " of type ONE_DIMENSION");
    ...
    public int getArea(){
        ...
        if (type==ONE_DIMENSION) {
            counts = (int [])histogram.getCounts();
            //sum up counts
            for (int i=lowerLimit;i<=upperLimit;i++) {
                sum+=counts[i];
            }
        } else if (type==TWO_DIMENSION) {
            counts2d=(int [][])histogram.getCounts();
            //sum up counts
            for (int i=0;i<sizeX;i++){
                for (int j=0;j<sizeY;j++){
                    if(insideGate[i][j]){
    ...
    public double getCentroid() {
        ...
        if (type==ONE_DIMENSION) {
            ...
            for (int i=lowerLimit;i<=upperLimit;i++){
                area+=counts[i];
                centroid+=(double)(i*counts[i]);
            }
            // calculate centroid
            if (area>0){	    // must have more than zero counts
                centroid=centroid/(double)area;
            } else {
                centroid=0.0;
            }
        } else if (type==TWO_DIMENSION) {
