 ...
 * 
 */
public class HistApplet extends Applet implements ActionListener, ItemListener {
...
//	}
	...
	if (ie.getItemSelectable()==histogramChooser) {
		...
		//error no such histogram
		System.err.println("Error: no item in histogram choicer "+ie.getItem()); 
	    }
...
//		    upperLimit=gate.getLimits1d()[1];		    			
		    console.messageOut("Gate: "+gate.getName()+ ", Ch. "+lowerLimit+" to "
								    +upperLimit , JamConsole.NEW);
		    console.messageOut("  Area = "+area , JamConsole.END);		
		} else {
		    area=gate.getArea();
		    console.messageOut("Gate "+gate.getName(), JamConsole.NEW);
		    console.messageOut(", Area = "+area, JamConsole.END );		
		}		    
...
// FIXME	    histIO.readSpeFile(fis);
	...
	Vector gateList;
	    ...
	    throw new JamException("Remote lookup up failed URL: "+stringURL+ "Excpetion:"+re.getMessage());
	    
	} catch (java.net.MalformedURLException mue){
	    throw new JamException("Remote look up malformed URL: "+stringURL);	
	    
	} catch (NotBoundException nbe){
	    throw new JamException("Remote look up could not find name "+stringURL);		    
...
//	    System.out.println("name "+remoteData.getName());
     ...
     */    
    public void setHistogramList(Vector histogramList){
