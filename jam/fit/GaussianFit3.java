 ...
 */
 public abstract class GaussianFit3 extends NonLinearFit {
	...
	//numPeaks.setValue(1);
	area1=new Parameter(AREA1, Parameter.DOUBLE, Parameter.FIX, Parameter.ESTIMATE);
	area1.setEstimate(true);
	centroid1=new Parameter(CENTROID1, Parameter.DOUBLE,  Parameter.FIX,  Parameter.MOUSE);
	centroid1.setValue(100.0);
	width1=new Parameter(WIDTH1, Parameter.DOUBLE, Parameter.FIX);
	width1.setValue(5.0);
	area2=new Parameter(AREA2, Parameter.DOUBLE, Parameter.FIX, Parameter.ESTIMATE);
	area2.setValue(0.0);
	area2.setFix(true);
	centroid2=new Parameter(CENTROID2, Parameter.DOUBLE,  Parameter.FIX,  Parameter.MOUSE);
	width2=new Parameter(WIDTH2, Parameter.DOUBLE, Parameter.FIX);
	width2.setValue(5.0);
	area3=new Parameter(AREA3, Parameter.DOUBLE, Parameter.FIX, Parameter.ESTIMATE);
	area3.setValue(0.0);
	area3.setFix(true);
	centroid3=new Parameter(CENTROID3, Parameter.DOUBLE,  Parameter.FIX,  Parameter.MOUSE);
	...
	double rangeOffset;//channel diff +/- from centroid to sum over for area 
    
	
	int i,minCH,maxCH,peakLow,peakHigh,imin,imax;
	...
	Darea3=getParameter(AREA3).getDoubleValue();
		...
		Darea1 = 0.0;
		rangeOffset = Dwidth1/SIGMA_TO_FWHM * 3;//3 sigma gets 99.6% of error
		...
		Darea2 = 0.0;
		rangeOffset = Dwidth2/SIGMA_TO_FWHM * 3;//3 sigma gets 99.6% of error
		...
		Darea3 = 0.0;
		rangeOffset = Dwidth3/SIGMA_TO_FWHM * 3;//3 sigma gets 99.6% of error
	...
	if (numPeaks.getIntValue()>1) chVector.element[3][0]=getParameter(CENTROID2).getDoubleValue();
	if (numPeaks.getIntValue()>2) chVector.element[4][0]=getParameter(CENTROID3).getDoubleValue();
	...
	return p("A")+p("B")*y+p("C")*y*y+p(AREA1)/Dwidth1*a*exp1+p(AREA2)/Dwidth2*a*exp2
	...
	exp3=Math.exp(-b*diff3*diff3/(Dwidth3*Dwidth3));
	    ...
	    temp=temp + c*p(AREA1)*exp1*diff1*diff1/(Dwidth1*Dwidth1*Dwidth1*Dwidth1);
	} else if (parName.equals(WIDTH2)) {
	    temp=-a*p(AREA2)*exp1/(Dwidth2*Dwidth2);
	    temp=temp + c*p(AREA2)*exp2*diff2*diff2/(Dwidth2*Dwidth2*Dwidth2*Dwidth2);
	} else if (parName.equals(WIDTH3)) {
	    temp=-a*p(AREA3)*exp3/(Dwidth3*Dwidth3);
	    temp=temp + c*p(AREA3)*exp3*diff3*diff3/(Dwidth3*Dwidth3*Dwidth3*Dwidth3);
