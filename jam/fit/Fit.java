 ...
 * @see	    GaussianFit
 */
public abstract class Fit implements ActionListener, ItemListener, PlotMouseListener {
    ...
    /**
     * Button to update <code>textHistName</code>
     */
    private JButton bUpdateHistName;
     ...
     */
    public void createDialog(Frame frame, Display display, MessageHandler msgHandler) throws FitException{
	...
	try {	
		...
		if (parameter.type==Parameter.DOUBLE){
			...
			Double.valueOf(textData[i].getText().trim()).doubleValue());
		    if(parameter.errorOption){ 
			((Parameter)(parameters.elementAt(i))).setError(
			 Double.valueOf(textError[i].getText().trim()).doubleValue());
		    }			
		    if (parameter.knownOption){
			((Parameter)(parameters.elementAt(i))).setKnown(
			 Double.valueOf(textKnown[i].getText().trim()).doubleValue());
		    }
		} else if (parameter.getType()==Parameter.INT) {
		    ((Parameter)(parameters.elementAt(i))).setValue(
			Integer.valueOf(textData[i].getText().trim()).intValue());
		    if (parameter.knownOption){
			((Parameter)(parameters.elementAt(i))).setKnown(
			 Double.valueOf(textKnown[i].getText().trim()).doubleValue());
		    }
		    ...
		    ((Parameter)(parameters.elementAt(i))).setValue(cOption[i].isSelected());
		}
		
	    }
	} catch (NumberFormatException nfe) {
	    clear();
	    throw new FitException("Invalid input, parameter: "+parameter.getName());
	} 
    }
	...
	double value;
	double error;
	//if (param.type!=Parameter.DOUBLE) throw new FitException("Can't formatError() on non-DOUBLE.");
	if (!param.errorOption) throw new FitException("No error term for this parameter.  Can't formatError().");
    ...
    private void addComponent(Component component, int gridx, int gridy, int width, int height) {
	...
	if (err < 0.0) throw new FitException ("format() can't use negative error.");
	if (err > 0.0) {
	...
	if (err == 0.0) throw new FitException("integerDigits called with 0.0");
	if (err >= 1.0) {
	...
	if (x <= 0.0) throw new FitException("Can't call firstSigFig with non-positive number.");
