...
public class LevenbergMarquadt {
     ...
     */
    public void setup( double [] counts, double [] errors, int minChannel,int maxChannel){
    	this.data = counts;
	this.errors = errors;
	...
	if (firstCall){
	    ...
	    oneda=new Matrix(nVar,1,0.0);//mfit x 1 matrix filled w/ zeroes
	    lambda=0.001;
	...
	for(j=0;j<nVar;j++){
	    covar.element[j][j]=covar.element[j][j]*(1.0+lambda);//aubment diagonal elements
	}
	...
	if (!allDone){
	    ...
	    for (l=0;l<nPar;l++){
			...
			tryParameters[l].setValue(tryParameters[l].getDoubleValue()+da.element[j][0]);
			j++;
	...
	//NonLinearFit nlf=nonLinFit;
	Parameter parameter;
			
	//set parameter values in funk
	for(i=0;i<params.length;i++){
	    nonLinFit.setParameter(params[i].getName(),params[i].getDoubleValue());
	}
	...
	for (i=minChannel;i<=maxChannel;i++){
		...
		if (!params[l].isFix()){
			...
			if (!params[m].isFix()){
			    k++;
			    space.element[j][k] = space.element[j][k]+weight * dyda[m];
			}
		    }
