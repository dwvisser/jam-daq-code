...
class Plot1d extends Plot  {
    ...
    void paintHistogram(Graphics g) {
        ...
        if (autoPeakFind) {
        	try{
        		graph.drawPeakLabels(currentHist.findPeaks(sensitivity,width,pfcal));
    ...
    void paintFit(Graphics g){
        g.setColor(PlotColorMap.fit);
        graph.drawLine(fitChan, fitFunc);
        if(fitResd!=null) {
            g.setColor(PlotColorMap.residual);
            graph.drawLine(fitChan, fitResd);
        }
    }
