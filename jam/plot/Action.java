...
class Action implements PlotMouseListener, PreferenceChangeListener {
	 ...
	 */
	synchronized void doCommand(String inCommand, final double[] inParams,
			boolean console) {
	...
	void overlay(double[] hist) {
		...
		for (int i = 0; i < hist.length; i++) {
			...
			if (h != null) {
				...
				} else if (display.getPlotContainer().getDimensionality() != 1) {
					textOut
	...
	/*
	 * void overlayEnable(double[] enable) { init(); textOut.messageOut("Overlay
	 * histogram numbers: ", MessageHandler.NEW); if (enable[0] < 1.0) {
	 * overlayState = false; } else { overlayState = true; }
	 * display.setOverlay(overlayState); done(); }
	 */
	...
	void displayGate(Object[] params) {
		...
		if (gate.getDimensionality() == 1) {
			...
			textOut.messageOut("Gate: " + gate.getName() + ", Ch. "
					+ lowerLimit + S_TO + upperLimit, MessageHandler.NEW);
			textOut.messageOut("  Area = " + area + ", Centroid = " + centroid,
					MessageHandler.END);
		} else {
			textOut.messageOut("Gate " + gate.getName(), MessageHandler.NEW);
	...
	private void range(boolean console) {
		if (!commandPresent) {
			...
			final PlotContainer plot = display.getPlotContainer();
	...
	private void expand() {
		...
		if (!commandPresent) {
			...
			synchronized (cursor) {
				...
				addClick(cursor);
				textOut.messageOut(cursor.getCoordString() + S_TO);
			}
		} else {
	...
	private void linear() {
		isCursorCommand = false;
		display.getPlotContainer().setLinear();
		done();
	}
	...
	private void log() {
		isCursorCommand = false;
		display.getPlotContainer().setLog();
		done();
	}
	...
	private void changeScale() {
		isCursorCommand = false;
		if (display.getPlotContainer().getLimits().getScale() == Scale.LINEAR) {
			log();
		} else {
			linear();
		}
	...
	private void auto() {
		isCursorCommand = false;
		display.getPlotContainer().autoCounts();
		done();
	}
	...
	private void netArea() {
		...
		if (!commandPresent) {
			...
			synchronized (cursor) {
				...
				currentPlot.markChannel(cursor);
				if (currentPlot.getDimensionality() == 1) {
					textOut.messageOut(" and " + cursor.getX() + S_TO);
	...
	private void done() {
		synchronized (this) {
			...
			currentCommand = null;
			clicks.clear();
			display.getPlotContainer().setSelectingArea(false);
		}
	}
