...
public class HistogramIO implements FilenameFilter{
    ...
    final static int BUFFER_SIZE=256*256*4;  //size of one 2D histogram int is 4 bytes
    ...
    public void writeJHFFile(File fileSave){
		...
		try {
			msgHandler.messageOut("Save Version 01 File:  "+fileSave.getName(), MessageHandler.NEW);				
			writeJHFData(fileSave);					    	
		} catch(ImpExpException iee){
	    	msgHandler.errorOutln("writeJHFFile() ImpExpException: " + iee.getMessage());	
		}
    }
    ...
    public boolean readJHFFile(File fileOpen) throws ImpExpException{
		...
		try {
	    	DataBase.clearAllLists();
	   	 	msgHandler.messageOut("Open "+fileOpen.getName(), MessageHandler.NEW);				
		...
		/*} catch(FileNotFoundException exc){
	    	...
	    	msgHandler.errorOutln("Reading JHF file: IOException: "+ioe.getMessage());	
	    	success = false;
		} 
		return success;
    }
	...
	try {
	    msgHandler.messageOut("Reload "+fileOpen.getName(), MessageHandler.NEW);				
	    ...
	    msgHandler.errorOutln("Reloading JHF file ImpExpException: "+iee.getMessage());	
	    success = false;
	} 
	return success;
    }
     ...
     */    
    public File getFileOpen(String msg, String extension) throws ImpExpException{
     ...
     */        
    public File getFileSave(String msg, String extension) throws ImpExpException{
     ...
     */    
    public File getFile(String msg, String extension, int state) throws ImpExpException{
	...
	throw new ImpExpException("Problem calling setExtension(): "+ue.getMessage());
      }		
    }
	...
	BufferedOutputStream bos=new BufferedOutputStream(fileStream,BUFFER_SIZE);
	...
	if (writeHistograms){
		...
		String name=StringUtilities.makeLength(hist.getName(),HIST_NAME_LENGTH);	
		for(int i=0;i<HIST_NAME_LENGTH;i++){
		    dos.writeChar(name.charAt(i));
		}     
		dos.writeInt(hist.getNumber());
		String title=StringUtilities.makeLength(hist.getTitle(),HIST_TITLE_LENGTH);
	...
	//write out gates
	allGates=Gate.getGateList().elements();
	...
	if (numGates>0){
	    ...
	    while(allGates.hasMoreElements()){
		    Gate gate =  (Gate) (allGates.nextElement());
	...
	//write out scalers	        
	allScalers=Scaler.scalerList.elements();    
	...
	if (writeScalers) {		
	    ...
	    if (numScalers>0){	
		while(allScalers.hasMoreElements()){
		    Scaler scaler=((Scaler)allScalers.nextElement());
	...
	throw new ImpExpException("Problem writing JHF file: "+ioe.getMessage());
      }	catch (DataException de) {
	throw new ImpExpException("Problem accessing data object in memory: "+de.getMessage());
      }	
    }	              
	...
	throw new ImpExpException("Problem reading JHF file: "+ioe.getMessage());
      }		   	
    }
    /**
     * read in a jhf file
	...
	if(!eof){
	    ...
	    if (numGates>0){//read past gates, but don't actually do anything with them
		msgHandler.messageOut(" gates");		    
	...
	throw new ImpExpException("Problem creating data object: "+de.getMessage());
      }		   	
    }
	...
	while (!eof){
		...
		if (mode==OPEN_MODE){
			...
			msgHandler.messageOut(name+": "+de.getMessage());
		    }
		} else {//RELOAD_MODE
		    hist = Histogram.getHistogram(name);
		    try {
		...
		if (mode==OPEN_MODE){
			...
			throw new ImpExpException("Problem creating histogram from file: "+de.toString());
		    }
		} else {//RELOAD_MODE
		    hist = Histogram.getHistogram(name);		
		    try {			
	...
	msgHandler.messageOut(", "+numHist+" histograms.", MessageHandler.END);
	dis.close();
      } catch (IOException ioe) {
	throw new ImpExpException("Problem reading JHF file: "+ioe.getMessage());
      }		   	
