...
public class HDFile extends RandomAccessFile implements HDFconstants {
    ...
    /**
     * Vector of objects in the file.
     */
    public Vector DataObjects;
    ...
    public HDFile(File file, String mode) throws HDFException, IOException{
        ...
        if (mode=="rw") {//Saving a file
            ...
            if (!checkMagicWord()) throw new HDFException("Not a valid HDF File!");
            DataObjects = new Vector();
        }
    }
    ...
    public void setOffsets(){
        ...
        for (temp = DataObjects.elements() ; temp.hasMoreElements() ;) {
            ob=(DataObject)(temp.nextElement());
    ...
    public void writeDataObject(DataObject data) throws HDFException{
        try {
            ...
            throw new HDFException("Problem writing HDF data object"+ e.getMessage());
        }
    }
    ...
    /**
     * Returns the existing valid SDD type for the histogram, creating a new one if necessary.
     */
    public ScientificDataDimension getSDD(Histogram h){
        ...
        if (h.getType()==Histogram.ONE_DIM_INT||h.getType()==Histogram.TWO_DIM_INT) {
            type = NumberType.INT;
        } else {
            type = NumberType.DOUBLE;
        }
        for (temp = ofType(DataObject.DFTAG_SDD).elements() ; temp.hasMoreElements() ;) {
            sdd=(ScientificDataDimension)temp.nextElement();
            if (sdd.getType()==type&&sdd.getSizeX()==sizeX) {
                if (rank==1 && rank==sdd.getRank()){
                    return sdd;
                } else if (rank==2 && rank==sdd.getRank() &&
                sdd.getSizeY()==sizeY) {
                    return sdd;
                }
            }
        }
        return new ScientificDataDimension(this,h);
    }

    /**
     * Returns the existing valid SDD type for the histogram, creating a new one if necessary.  DOUBLE type
    ...
    public void writeDataDescriptorBlock() throws HDFException{
        ...
        try {
            ...
            throw new HDFException("Problem writing DD block: "+e.getMessage());
        }
    }
    ...
    public void writeAllObjects(MessageHandler msg) throws HDFException {
        ...
        counter = fileSize;
        outbar = 9;
        for (temp = DataObjects.elements() ; temp.hasMoreElements() ;counter -= ob.getLength()) {
            ob=(DataObject)(temp.nextElement());
            if (ob.getLength()==0) throw new HDFException("DataObject with no length encountered, halted writing HDF File");
            writeDataObject(ob);
    ...
    public void printDDblock() throws HDFException{
        ...
        try {
            ...
            throw new HDFException("Problem printing DD block: "+e.getMessage());
        }
    }
        ...
        try{
            ...
            do {
                ...
                for (i=1;i<=numDD;i++) {
                    ...
                    switch (tag) {
                        case DataObject.DFTAG_DIA	    :	new DataIDAnnotation(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_DIL	    :	new DataIDLabel(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_VERSION   :	new LibVersion(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_NT	    :	new NumberType(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_NDG	    :	new NumericalDataGroup(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_SD	    :	new ScientificData(this,offset,length,ref);
                        break;
                        case DataObject.DFTAG_SDD	    :	new ScientificDataDimension(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_SDL	    :	new ScientificDataLabel(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_VG	    :	new VirtualGroup(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_VH	    :	new VdataDescription(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_VS	    :	new Vdata(this,bytes,ref);
                        break;
                        case DataObject.DFTAG_MT	    :	break;//do nothing
                        case DataObject.DFTAG_FID	    :	break;//do nothing
                        case DataObject.DFTAG_FD	    :	break;//do nothing
                        case DataObject.DFTAG_SDS	    :	break;//do nothing
                        default			    :	System.err.println("Unrecognized tag: "+tag);
                    }
                }
                if (nextBlock==0) {
                    doAgain=false;
            ...
            throw new HDFException("Problem reading HDF objects: "+e.getMessage());
        }
    }
    ...
    public Vector ofType(DataObject in){
        return ofType(DataObjects,(short)in.getTag());
    }

    /**
     * Returns a Vector of <code>DataObject</code>'s of the
     * type specified by <code>tag</code>.
     */
    public Vector ofType(Vector in, short tagType){
        Vector output;
        Enumeration temp;
        DataObject ob;

        output=new Vector();
        for (temp = in.elements() ; temp.hasMoreElements() ;) {
            ob=(DataObject)(temp.nextElement());
    ...
    public Vector ofType(short tagType){
    ...
    public void addFileNote() throws IOException {
        ...
        JamProperties.getProperties().store(baos,"Jam Properties at time of save:");
        notation=new String(baos.toByteArray());
        notation = "All error bars on histogram counts should be considered Poisson, unless a\n"+
        "Numerical Data Group labelled 'Errors' is present, in which case the contents\n"+
        "of that should be taken as the error bars."+notation;
    ...
    /**
     * Checks whether this file contains the HDF magic word at the beginning.
     *
    ...
    /**
     * Checks whether the file passed contains the HDF magic word at the beginning.
     *
     * @param f file to check
     * @return <code>true</code> if this file has a valid HDF magic word
     */
    public static boolean checkMagicWord(File f) {
        try {
            RandomAccessFile raf=new RandomAccessFile(f,"r");
            raf.seek(0);
            return (raf.readInt() == HDF_HEADER) ;
        } catch (IOException e) {
            return false;
        }
    }
