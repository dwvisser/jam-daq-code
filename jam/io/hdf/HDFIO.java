...
public class HDFIO implements DataIO, JamHDFFields {
	 ...
	 * options given. (Parameters are always written.)
	 * 
	 * @param wrthis
	 ...
	 * spectra. Any <code>Parameters</code> are always written with this call.
	 * 
	 * @param wrthis
	 *            if true, Histograms will be written
	 * @param wrtgate
			...
			lookForSpectrum: while (temp.hasNext()) {
				...
				if (name.trim().equals(spectrumName)) {
					if (numbers.length == 1) { //only one NDG -- the data
						...
						/*
						 * determine which of the two contains error bars and
						 * ignore it, assigning the other to ndg
						 */
						if (DataIDLabel.withTagRef(labels,
								DataObject.DFTAG_NDG, numbers[0].getRef())
