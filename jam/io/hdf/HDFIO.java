...
public final class HDFIO implements DataIO, JamFileFields {
	 ...
	 * @param writeScalers
	 *            whether to write out histograms scalers
	 * @param writeParams
	 *            whether to write out gates, calibration and parameters
	 */
	 ...
	 * @throws HDFException
	 */
	private void convertHDFToJam(FileOpenMode mode, String fileName,
			List existingGroupList, List histAttributeList) throws HDFException {
		hdfToJam.setInFile(inHDF);
		// Find groups
		final List groupVirtualGroups = hdfToJam.findGroups(mode,
				existingGroupList);
		...
		GROUPLIST: while (groupIter.hasNext()) {
			...
			while (histIter.hasNext()) {
				...
				final Histogram hist = (Histogram) hdfToJam.convertHistogram(
						currentGroup, histVGroup, histAttributeList, mode);
				if (hist != null)
					histCount++;
				...
				if (hist != null && mode != FileOpenMode.ADD) {
					...
					while (gateIter.hasNext()) {
						final VirtualGroup gateVGroup = (VirtualGroup) gateIter
								.next();
						Gate gate = hdfToJam
								.convertGate(hist, gateVGroup, mode);
						if (gate != null)
							gateCount++;
					}
					/* Load calibration. */
	...
	private void convertHDFToJamOriginal(FileOpenMode mode, String fileName,
		...
		if (mode != FileOpenMode.ADD) {
			...
			if (vddParam != null) {
				paramCount = hdfToJam.convertParameters(currentGroup, vddParam,
						mode);
			}
		}
	}
	...
	private void convertJamToHDF(List groups, List histList, boolean writeData,
			boolean wrtSettings, boolean suppressEmpty) throws HDFException {
	 ...
	 *            whether to open or reload
	 * @param histNames
	 *            names of <code>Histogram</code>'s to read in
	 * @return <code>true</code> if successful
	 */
	public boolean readFile(FileOpenMode mode, File infile,
	...
	private void spawnAsyncReadFile(final FileOpenMode mode,
			final File[] inFiles, final List groupList,
			final List histAttributeList) {
		uiMessage = "";
		uiErrorMsg = "";
	 ...
	 *            whether to write gates and parameters
	 */
	private void writeFile(final File file, final List groups,
			final List histograms, boolean writeData, boolean wrtSettings) {
