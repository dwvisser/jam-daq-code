...
public class NumericalDataGroup extends DataObject {
  ...
  protected void refreshBytes() throws HDFException{
    ...
    DataObject ob;
    try{
      numBytes=4*elements.size();  //see DFTAG_NDG specification for HDF 4.1r2
  ...
  public void interpretBytes() throws HDFException {
    ...
    try{
      for(i=0;i<numItems;i++){
        t=dis.readShort();
        r=dis.readShort();
        addDataObject(file.getObject(t,r));//look up tag/ref in file and add object
      }
    } catch (IOException e) {
      throw new HDFException ("Problem interpreting NDG: "+e.getMessage());
    }
  }
