...
final class ConvertHDFObjToJamObj implements JamFileFields {
	...
	private static final StringUtilities STRING_UTIL = StringUtilities.instance();
	 ...
	 */
	int convertParameters(Group group, VDataDescription vdd, FileOpenMode mode)
			throws HDFException {
		int numParams = 0;
	...
	int convertParameters(Group group, VirtualGroup currVG, FileOpenMode mode)
			throws HDFException {
		int numParameters = 0;
	 ...
	 * @return number of histograms
	 */
	List findGroups(FileOpenMode mode, List existingGroupList)
			throws HDFException {
	...
	private AbstractCalibrationFunction makeCalibration(Histogram hist,
			String funcName) throws HDFException {
		Class calClass;
