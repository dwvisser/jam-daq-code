...
public class VdataDescription extends DataObject {
     ...
     * 
     */
    public VdataDescription(HDFile fi,String name, String classtype,
				int size,String [] names, short [] types,short [] orders){
	...
	if ((names.length!=types.length)||(names.length!=orders.length)){
	    System.err.println("VdataDescription(): was not called with all same dimensions!");
	} else {//OK
		...
		switch (types[i]) {
		    case DFNT_INT16 :	isize[i]=(short)(order[i]*2);
					break;
		    case DFNT_INT32 :	isize[i]=(short)(order[i]*4);
					break;
		    case DFNT_CHAR8 :	isize[i]=order[i];
					break;
		    case DFNT_FLT32 :   isize[i]=(short)(order[i]*4);
					break;
		    case DFNT_DBL64 :   isize[i]=(short)(order[i]*8);
					break;
		    default	    :	System.err.println("Unknown Vdata field type!, field: "+i+", type: "+
								types[i]);
		}
		ivsize += isize[i];
	    }
	...
	int byteLength=22 + 10 * nfields + name.length() + _class.length(); // see p. 6-42 HDF 4.1r2 specs
     ...
     */
    static public VdataDescription ofName(Vector in, String which){
	VdataDescription output;
	Enumeration temp;
	VdataDescription vdd;
	
	output=null;
	for (temp = in.elements() ; temp.hasMoreElements() ;) {
	    vdd=(VdataDescription)(temp.nextElement());
	    //System.out.println("VDD.ofName("+which+"): "+vdd.getName());
