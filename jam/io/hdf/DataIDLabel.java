...
public class DataIDLabel extends DataObject {
	...
	try{
	    ...
	    throw new HDFException ("Problem interpreting DIL: "+e.getMessage());
	}
    }
    ...
    static public DataIDLabel withTagRef(Vector labels, int tag, int ref){
	DataIDLabel output;
	Enumeration temp;
	DataIDLabel dil;
	...
	for (temp = labels.elements() ; temp.hasMoreElements() ;) {
	    dil=(DataIDLabel)(temp.nextElement());
	    if ((dil.getObject().getTag()==tag)&&(dil.getObject().getRef()==ref)){
		output=dil;
	    }
	}
	return output;
    ...
    /*static public DataIDLabel withTagRef(Vector labels, short tag, short ref){
	return withTagRef(labels,(int)tag,(int)ref);
    }*/
}
