...
public class Vdata extends DataObject {   
	...
	this.ref=(short)description.getRef();//experiment, see if using same ref fixes problem
    }
    /**
     * Constructor
     */    
	...
	out=null;
	int size;
	//System.out.println("getBytes(Row = "+row+", Col = "+col+")");
	...
	//XXXtry{
		...
		switch (types[col]){
			...
			//numBytes=2;			
			//XXXbaos = new ByteArrayOutputStream(numBytes);
			//dos = new DataOutputStream(baos);
			//dos.writeShort(((Short)(cells[col][row])).shortValue());
			...
			//numBytes=4;						
			//XXXbaos = new ByteArrayOutputStream(numBytes);												
			//XXXXdos = new DataOutputStream(baos);
			//dos.writeInt(((Integer)(cells[col][row])).intValue());
			...
			break;
		    case	VdataDescription.DFNT_DBL64 :
			double doubleValue=((Double)(cells[col][row])).doubleValue();
			out= doubleToBytes(doubleValue);		    
			
			break;
		    case	VdataDescription.DFNT_CHAR8 :
			char charValue=((Character)(cells[col][row])).charValue();
		    ...
		    default				    : 
			System.err.println("Vdata.getBytes("+row+","+col+") data type not good.");
		}
	    } else {// order > 1
		switch (types[col]){
		    case	VdataDescription.DFNT_INT16 :
			...
			//XXXbaos = new ByteArrayOutputStream(numBytes);
			//dos = new DataOutputStream(baos);			
			//dos.writeShort(((Short [])(cells[col][row]))[i].shortValue());			
			//out=baos.toByteArray();
			break;
		    case	VdataDescription.DFNT_INT32 :
			size=4;
			...
			//XXXXnumBytes=4*order[col];
			//baos = new ByteArrayOutputStream(numBytes);
			//dos = new DataOutputStream(baos);
			//for (int i=0;i<order[col];i++){
			//    dos.writeInt(((Integer [])(cells[col][row]))[i].intValue());
			//}
			//out=baos.toByteArray();
			break;
		    case	VdataDescription.DFNT_FLT32 :
			...
			//XXXbaos = new ByteArrayOutputStream(numBytes);
			//dos = new DataOutputStream(baos);
			//    dos.writeFloat(((Float [])(cells[col][row]))[i].floatValue());			
			//out=baos.toByteArray();
			break;
		    case	VdataDescription.DFNT_DBL64 :
			size=8;
			...
			//XXXnumBytes=8*order[col];
			//baos = new ByteArrayOutputStream(numBytes);
			//dos = new DataOutputStream(baos);
			//for (int i=0;i<order[col];i++){
			//    dos.writeDouble(((Double [])(cells[col][row]))[i].doubleValue());
			//}
			//out=baos.toByteArray();
			break;
		    case	VdataDescription.DFNT_CHAR8 :		    
			...
			//XXXbaos = new ByteArrayOutputStream(numBytes);
			//dos = new DataOutputStream(baos);
			//dos.writeBytes((String)(cells[col][row]));
			//out=baos.toByteArray();
			break;
	...
	//} catch (IOException ioe) {
	 //   System.err.println("Vdata.getBytes(): "+ioe);
	//}
    return out;
    }
	...
	if (types[col]==VdataDescription.DFNT_CHAR8){
	    ...
	    System.err.println("VS_"+getTag()+"/"+getRef()+".getString("+row+","+col+"): cell not string!");
	}
	return out;
    }
	...
	if (types[col]==VdataDescription.DFNT_INT32){
	    ...
	    System.err.println("VS_"+getTag()+"/"+getRef()+".getInt("+row+","+col+"): cell not integer!");
	}
	return out;
    }
	...
	if (types[col]==VdataDescription.DFNT_INT32){
	    ...
	    System.err.println("VS_"+getTag()+"/"+getRef()+".getInt("+row+","+col+"): cell not short!");
	}
	return out;
    }
	...
	if (types[col]==VdataDescription.DFNT_FLT32){
	    ...
	    System.err.println("VS_"+getTag()+"/"+getRef()+".getFloat("+row+","+col+"): cell not float!");
	}
	return out;
    }
