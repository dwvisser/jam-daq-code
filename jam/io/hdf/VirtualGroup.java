...
public class VirtualGroup extends DataObject {
	...
	DataObject ob;
      try{
	numBytes=14+4*elements.size()+name.length()+type.length();  //see DFTAG_VG specification for HDF 4.1r2
	baos=new ByteArrayOutputStream(numBytes);
	dos=new DataOutputStream(baos);
	dos.writeShort(elements.size());
	for (temp = elements.elements() ; temp.hasMoreElements() ;) {
	...
	try{
	    ...
	    throw new HDFException ("Problem interpreting VG: "+e.getMessage());
	}
    }
	...
	if (data==null) throw new HDFException("Can't add null to vGroup.");
    ...
    /**
     * Returns a Vector of <code>VirtualGroup</code>'s of the 
     ...
     */
    static public Vector ofType(Vector in, String groupType){
	Vector output;
	Enumeration temp;
	VirtualGroup vg;
	...
	for (temp = in.elements() ; temp.hasMoreElements() ;) {
	    vg=(VirtualGroup)(temp.nextElement());
	    //System.out.println("VG.ofType(): "+vg.getType());
	    if (vg.getType()==groupType){
		output.addElement(vg);
	    }
	}
	return output;
    }
     ...
     * @param groupType	type string showing what kind of info is contained
     */
    static public VirtualGroup ofName(Vector in, String groupName){
	VirtualGroup output;
	Enumeration temp;
	VirtualGroup vg;
	...
	for (temp = in.elements() ; temp.hasMoreElements() ;) {
	    vg=(VirtualGroup)(temp.nextElement());
	    //System.out.println("VG.ofName("+groupName+"): "+vg.getName());
