...
final class ScientificData extends AbstractData {
     ...
     */
    Object getData(HDFile infile, int histDim, byte histNumType, int xlen,
			int ylen) throws HDFException {
    	Object rval;
    ...
    int[][] getData2d(HDFile infile, int xlen, int ylen)
            throws HDFException {
    ...
    double[][] getData2dD(HDFile infile, int xlen, int ylen)
            throws HDFException {
    ...
    void init(int byteOffset, int len, short tagType, short reference) {
        super.init(byteOffset, len, tagType, reference);
