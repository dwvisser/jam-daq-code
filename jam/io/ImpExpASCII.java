 ...
 * This is usefol for easy import into other applications.  It only works for
 * 1-D histograms. E.g. <code><br>
 * 0 0<br>
 * 1 2<br>
 * 3 5<br>
 * ...</code>
...
public class ImpExpASCII extends ImpExp {
    ...
    public void saveFile(Histogram hist) throws ImpExpException {
        if((hist.getDimensionality()==Histogram.TWO_DIM_INT)||
        (hist.getDimensionality()==Histogram.TWO_DIM_DOUBLE)){
            if (msgHandler != null) msgHandler.errorOutln("Cannot write out 2 dimensional ASCII files.");
        } else {
    ...
    protected void readHist(InputStream inStream)  throws ImpExpException {
        ...
        try {
            ...
            st.ttype!=StreamTokenizer.TT_NUMBER){//execute loop code only if not number or EOF read
                if(firstLine){
                    title=st.sval;
                }
            ...
            if(st.nextToken()!=StreamTokenizer.TT_EOF&&st.ttype==StreamTokenizer.TT_EOL){//no second number token
                tokenLine=1;
                counts[0]=temp;
                size=1;
                //second number token
            ...
            if(tokenLine==1){
                ...
                while(st.nextToken()!=StreamTokenizer.TT_EOF && st.ttype==StreamTokenizer.TT_NUMBER){
                    ...
                    //second token eol
                    if(st.nextToken()!=StreamTokenizer.TT_EOF &&st.ttype!=StreamTokenizer.TT_EOL){
                        throw new ImpExpException("Data line not ended with EOL Line: "+channel+"  [ImpExpASCII]");
                    }
                    size=channel+1;
                }
                ...
                while(st.nextToken()!=StreamTokenizer.TT_EOF && st.ttype==StreamTokenizer.TT_NUMBER){
                    ...
                    if (st.nextToken()==StreamTokenizer.TT_NUMBER){
                        ...
                        throw new ImpExpException("Data line does not have two values  Channel: "+channel+"  [ImpExpASCII]");
                    }
                    //third token eol
                    if(st.nextToken()!=StreamTokenizer.TT_EOF && st.ttype!=StreamTokenizer.TT_EOL){
                        throw new ImpExpException("Data line not ended with EOL Channel: "+channel+"  [ImpExpASCII]");
                    }
                    if (channel>size) {
                        size=channel+1;
                    }
     ...
     * @exception   ImpExpException    all exceptions given to <code>ImpExpException</code> go to the msgHandler
     */
    protected void writeHist(OutputStream buffout, Histogram hist) throws ImpExpException {
        PrintWriter pw;
        int [] counts;
        double [] countsD;
        try {
            ...
            if (hist.getType()==Histogram.ONE_DIM_INT){
                counts=(int [])hist.getCounts();
                for (int i=0; i<hist.getSizeX(); i++) {
                    //output a row of data  channel counts
                    pw.print(i); pw.print("   "); pw.println(counts[i]);
                }
            } else {
                countsD=(double [])hist.getCounts();
                for (int i=0; i<hist.getSizeX(); i++) {
                    //output a row of data  channel counts
                    pw.print(i); pw.print("   "); pw.println(countsD[i]);
                }
            }
