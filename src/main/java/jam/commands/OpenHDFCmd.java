...
final class OpenHDFCmd extends AbstractCommand
  ...
  @Inject
  OpenHDFCmd(final Frame frame, final HDFIO hdfio, final Broadcaster broadcaster) {
    super("Open\u2026");
  ...
  private void notifyApp(final File file) {
    ...
    /* Set general status. */
    GuiceInjector.getObjectInstance(JamStatus.class).setOpenFile(file);
     ...
     * histogram
     */
    final List<Group> groups = jam.data.Warehouse.getGroupCollection().getList();
    if (!groups.isEmpty()) {
      firstGroup = groups.get(0);
      GuiceInjector.getObjectInstance(JamStatus.class).setCurrentGroup(firstGroup);
