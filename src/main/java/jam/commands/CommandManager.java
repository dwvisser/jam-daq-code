...
public final class CommandManager implements CommandListener, ActionCreator {
  ...
  private static final Map<String, Commandable> INSTANCES =
      Collections.synchronizedMap(new HashMap<String, Commandable>());

  private static final Logger LOGGER =
      Logger.getLogger(CommandManager.class.getPackage().getName());

  private final transient CommandMap commandMap = new CommandMap(this);
  ...
  /**
   * See if we have the instance created, create it if necessary, and return whether it was
   * successfully created.
   *
   * @param strCmd name of the command
   * @return <code>true</code> if successful, <code>false</code> if the given command doesn't exist
   */
  private boolean createCmd(final String strCmd) {
    final boolean exists = commandMap.containsKey(strCmd);
    if (exists) {
      final Class<? extends Commandable> cmdClass = commandMap.get(strCmd);
  ...
  /**
   * Produce a menu item that invokes the action given by the lookup table in <code>
   * jam.commands.CommandManager</code>
   *
  ...
  public JMenuItem getMenuItem(final String name) {
    ...
    if (null == action) {
      throw new IllegalArgumentException("Couldn't find action for '" + name + "'.");
    }
  ...
  /**
   * @param name of menu
   * @param commandNames of commands
   * @return a menu
   */
    ...
    if (createCmd(strCmd)) {
      if (currentCom.isEnabled()) {
        try {
          ...
          LOGGER.log(Level.SEVERE, "Performing command " + strCmd + "; " + cle.getMessage(), cle);
        }
      } else {
  ...
  public void setEnabled(final String cmd, final boolean enable) {
    ...
    if (null == action) {
      throw new IllegalArgumentException("Couldn't find action for '" + cmd + "'.");
    }
