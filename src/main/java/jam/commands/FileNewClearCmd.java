...
final class FileNewClearCmd extends AbstractCommand implements PropertyChangeListener {
  ...
  @Inject
  FileNewClearCmd(final JFrame frame, final JamStatus status, final Broadcaster broadcaster) {
    super("Clear data\u2026");
    this.frame = frame;
    this.status = status;
    this.broadcaster = broadcaster;
    putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_N, CTRL_MASK));
    enable();
  }
  ...
  protected void execute(final Object[] cmdParams) {
    if (JOptionPane.YES_OPTION
        == JOptionPane.showConfirmDialog(
            frame, "Erase all current data?", "New", JOptionPane.YES_NO_OPTION)) {
      ...
      this.broadcaster.broadcast(BroadcastEvent.Command.HISTOGRAM_SELECT, null);
    }
  }
