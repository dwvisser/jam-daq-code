...
public final class ScalersCmd extends AbstractCommand {
   ...
   * Default constructor.
   *
   * @param broadcast for issuing scaler commands
   * @param status application status
   * @param broadcaster broadcasts any updates
   */
  @Inject
  public ScalersCmd(
      final BroadcastUtilities broadcast, final JamStatus status, final Broadcaster broadcaster) {
    super();
    this.broadcast = broadcast;
    this.status = status;
    this.broadcaster = broadcaster;
    putValue(SHORT_DESCRIPTION, "Read or zero scalers, depending on parameter.");
  }
  ...
  protected void execute(final Object[] cmdParams) {
    ...
    if (param == READ) {
      ...
      LOGGER.severe("Incomplete command: need 'scaler zero' or 'scaler read'.");
    }
  }
