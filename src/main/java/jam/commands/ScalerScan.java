...
public final class ScalerScan implements ProgressUpdater {
  ...
  private static final Logger LOGGER = Logger.getLogger(ScalerScan.class.getPackage().getName());
  ...
  private transient File pathToRuns =
      new File(
          JamProperties // NOPMD
   ...
   * Constructor.
   *
   * @param executor executor service to allocate threads to tasks
   */
  @Inject
  ...
  private File getFile(final boolean dir) {
    ...
    chooser.setFileSelectionMode(dir ? JFileChooser.DIRECTORIES_ONLY : JFileChooser.FILES_ONLY);
  ...
  private void doIt() {
    ...
    try {
      ...
      pBstatus =
          new ProgressMonitor(
              frame, "Scanning HDF Files for scaler values", "Initializing", firstRun, lastRun);
      if (pathToRuns.exists() && pathToRuns.isDirectory()) {
        ...
        if (!pBstatus.isCanceled()) {
          final String title =
              txtRunName.getText() + ", runs " + txtFirst.getText() + " to " + txtLast.getText();
  ...
  private void scanFileIfItExists(
      final ScanForScalers scanner,
      final int firstRun,
      final int run,
      final String filename,
      final StringBuffer outText)
