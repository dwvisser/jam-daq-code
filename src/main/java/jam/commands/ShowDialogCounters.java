...
final class ShowDialogCounters extends AbstractShowDialog implements PropertyChangeListener {
  ...
  /** Initialize command */
  @Inject
  ShowDialogCounters(final DisplayCounters displayCounters, final JamStatus status) {
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    if (((BroadcastEvent) evt).getCommand() == BroadcastEvent.Command.SORT_MODE_CHANGED) {
      ...
      setEnabled(
          mode == SortMode.ONLINE_DISK || mode == SortMode.ON_NO_DISK || mode == SortMode.OFFLINE);
    }
  }
}
