...
final class SaveSortGroupHDFCmd extends AbstractCommand implements PropertyChangeListener {
  ...
  @Inject
  SaveSortGroupHDFCmd(final Frame frame, final JamStatus status, final HDFIO hdfio) {
  ...
  private void saveSortGroup(final File file) {
    ...
    if (mode == SortMode.ONLINE_DISK || mode == SortMode.ON_NO_DISK || mode == SortMode.OFFLINE) {
      /* find sort group */
  ...
  private void enable() {
    ...
    setEnabled(
        mode == SortMode.OFFLINE || mode == SortMode.ONLINE_DISK || mode == SortMode.ON_NO_DISK);
  }
}
