...
/**
 * Abstract implementation of <code>Commandable</code> which can set/unset a given preference.
 *
...
public abstract class AbstractSetBooleanPreference extends AbstractCommand { // NOPMD
  ...
  static {
    ...
    CHECK_MARK = new ImageIcon(loader.getResource("jam/commands/checkmark.png"));
    CLEAR = new ImageIcon(loader.getResource("jam/ui/clear.png"));
  }

  /** Set to <code>true</code> here. Set differently in <em>constructor</em> if necessary. */
  ...
  /**
   * If there is at least one element, and it is a <code>Boolean</code>, set the state to the same
   * value. Otherwise, toggle the state.
   *
  ...
  /**
   * Sees if the first parameter is one of the items in the list. If so, sets preference to <code>
   * true</code>, otherwise <code>false</code>. If there are no parameters, the state is toggled.
   *
   * <ul>
   *   <li>on
   *   <li>true
   ...
   */
  @Override
  protected final void executeParse(final String[] cmdTokens) throws CommandListenerException {
