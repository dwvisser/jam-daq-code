...
final class NewHistogramCmd extends AbstractCommand {
  ...
  protected void execute(final Object[] cmdParams) {
    ...
    final HistogramType hType = type == 1 ? HistogramType.ONE_D_DOUBLE : HistogramType.TWO_D_DOUBLE;
    final int sizeX = (Integer) cmdParams[3];
    final int sizeY = (Integer) cmdParams[4];
    final Group currentGroup =
        (Group) GuiceInjector.getObjectInstance(JamStatus.class).getCurrentGroup();
    Factory.createHistogram(currentGroup, hType.getSampleArray(sizeX, sizeY), name, title);
  }
  ...
  protected void executeParse(final String[] cmdParams) {
    ...
    final Group workingGroup = Factory.createGroup("Working", Group.Type.FILE);
    Factory.createHistogram(workingGroup, hType.getSampleArray(sizeX, sizeY), name, title);
  }
}
