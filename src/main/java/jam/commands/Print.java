...
final class Print extends AbstractPrintingCommand
  ...
  private final transient PropertyChangeListener selectionObserver =
      new SelectionObserver(this, this);
  ...
  Print(final PlotDisplay display) {
    ...
    putValue(NAME, "Print\u2026");
    putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_P, CTRL_MASK));
  ...
  protected void execute(final Object[] cmdParams) {
    if (firstTime) {
      LOGGER.warning(
          "On some systems, it will be necessary to first "
    ...
    if (job.printDialog()) {
      final String name = ((AbstractHistogram) SelectionTree.getCurrentHistogram()).getFullName();
      LOGGER.info("Preparing to send histogram '" + name + "' to printer\u2026");
      try {
        ...
        String mess = getClass().getName() + ": " + e.getMessage();
