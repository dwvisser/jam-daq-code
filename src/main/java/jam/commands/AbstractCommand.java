...
/**
 * Implementation of <code>Commandable</code> interface in which <code>actionPerformed()</code>
 * executes <code>performCommand(null)</code>, which in turn executes the abstract method, <code>
 * execute(null)</code>.
 *
 * @author Ken Swartz
 */
public abstract class AbstractCommand extends AbstractAction implements Commandable {
  ...
  protected static final Logger LOGGER =
      Logger.getLogger(AbstractCommand.class.getPackage().getName());
  ...
  /**
   * Perform a command and log it. This calls <code>execute()</code> with the given parameters.
   *
  ...
  /**
   * Perform a command and log it. This calls <code>executeParse()</code> with the given parameters.
   *
   * @param strCmdParams the command parameters as strings
   */
  public void performParseCommand(final String[] strCmdParams) throws CommandListenerException {
    try {
  ...
  protected Icon loadToolbarIcon(final String path) {
    ...
    if (urlResource == null) {
      JOptionPane.showInputDialog(
          null, "Can't load resource: " + path, "Missing Icon", JOptionPane.ERROR_MESSAGE);
