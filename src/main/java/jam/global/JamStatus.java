...
public final class JamStatus implements AcquisitionStatus, QuerySortMode {
  ...
  private final transient Set<String> overlayNames = new HashSet<>();
  ...
  public List<String> getOverlayHistograms() {
    synchronized (this) {
      return Collections.unmodifiableList(new ArrayList<>(overlayNames));
    }
  }
   ...
   *
   * @param status the current status of the Jam application
   */
  ...
  /**
   * Sets <code>FILE</code> sort mode, and stores the given file as the last file accessed.
   *
   ...
   *
   * @param runState new run state
   */
  public void setRunState(final RunState runState) {
    broadcaster.broadcast(BroadcastEvent.Command.RUN_STATE_CHANGED, runState);
  }
