 ...
 * object itself to decide when it's appropriate to stop after a stop request has been sent to it.
 * Based on code given in "Java Design" by Peter Coad and Mark Mayfield.
 *
...
public class GoodThread extends Thread {
  ...
  /**
   * Simply calls the superclass's constructor of the same signature. We start with the state equal
   * to <code>RUN</code>.
   */
  public GoodThread() {
    super();
  ...
  protected static final Logger LOGGER = Logger.getLogger(GoodThread.class.getPackage().getName());
  ...
  /**
   * Should be called frequently by the thread's <code>run()</code> method. Code in the run() method
   * will decide what to do for a <code>STOP</code> state. If the state is <code>SUSPEND</code>,
   * <code>checkState()</code> takes care of suspending the thread, and it takes a call to <code>
   * setState(int)</code> with either <code>STOP</code> or <code>RUN</code> to get the thread out of
