...
public final class RuntimeSubclassIdentifier {
  ...
  private static final ClassLoader DEF_LOADER = RuntimeSubclassIdentifier.class.getClassLoader();
   ...
   * @param classpath representation of the proper classpath for loading this file
   * @return representation of the proper full reference to the class
   */
  ...
  private static String jarEntryToClassname(final ZipEntry entry, final String starts) {
    final String entryname = entry.getName();
    String classname = null;
    if (entryname.startsWith(starts) && entryname.endsWith(CLASS_EXT)) {
      classname = entryname.substring(0, entryname.length() - CLASS_EXT.length());
  ...
  /**
   * Adds class name to the given collection if it instances of the given superclass can be made
   * with it. If not, does nothing.
   *
   * @param classname the class name in question
   * @param superclass the superclass
   * @param loader the class loader we need to resolve the classname with
  ...
  /**
   * Find all the classes inheriting or implementing a given class in a given package and any
   * sub-packages. WARNING: a jar file as classpath hasn't been implemented.
   *
   * @param <T> type to find implementors of
   * @param classpath folder containing the classpath to search
   * @param superclass the Class object to be assignable to
   * @return an alphabetically ordered set of classes assignable as requested
   */
  public <T> Set<Class<? extends T>> find(final File classpath, final Class<T> superclass) {
    ...
    if (classpath != null) {
      ...
      final Set<Class<?>> addFrom =
          nameSetToClassSet(
              getClassesRecursively(superclass, classpath.getAbsolutePath(), classpath, loader),
              loader);
  ...
  private Set<String> findClassNamesFromJarURL(
      final URL url, final Class<?> toSubclass, final String starts) {
  ...
  /**
   * Given the root of a classpath, find the given class and load it into memory, passing the
   * reference to the Class back to the caller.
   *
  ...
  private Set<Class<?>> nameSetToClassSet(final SortedSet<String> names, final ClassLoader loader) {
