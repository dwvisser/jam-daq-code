...
public class ParameterList implements Iterable<Parameter<?>> {
   ...
   * @see Parameter
   */
  private final transient List<Parameter<?>> parameters = new ArrayList<>();
   ...
   * @see Parameter
   */
  private final transient Map<String, Parameter<?>> parameterTable =
      Collections.synchronizedMap(new HashMap<String, Parameter<?>>());

  /** Data field values. */
  private final transient Map<Parameter<?>, JTextComponent> textData = new HashMap<>();

  /** Fit error field values. */
  private final transient Map<Parameter<?>, JLabel> textError = new HashMap<>();

  /** Checkboxes for finding initial estimates. */
  private final transient Map<Parameter<?>, JCheckBox> cEstimate = new HashMap<>();

  /** Checkboxes for parameter fixing. */
  private final transient Map<Parameter<?>, JCheckBox> cFixValue = new HashMap<>();

  /** Checkboxes for miscellaneous options. */
  private final transient Map<Parameter<?>, JCheckBox> cOption = new HashMap<>();

  /**
   * @param newParameter added to list
   */
  ...
  /**
   * @param which name of parameter to get
   ...
   * @param data the text field associated with it
   */
  protected void associateTextField(final Parameter<?> parameter, final JTextComponent data) {
  ...
  protected void highlightFields(final JLabel statusOut) {
    ...
    if (!tempList.isEmpty()) {
      final StringBuilder temp = new StringBuilder("Click spectrum to set: ");
  ...
  /**
   * @param parameter to associate error label with
   * @param error the label
   */
  protected void associateErrorLabel(final Parameter<Double> parameter, final JLabel error) {
    this.textError.put(parameter, error);
  }

  /**
   * @param parameter to associate estimate checkbox with
   * @param estimate the estimate checkbox
   */
  protected void associateEstimateCheckbox(
      final Parameter<Double> parameter, final JCheckBox estimate) {
  ...
  /**
   * @param parameter to associate the fix checkbox with
   * @param fixed the fix checkbox
   */
  protected void associateFixCheckbox(final Parameter<Double> parameter, final JCheckBox fixed) {
    ...
    for (Parameter<?> parameter : this.parameters) {
      ...
      if (parameter.isDouble() || parameter.isInteger()) {
        ...
        if (parameter.canBeFixed()) {
          this.cFixValue.get(parameter).setSelected(parameter.isFixed());
          setFixed((Parameter<Double>) parameter);
        }
  ...
  protected void associateOptionCheckbox(
      final Parameter<Boolean> parameter, final JCheckBox option) {
  ...
  protected void updateParameterFromDialog(final Parameter<?> parameter) {
    if (parameter.isDouble()) {
      ...
      paramDouble.setValue(Double.valueOf(this.textData.get(parameter).getText().trim()));
      if (parameter.hasErrorBar()) {
        parameter.setError(
            Double.parseDouble(this.textError.get(parameter).getText().substring(1)));
      }
    } else if (parameter.isInteger()) {
      final Parameter<Integer> paramInt = (Parameter<Integer>) parameter;
      paramInt.setValue(Integer.valueOf(this.textData.get(parameter).getText().trim()));
  ...
  @SuppressWarnings("unchecked")
  protected void addParamWestMiddle(
      final Parameter<?> parameter, final JPanel west, final JPanel middle) {
    ...
    if (parameter.isDouble() || parameter.isInteger()) {
      ...
      final JCheckBox option = new JCheckBox(boolParam.getName(), boolParam.getValue());
  ...
  @SuppressWarnings("unchecked")
  protected void addParameterGUI(
      final Parameter<?> parameter, final JPanel west, final JPanel center, final JPanel east) {
    ...
    if (parameter.canBeEstimated()) {
      final JCheckBox estimate = new JCheckBox("Estimate", parameter.estimate);
      final Parameter<Double> paramDouble = (Parameter<Double>) parameter;
