...
final class PlotInteraction implements jam.plot.PlotMouseListener {
  private final transient PlotDisplay plotDisplay;

  private final transient ParameterList parameterList;
  ...
  protected void displayFit(
    ...
    plotDisplay.getPlotContainer().displayFit(signals, background, residuals, lowerLimit);
  }
  ...
  protected void updateParametersFromDialog() throws FitException {
    for (Parameter<?> parameter : this.parameterList) {
      try {
        ...
        this.clear();
        throw new FitException("Invalid input, parameter: " + parameter.getName(), nfe);
      }
    }
  }
}
