...
/**
 * Takes 4 channels as input...limits of background, and limits of peak. Returns a background level
 * and peak intensity.
 *
 * @author Dale Visser
...
public class PeakIntensity extends AbstractFit {
  ...
  private final transient Parameter<Integer> lowChannel =
      new Parameter<>(
          "Low Channel", Parameter.MOUSE, Parameter.NO_OUTPUT, Parameter.INT, Parameter.KNOWN);

  private final transient Parameter<Integer> highChannel =
      new Parameter<>(
          "High Channel", Parameter.MOUSE, Parameter.NO_OUTPUT, Parameter.INT, Parameter.KNOWN);

  private final transient Parameter<Integer> lowPeak =
      new Parameter<>(
          "Low Peak", Parameter.MOUSE, Parameter.NO_OUTPUT, Parameter.INT, Parameter.KNOWN);

  private final transient Parameter<Integer> highPeak =
      new Parameter<>(
          "High Peak", Parameter.MOUSE, Parameter.NO_OUTPUT, Parameter.INT, Parameter.KNOWN);

  /** function <code>Parameter</code>--constant background term */
  private final transient Parameter<Double> paramA, paramB;

  private final transient Parameter<Double> paramPeakArea, paramPeakCentroid;

  /** Class constructor. */
  public PeakIntensity() {
    super("Peak Intensity");
    final Parameter<String> comment = new Parameter<>("Comment", Parameter.TEXT);
  ...
  private void fitSlopedBackground(
      final int chLow, final int chHigh, final int peakLow, final int peakHigh) {
    double var;
    double sum = 0.0;
    double sumX = 0.0;
    double sumY = 0.0;
    ...
    final double bkgdConstError = Math.sqrt((1 + sumX * sumX / (sum * st2)) / sum);
  ...
  private void fitFlatBackground(
      final int chLow, final int chHigh, final int peakLow, final int peakHigh) {
    double sumY = 0.0;
    for (int i = chLow; i <= chHigh; i++) {
      sumY += counts[i];
