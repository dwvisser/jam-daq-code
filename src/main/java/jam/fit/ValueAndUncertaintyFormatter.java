...
public final class ValueAndUncertaintyFormatter {
  ...
  private final transient NumberUtilities util;
  ...
  private int firstSigFig(final double value) {
    if (value <= 0.0) {
      throw new IllegalArgumentException("Can't call firstSigFig with non-positive number.");
    }
    return (int) Math.floor(value / Math.pow(10.0, Math.floor(util.log10(value))));
  }
  ...
  private int decimalPlaces(final double value, final int sigfig) {
    ...
    if (value <= 0.0 || value >= 1.0) {
      throw new IllegalArgumentException("Must call decimalPlaces() with value in (0,1).");
    }
    if (sigfig < 1) {
