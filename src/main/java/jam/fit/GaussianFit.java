...
/**
 * This abstract class uses <code>NonLinearFit</code> to fit a single gaussian peak with a
 * background.. The background is a polynomial up to a quadradic term if desired. (Channel -
 * Centroid) is the term the polynomial is expanded in.
 *
 * @author Dale Visser
 * @version 0.5, 8/31/98
...
public final class GaussianFit extends AbstractNonLinearFit {
  ...
  public GaussianFit() {
    ...
    final Parameter<String> background = new Parameter<>("Background: ", Parameter.TEXT);
    background.setValue("A+B(x-Centroid)+C(x-Centroid)\u00b2");
    final Parameter<String> equation = new Parameter<>("Peak: ", Parameter.TEXT);
    equation.setValue(
        "2.354\u2219Area/(\u221a(2\u03c0)Width)\u2219exp[-2.354\u00b2(x-Centroid)\u00b2/(2"
            + " Width\u00b2)]");
    ...
    paramA = new Parameter<>("A", Parameter.DOUBLE, Parameter.FIX, Parameter.ESTIMATE);
    paramA.setEstimate(true);

    /* function <code>Parameter</code> --linear background term */
    final Parameter<Double> paramB = new Parameter<>("B", Parameter.FIX);
    paramB.setFixed(true);

    /* function <code>Parameter</code> --quadratic background term */
    final Parameter<Double> paramC = new Parameter<>("C", Parameter.FIX);
  ...
  /**
   * Overrides normal setParameters to make sure channels are in proper order. This Allows the fit
   * limits and centroids to be clicked in any order.
   */
  private void orderParameters() {
    final double[] sortMe = {
      this.lowChannel.getValue(), this.centroid.getValue(), this.highChannel.getValue()
    };
    Arrays.sort(sortMe);
   ...
   *
   * @param val value to calculate at
