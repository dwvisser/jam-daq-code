...
public class LoadFit {
  ...
  private static final Logger LOGGER = Logger.getLogger(LoadFit.class.getPackage().getName());
  ...
  private final transient Frame jamMain;
   ...
   * @param display plot display
   * @param broadcaster broadcasts state changes
   * @param rtsi for finding fit classes
   */
  @Inject
  public LoadFit(
      final JFrame frame,
      final PlotDisplay display,
    ...
    final PanelOKApplyCancelButtons.Listener callback =
        new PanelOKApplyCancelButtons.AbstractListener(dialog) {
          ...
          public void apply() {
            final Class<? extends AbstractFit> fit =
                (Class<? extends AbstractFit>) chooseFit.getSelectedItem();
            try {
              makeFit(fit);
  ...
  private Object[] getFitClasses() {
    final Set<Class<? extends AbstractFit>> set = rtsi.find("jam.fit", AbstractFit.class, false);
    set.addAll(rtsi.find("fit", AbstractFit.class, false));
    return set.toArray();
  }

  private void makeFit(final Class<? extends AbstractFit> fitClass) throws JamException {
    ...
    try {
      ...
      final Action fitAction =
          new AbstractAction(fitNameFront) {
      ...
      throw new JamException(" Fit Class cannot instantiate: " + fitName, ie);
