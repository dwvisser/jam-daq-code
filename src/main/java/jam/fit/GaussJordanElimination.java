...
final class GaussJordanElimination {
  ...
  private final transient Matrix inputMatrix; // n by n
  private final transient Matrix inputVectors; // n by m
  ...
  GaussJordanElimination(final Matrix inMatrix, final Matrix InputVector) {
    ...
    if (rows != inputVectors.rows) {
      throw new IllegalArgumentException(rows + " not equal to " + inputVectors.rows + "!!!");
    }
  ...
  private void doPivoting(final int index, final int pivotRow, final int pivotColumn)
      ...
      if (ll != pivotColumn) {
        ...
        for (int l = 0; l < rows; l++) {
          inputMatrix.element[ll][l] =
              inputMatrix.element[ll][l] - inputMatrix.element[pivotColumn][l] * dummy;
        }
        for (int l = 0; l < columns; l++) {
          inputVectors.element[ll][l] =
              inputVectors.element[ll][l] - inputVectors.element[pivotColumn][l] * dummy;
        }
      }
    }
  }
