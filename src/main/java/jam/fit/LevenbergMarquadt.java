...
/**
 * This uses the Levenberg-Marquadt prescription for finding the local minimum of chi-squared given
 * seed parameter values for the function to fit. The <code>NonLinearFit</code> class which creates
 * and calls this must supply the function evaluation and differentiation.
 *
 * @author Dale Visser
 * @version 0.5, 8/28/98
...
final class LevenbergMarquadt {
  ...
  private final transient NonLinearFit nonLinFit;
  ...
  /** smooth variation parameter between using exact solution, or a small step in that direction */
  ...
  private final transient Parameter<Double>[] parameters;
  ...
  private final transient int nPar;
  ...
  private final transient MessageHandler messages;
  ...
  public LevenbergMarquadt(final NonLinearFit nlf) {
    ...
    for (Parameter<?> param : nonLinFit.getParameters()) {
      if (param.isDouble()) {
        final boolean variableParameter = !(param.isOutputOnly() || param.isKnown());
  ...
  protected void setup(
      final double[] counts, final double[] errors, final int minChannel, final int maxChannel) {
   ...
   * @throws ArithmeticException if a math error occurs
  ...
  public void iterate(final Iteration iteration) throws ArithmeticException {
    ...
    for (index = 0; index < nVar; index++) {
      covar.element[index][index] = covar.element[index][index] * (1.0 + lambda);
    ...
    if (!allDone) {
      ...
      for (int l = 0; l < nPar; l++) {
        if (!tryParameters[l].isFixed()) {
          tryParameters[l].setValue(tryParameters[l].getValue() + deltaA.element[index][0]);
          index++;
        }
      }
  ...
  /**
   * Calculates space, vec, and chiSq from passed parameters. To do this, assigns parameter values
   * in nonLinFit
   *
      ...
      for (int l = 0; l < nPar; l++) {
        if (!params[l].isFixed()) {
          ...
          for (int m = 0; m <= l; m++) {
            if (!params[m].isFixed()) {
              column++;
              space.element[row][column] = space.element[row][column] + weight * dyda[m];
            }
          }
  ...
  private void outputParamsPostFit(final Parameter<Double>[] params) {
    ...
    for (Parameter<Double> param : params) {
      final Parameter<Double> nlfParam =
          (Parameter<Double>) nonLinFit.getParameter(param.getName());
