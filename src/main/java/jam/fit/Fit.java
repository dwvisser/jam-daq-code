...
interface Fit {
  ...
  /**
   * Returns the evaluation of a signal at one particular channel. Should return zero if the given
  ...
  /**
   * Calculates function value for a specific channel. Uses whatever the current values are for the
   * parameters in <code>parameters</code>.
   *
  ...
  /**
   * Changes parameter values to calculated estimates. Useful for <code>NonLinearFit</code>, which
   * requires reasonably close guesses in order to converge on a good chi-squared minimum.
   *
