...
/**
 * Class for containing a map of the ADC and TDC modules to be addressed, along with channels and
 * thresholds.
 */
public final class VME_Map {

  /** Contains all event parameters. */
  private final transient List<VME_Channel> eventParams = new ArrayList<>();

  private final transient EventSizeModeClient esmClient;

  /** Interval in milliseconds for the VME to insert scalers into the event stream. */
  private transient int interval = 1;

  private final transient Map<Integer, Byte> V775ranges = new HashMap<>();
   ...
   *
   * @param client the owner of this map
   */
  ...
  /**
   * Creates and adds an event parameter to the list of event parameters. V785 ADC's can have base
   * addresses from 0x20000000-0x2FFF0000. V775 TDC's can have base addresses from
  ...
  public int eventParameter(
      final int slot, final int baseAddress, final int channel, final int threshold)
      throws SortException {
    final VME_Channel vmec =
        new VME_Channel(/* this.messages, */ slot, baseAddress, channel, threshold);
  ...
  /**
   * Sets the time between when the front end is supposed to query the scaler module(s) and insert
   * the scaler values as a block in the event stream.
   *
  ...
  public void setV775Range(final int baseAddress, final int range) throws SortException {
    if (range < 141 || range > 1200) {
      throw new SortException(
          "Requested invalid TDC range: " + range + " ns, must be 141 to 1200 ns.");
    }
  ...
  /**
   * Gets the ranges of the TDC modules as a <code>Map</code>. The keys in the map are the <code>
   * Integer</code> base addresses of the TDC modules, and the values are the <code>Byte</code> that
   * actually gets written to the range register in the module to set the requested range.
   *
