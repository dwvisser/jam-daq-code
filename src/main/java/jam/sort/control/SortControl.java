...
/**
 * Class to control the offline sort process Allows you to enter the list of files to sort an the
 * output pre-sorted file if you have one
 *
 * @author Dale Visser and Ken Swartz
 * @version 1.0
 */
@Singleton
public final class SortControl extends javax.swing.JDialog implements OfflineController {
  ...
  private final transient JButton bbrowse;

  private final transient Action beginAction, haltAction;

  /** check box for writing out events */
  private final transient JCheckBox cout;
  ...
  private final transient MultipleFileChooser multiFile;
  ...
  private final transient JTextField textOutFile;
  ...
  @Inject
  public SortControl(final JFrame frame, final JamStatus status, final Icons icons) {
    ...
    ptop.add(new javax.swing.JLabel("Event Files to Sort", javax.swing.SwingConstants.RIGHT));
    /* List Panel - center - MultipleFileChooser */
    multiFile = new MultipleFileChooser(frame, new File(JamProperties.getPropString(EVENT_INPATH)));
    ...
    return new AbstractAction() {
      { // NOPMD
        ...
        putValue(
            ...
            "Begin sort of all files." + " If a sort was halted, we start over.");
   ...
   * For scripting.
   *
   * @param file to add to files to sort, recurses if folder
  ...
  public int addEventFile(final File file) {
    ...
    if (file != null && file.exists()) {
      final ExtensionFileFilter fileFilter =
          new ExtensionFileFilter(new String[] {"evn"}, "Event Files (*.evn)");
  ...
  /**
   * Called back by sorter when sort encounters a end-run-marker. Tell StorageDaemon to close file.
   * Tells user sorting is done and unlocks fields so that new files can be input to sort.
   */
  ...
  /**
   * Method not implement for SortControl, called by sorting when at end of writing file.
  ...
  public void beginSort() {
    ...
    if (writeEvents) {
      ...
      try {
        ...
        LOGGER.log(SEVERE, "Sort|Control.Begin: couldn't open event output file.", e);
      ...
      if (openSuccess) {
        try {
          ...
          LOGGER.log(SEVERE, "Sort|Control.Begin: couldn't write header to event output file.", e);
        }
      }
    } else {
      sortDaemon.setWriteEnabled(false);
  ...
  public boolean openNextFile() {
    ...
    if (!inputDaemon.closeEventInputListFile()) {
      LOGGER.severe("Could not close file: " + inputDaemon.getEventInputFileName());
    }
    if (inputDaemon.hasMoreFiles()) {
      if (inputDaemon.openEventInputListFile()) {
        LOGGER.info("Sorting next file: " + inputDaemon.getEventInputFileName());
        final RunInfo runInfo = RunInfo.getInstance();
        LOGGER.info("  Run number: " + runInfo.runNumber + " title: " + runInfo.runTitle);
      } else {
        LOGGER.severe("Could not open file: " + inputDaemon.getEventInputFileName());
      }
   ...
   *
   * @param file with list of event files
   * @return number of files added
   */
  public int readList(final File file) {
    ...
    lastFile = file;
    try {
      final BufferedReader reader = new BufferedReader(new java.io.FileReader(lastFile));
      String listItem;
      do {
      ...
      LOGGER.log(SEVERE, "Jam|Sort...: Unable to load list from file " + file, ioe);
    }
    return numFiles;
  }
   ...
   *
   * @param file for pre-sorted events to go to
   */
  ...
  private void endSort() {
    ...
    if (!inputDaemon.closeEventInputListFile()) {
      LOGGER.severe("Closing sort input event file: " + inputDaemon.getEventInputFileName());
    }
    if (writeEvents) {
      try {
        outputDaemon.closeEventOutputFile();
      } catch (SortException e) {
        LOGGER.log(SEVERE, "Sort|Control...: couldn't close event output file.", e);
      }
      LOGGER.info("Closed pre-sorted file: " + fileOut.getPath());
    }
    STATUS.setRunState(RunState.ACQ_OFF);
  ...
  private File getOutFile() {
    ...
    fileChooser.setFileFilter(new ExtensionFileFilter(new String[] {"evn"}, "Event Files (*.evn)"));
  ...
  /** Load the name of objects entered in dialog box give the list to storage deamon */
