 ...
 *
 * <ul>
 *   <li>stops acquisition
 *   <li>closes event file
 *   <li>writes out summary data file
 * </ul>
 *
 * @author Ken Swartz
...
public final class RunControl extends JDialog implements Controller, RunController {
  ...
  /**
   * Method called back from sort package when we are done writing out the event data and have
   * closed the event file.
   *
   ...
   *
   * <OL>
   *   <LI>Get run number and title
   *   <LI>Open file
   *   <LI>Tell disk Daemon the file
   *   <LI>Tell disk Daemon to write header
   *   <LI>Start disk Daemon
   *   <LI>Tell net daemon to send events into pipe
   *   <LI>Tell vme to start
   * </OL>
   *
  ...
  /**
   * End a data taking run tell VME to end, which flushes buffer with a end of run marker When the
   * storageDaemon gets end of run character, it will turn the netDaemon's eventWriter off which
   * flushes and close event file.
   *
   * <p>sort calls back isEndRun when it sees the end of run marker and write out histogram, gates
   * and scalers if requested
   */
  public void endRun() {
    ...
    LOGGER.info(
        "Ending run " + RunInfo.getInstance().runNumber + ", waiting for sorting to finish.");
    int numSeconds = 0;
    ...
    final String histFileName =
        RunInfo.getInstance().experimentName + RunInfo.getInstance().runNumber + ".hdf";
   ...
   *
   * @param value whether Jam is in a run, i.e., saving event data to disk
   */
   ...
   * @param name name of the current experiment
   * @param datapath path to event files
   * @param histpath path to HDF files
   * @param sortD the sorter thread
   * @param netD the network communication thread
   * @param diskD the storage thread
   */
  public void setupOn(
      final String name,
      final File datapath,
      final File histpath,
      final SortDaemon sortD,
      final NetDaemon netD,
  ...
  /** Starts acquisition of data. Figure out if online or offline an run appropriate method. */
  ...
  public void stopAcq() {
    ...
    if (this.isRunOn()) {
      LOGGER.info(
          "Stopped acquisition during a run: you will need to start acquisition again before you"
              + " can end the run.");
    } else {
      /*
