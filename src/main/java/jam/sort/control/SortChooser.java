...
final class SortChooser extends JComboBox<Class<? extends AbstractSortRoutine>> {
  ...
  private final transient List<Class<? extends AbstractSortRoutine>> listClasses =
      new ArrayList<>();

  private final transient RuntimeSubclassIdentifier subclassIdentifier =
  ...
  /**
   * Resolves the String objects into class names and loads the sorting class and event streams.
   *
   * @param userSpecifiedPath whether to use user-specified classPath
  ...
  protected void loadSorter(final boolean userSpecifiedPath) throws JamException {
    ...
    try {
      ...
      throw new JamException("Cannot instantiate sort routine: " + sortClass.getName(), ie);
    } catch (IllegalAccessException iae) {
      throw new JamException("Cannot access sort routine: " + sortClass.getName(), iae);
    }
  }
   ...
   *
   * @param isDefaultPath <code>true</code> to use the default classpath
   */
