...
abstract class AbstractSetup {
  ...
  protected static final Logger LOGGER =
      Logger.getLogger(AbstractSetup.class.getPackage().getName());

  /** Apply button. */
  protected final transient AbstractButton bapply;

  /** Press to browse for a classpath. */
  protected final transient AbstractButton bbrowsef = new JButton("Browse...");

  /** OK button */
  protected final transient AbstractButton bok;

  /** Toggle button for the default class path. */
  protected final transient JToggleButton btnDefaultPath;

  /**
   * When toggled, means that a user-supplied path should be used for populating the sort chooser.
   */
  protected final transient JToggleButton btnSpecifyPath;

  /** The dialog. */
  protected final transient JDialog dialog;
  ...
  protected final transient SortChooser sortChooser;

  /** path to base of sort routines' class path. */
  protected transient File userClassPath;

  /** Text field showing the sort class path. */
  protected final transient JTextField textSortPath;

  AbstractSetup(final String dialogName, final Broadcaster broadcaster) {
    super();
    this.broadcaster = broadcaster;
    // Jam properties needed
    final String defInStream = JamProperties.getPropString(PropertyKeys.EVENT_INSTREAM);
    final String defOutStream = JamProperties.getPropString(PropertyKeys.EVENT_OUTSTREAM);
    final String defSortPath = JamProperties.getPropString(PropertyKeys.SORT_CLASSPATH);
    final boolean useDefault = defSortPath.equals(JamProperties.DEFAULT_SORTPATH);
    final String defSortRoutine = JamProperties.getPropString(PropertyKeys.SORT_ROUTINE);
    ...
    dialog = new JDialog(GuiceInjector.getObjectInstance(JFrame.class), dialogName, false);
    textSortPath = new JTextField(defSortPath);
    textSortPath.addActionListener(
        new ActionListener() {
          public void actionPerformed(final ActionEvent event) {
    ...
    btnSpecifyPath.addItemListener(
        itemEvent -> {
          if (btnSpecifyPath.isSelected()) {
            selectPath(!btnSpecifyPath.isSelected());
          }
        });
    btnDefaultPath = new JRadioButton("Use help.* and sort.* in default classpath", useDefault);
    btnDefaultPath.setToolTipText(
        "Don't include your sort routines in the default"
            + " classpath\n if you want to be able to edit, recompile and reload "
            + " without first quitting Jam.");
    btnDefaultPath.addItemListener(
        event -> {
    ...
    // Input streams
    final Set<Class<? extends AbstractEventInputStream>> inStreams =
        getClasses("jam.sort.stream", AbstractEventInputStream.class);
    ...
    // Output streams
    final Set<Class<? extends AbstractEventOutputStream>> outStreams =
        getClasses("jam.sort.stream", AbstractEventOutputStream.class);
  ...
  protected final File browseSortPath() {
    ...
    final int option = chooser.showOpenDialog(GuiceInjector.getObjectInstance(JFrame.class));
    /* save current values */
   ...
   */
  private <T> Set<Class<? extends T>> getClasses(final String inPackage, final Class<T> inClass) {
    final RuntimeSubclassIdentifier classFinder =
        GuiceInjector.getObjectInstance(RuntimeSubclassIdentifier.class);
    final Set<Class<? extends T>> lhs =
        new java.util.LinkedHashSet<>(classFinder.find(inPackage, inClass, false));
    lhs.remove(inClass);
    return lhs;
  }
   ...
   *
   * @throws JamException error given with useful error message to the final user
   */
  protected final void initializeSorter() throws JamException {
    ...
    try {
      ...
      message
          .append("Exception in AbstractSortRoutine: ")
          .append(sortName)
          .append(".initialize(); Message= '")
      ...
      message
          .append(sortName)
          .append(
              " attempts to allocate too much memory. Reduce its requirments or start Jam with more"
                  + " available heap space. The current maximum amount of memory available to the"
                  + " JVM is ");
  ...
  /** Do what it takes to open up the tree to the first histogram in the sort routine. */
  public void selectFirstSortHistogram() {
    ...
    GuiceInjector.getObjectInstance(JamStatus.class).setCurrentGroup(sortGroup);
  ...
  /**
   * Finds a class matching the given string in the collection, and attempts to select it from the
   * chooser.
   *
   * @param jcb chooser
   * @param collection of <code>Class</code> objects
   * @param defInStream name of class to try to select
   ...
   * @param inStream stream class
   * @param outStream event input event output stream class
   */
  public void setupSort(
      final java.io.File classPath,
