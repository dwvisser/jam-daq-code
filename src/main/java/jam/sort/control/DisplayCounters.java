...
/**
 * Displays buffer counters of sort threads. Gives the number of buffers and events received and
 * sorted.
 *
 * @author Ken Swartz
...
public final class DisplayCounters extends JDialog implements PropertyChangeListener { // NOPMD
  ...
  private final transient CounterPanel pBuffRecv = new CounterPanel("Packets received");

  private final transient CounterPanel pBuffSent = new CounterPanel("Packets sent");

  private final transient CounterPanel pBuffSort = new CounterPanel("Buffers sorted");

  private final transient CounterPanel pBuffWrit = new CounterPanel("Buffers written");

  private final transient JPanel pCenter;

  private final transient CounterPanel pEvntRecv = new CounterPanel("Events received");

  private final transient CounterPanel pEvntSent = new CounterPanel("Events sent");

  private final transient CounterPanel pEvntSort = new CounterPanel("Events sorted");

  private final transient CounterPanel pFileRead = new CounterPanel("Files read");

  private final transient CounterPanel pSortSample = new CounterPanel("Current sampling fraction");
  ...
  private DisplayCounters(
      final JamStatus status, final Frame frame, final Broadcaster broadcaster) {
    ...
    bclear.addActionListener(
        event -> {
          ...
          if (status.getSortMode().isOnline()) {
            ...
            pEvntRecv.setText(String.valueOf(sortDaemon.getEventCount()));
            ...
            pBuffRecv.setText(String.valueOf(netDaemon.getPacketCount()));
            pBuffWrit.setText(space);
            storeDaemon.setBufferCount(0);
            pBuffWrit.setText(String.valueOf(storeDaemon.getBufferCount()));
            ...
            sortDaemon.setEventCount(0);
            sortDaemon.setSortedCount(0);
            pEvntSort.setText(String.valueOf(sortDaemon.getEventCount()));
   ...
   * Setup for online
   *
   * @param net network process
   * @param sod sorting process
   * @param std event record storage process
   */
  public void setupOn(final NetDaemon net, final SortDaemon sod, final AbstractStorageDaemon std) {
    synchronized (this) {
      netDaemon = net;
      sortDaemon = sod;
      storeDaemon = std;
