...
public final class SetupSortOff extends AbstractSetup {
  ...
  private static final String SETUP_LOCKED = "Setup Locked";

  /* dialog box widgets */
  private final transient JCheckBox checkLock = new JCheckBox(SETUP_LOCKED, false);
  ...
  protected SetupSortOff(
    ...
    panelB.add(bok);
    panelB.add(bapply);
    final JButton bcancel = new JButton(new jam.ui.WindowCancelAction(dialog));
    panelB.add(bcancel);
  ...
  protected void doApply(final boolean dispose) {
    try {
      if (this.status.canSetup()) {
        ...
        final JamException exception = new JamException("Can't set up sorting, mode locked.");
  ...
  private void loadEventInput() throws JamException {
    try { // create new event input stream class
      synchronized (this) {
        final Class<? extends AbstractEventInputStream> class1 =
      ...
      final String msg = "Cannot instantiate event input stream: " + inChooser.getSelectedItem();
      LOGGER.log(Level.SEVERE, msg, ie);
      throw new JamException(msg, ie);
    } catch (IllegalAccessException iae) {
      final String msg = "Cannot access event input stream: " + inChooser.getSelectedItem();
  ...
  private void loadEventOutput() throws JamException {
    try { // create new event output stream class
      ...
      throw new JamException(
          "Cannot instantize event output stream: " + outStream.getClass().getName(), ie);
    } catch (IllegalAccessException iae) {
      throw new JamException(
          "Cannot access event output stream: " + outStream.getClass().getName(), iae);
    }
  }
  ...
  protected void lockMode(final boolean lock) {
    ...
    if (lock) {
      status.setSortMode(SortMode.OFFLINE, sortChooser.getSortRoutine().getClass().getName());
  ...
  /**
   * Resets offline data aquisition. Kills sort daemon. Clears all data areas: histograms, gates,
