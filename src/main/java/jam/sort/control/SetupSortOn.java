...
public final class SetupSortOn extends AbstractSetup {
  ...
  private final transient AbstractButton bbrowseh, bbrowsed, bbrowsel;

  /* stuff for dialog box */
  private final transient AbstractButton cdisk = new JCheckBox("Events to Disk", true);

  private final transient AbstractButton checkLock = new JCheckBox("Setup Locked", false);

  private final transient AbstractButton clog = new JCheckBox("Log Commands", false);

  private final transient ConsoleLog consoleLog;
  ...
  private final transient JTextField textPathHist, textPathData, textPathLog;

  private final transient DisplayCounters displayCounters;
  ...
  private SetupSortOn(
    ...
    final JPanel pradio = new JPanel(new FlowLayout(FlowLayout.CENTER, noSpace, noSpace));
    pEntries.add(pradio);
    ...
    textPathHist.setToolTipText("Path to save HDF summary files at the end of each run.");
    ...
    pBottom.add(bok);
    pBottom.add(bapply);
    pBottom.add(new javax.swing.JButton(new jam.ui.WindowCancelAction(dialog)));
  ...
  private JPanel createEntriesPanel(
      final int gap, final int topInset, final int noSpace, final java.awt.Container dcp) {
  ...
  /** Save the names of the experiment, the sort file and the event and histogram directories. */
  ...
  protected void lockMode(final boolean lock) {
    ...
    GuiceInjector.getObjectInstance(JamStatus.class).setSortMode(sortMode, name);
  ...
  private void resetAcq(final boolean killSort) {
    ...
    this.broadcaster.broadcast(jam.global.BroadcastEvent.Command.HISTOGRAM_NEW);
  }

  private void setup(final boolean dispose)
      throws CommunicationsException, JamException, IOException, SortException {
    ...
    if (clog.isSelected()) { // if needed start logging to file
      ...
      final String logFile = consoleLog.setLogFileName(logPathTry.getCanonicalPath());
      LOGGER.info("Logging to file: " + logFile);
      consoleLog.setLogFileOn(true);
    } else {
      consoleLog.setLogFileOn(false);
    }
    consoleLog.messageOutln("Setup Online Data Acquisition,  Experiment Name: " + exptName);
    ...
    if (sortRoutine != null) {
      ...
      consoleLog.messageOutln("Communications and processing daemons successfully initiated.");
  ...
  protected void setupSort() throws jam.sort.SortException, JamException {
    ...
    try { // create new event input stream class
      inStream =
          ((Class<? extends AbstractEventInputStream>) inChooser.getSelectedItem())
    ...
    try { // create new event input stream class
      outStream =
          ((Class<? extends AbstractEventOutputStream>) outChooser.getSelectedItem())
              .getDeclaredConstructor()
              .newInstance();
    ...
    final RingBuffer storageRing = this.ringFactory.create(!cdisk.isSelected());
    ...
    final String hostDataIP = JamProperties.getPropString(PropertyKeys.HOST_DATA_IP);
    final int hostDataPort = JamProperties.getPropInt(PropertyKeys.HOST_DATA_P_RECV);
    netDaemon = new NetDaemon(sortingRing, storageRing, hostDataIP, hostDataPort);

    /* Tell control about everything. */
    final String exptName = textExpName.getText().trim();
    runControl.setupOn(exptName, dataFolder, histFolder, sortDaemon, netDaemon, diskDaemon);
