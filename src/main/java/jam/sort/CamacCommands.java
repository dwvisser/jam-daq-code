...
/**
 * Class for storing and sending CNAF commands to the crate controller. In the user's sort file, an
 * instance of <code>CamacCommands</code> is created, and the CNAF command lists defined one by one.
 *
 * <p>The five command list types are:
 *
 * <ul>
 *   <li>event
 *   <li>init
...
public class CamacCommands {
  private final transient List<CNAF> eventCmds = new ArrayList<>();

  private final transient List<CNAF> initCommands = new ArrayList<>();

  private final transient List<CNAF> scalerCmds = new ArrayList<>();

  private final transient List<CNAF> clearCmds = new ArrayList<>();

  private final transient EventSizeModeClient client;
  ...
  /**
   * The various lists of CNAF commands are initialized by the constructor and initially contain no
   * commands.
   *
   * @param client <code>AbstractSortRoutine</code> to which this object belongs
   * @see jam.sort.AbstractSortRoutine#cnafCommands
   */
  ...
  /**
   * Adds the command specified by the arguments to the next position in the specified event command
   * list.
   *
   ...
   * @return index in eventData array that the <CODE>AbstractSortRoutine</CODE> processes
   */
  public int eventRead(
      final int crate, final int number, final int address, final int function, final int data)
      throws SortException {
    // paramId+1 since stream has 1 for first element
    final int paramId = eventCmds.size() + 1;
    eventSize++;
    eventCmds.add(new CNAF(paramId, crate, number, address, function, data));
    client.setEventSizeMode(EventSizeMode.CNAF);
    return (paramId - 1);
  }

  /**
   * Adds the command specified by the arguments to the next position in the only event command
   * list.
   *
   * @param crate crate number
   * @param number slot containing device to receive the command
   ...
   */
  public int eventRead(final int crate, final int number, final int address, final int function)
      throws SortException {
  ...
  public void eventCommand(
      final int crate, final int number, final int address, final int function) {
    eventCmds.add(new CNAF(0, crate, number, address, function, 0));
  }

  /**
   * Adds the command specified by the arguments to the next position in the init command list.
   *
   * @param crate crate number
   * @param number slot containing device to receive the command
   * @param address address number to receive the command
   ...
   * @param function code indicating the action to be taken
   */
  public void init(final int crate, final int number, final int address, final int function) {
    initCommands.add(new CNAF(0, crate, number, address, function, 0));
  }

  /**
   * Adds the command specified by the arguments to the next position in the scaler command list.
   *
   * @param crate crate number
   * @param number slot containing device to receive the command
   * @param address address number to receive the command
  ...
  public int scaler(
    ...
    scalerCmds.add(new CNAF(scalerId, crate, number, address, function, data));
    return scalerId + 1;
  }
   ...
   * @return number of scaler
   */
  public int scaler(final int crate, final int number, final int address, final int function) {
    return scaler(crate, number, address, function, 0);
  }

  /**
   * Adds the command specified by the arguments to the next position in the clear command list.
   *
   * @param crate crate number
   * @param number slot containing device to receive the command
   * @param address address number to receive the command
   ...
   * @param function code indicating the action to be taken
   */
  public void clear(final int crate, final int number, final int address, final int function) {
