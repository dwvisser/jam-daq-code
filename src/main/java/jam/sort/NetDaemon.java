...
/**
 * <code>NetDeamon</code> receives packets from the network and sends the data data into one or two
 * pipes. One pipe to a <code>SortDeamon</code>, and the other is a <code>TapeDaemon</code> or
 * <code>DiskDaemon</code> if one is activated.
 *
 * @author Ken Swartz
 * @author Dale Visser
 * @version 0.5
...
public final class NetDaemon extends GoodThread {
  ...
  private final transient DatagramSocket dataSocket;

  /** ring buffers for passing events to sorting */
  private final transient RingBuffer sortingRing;

  /** ring buffers for passing events to storage */
  private final transient RingBuffer storageRing;
   ...
   *
   * @param sortRing buffer the events are sent to for sorting
   * @param storeRing buffer the events are sent to for storage
   ...
   */
  public NetDaemon(
      final RingBuffer sortRing, final RingBuffer storeRing, final String host, final int port)
    ...
    try {
      ...
      throw new SortException(
          getClass().getName()
              + "Data socket couldn't bind to address = "
              + host
              + ", port = "
              + port
              + ". (Other copies of Jam running online?)",
          be);
    } catch (IOException e) {
  ...
  /**
   * Called when we've definitely lost a buffer we were meant to receive due to a full ring buffer.
   *
  ...
  public void setEmptyBefore(final boolean state) {
    synchronized (this) {
      if (!emptyBefore && state) {
        final String mesg =
            "The sorting process lost a buffer. Click 'OK' to\n"
        ...
        final boolean confirmed =
            JOptionPane.showConfirmDialog(
                    GuiceInjector.getObjectInstance(JFrame.class),
                    mesg,
                    "Buffer lost. End run?",
  ...
  /**
   * Runs in an infinite loop receiving data from the local net and stuffing it into a couple of
   * pipes.
   *
  ...
  public void receiveLoop() throws SortException, IOException {
    ...
    final byte[] bufferOut = GuiceInjector.getObjectInstance(RingBufferFactory.class).freshBuffer();
    ...
    while (checkState()) { // loop as long as state is RUN
      ...
      if (checkState()) {
        ...
        LOGGER.warning(
            "Warning: received buffer while NetDaemon thread"
                + " state was not RUN: state="
                + this);
      }
    } // end RUN loop
  }
  ...
  public void setWriter(final boolean writerOn) {
    if (storageRing.isNull()) {
      LOGGER.warning(getClass().getName() + ": Can't write out events. There is no ring buffer.");
    } else {
   ...
   *
   * @param action the action which ends the current run
   */
