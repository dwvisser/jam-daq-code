...
/**
 * <code>SimpleRingBuffer</code> is a list of buffers which starts repeating after the last buffer
 * is filled. It allows asynchronous inter-Thread communication. If a buffer is placed in a full
 * ring, an exception is thrown.
 *
 * @author Ken Swartz
 * @version 0.5,0.9
 * @since JDK 1.1
...
public final class SimpleRingBuffer implements RingBuffer {
  ...
  public boolean tryPutBuffer(final byte[] inBuffer) {
    ...
    synchronized (this) {
      ...
      if (isFull()) {
        String message =
            "Lost a buffer in thread \""
                + Thread.currentThread().getName()
                + "\" when putBuffer() called while already full.";
        System.err.println(message);
      } else {
        System.arraycopy(inBuffer, 0, buffer[posPut & MASK], 0, inBuffer.length);
  ...
  /**
   * Clear the buffer. This is a quick operation, since it just involves resetting the put and get
   * pointers.
   */
  public void clear() {
    synchronized (this) {
  ...
  /**
   * Passes back a copy of the current buffer in the given <code>byte</code> array.
   *
      ...
      while (isEmpty()) {
        try {
          ...
          JOptionPane.showMessageDialog(
              null, ie.getMessage(), getClass().getName(), JOptionPane.ERROR_MESSAGE);
        }
      }
