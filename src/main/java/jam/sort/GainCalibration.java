...
/**
 * Utility class for writers of sort routines to easily read in calibrations and use them.
 *
 * @author <a href="mailto:dwvisser@users.sourceforge.net">Dale W Visser</a>
 */
public final class GainCalibration {

  private final transient Map<Integer, Double> gains = new HashMap<>();

  private final transient Map<Integer, Double> offsets = new HashMap<>();
  ...
  /**
   * Returns an integer approximation to the gain-adjusted value for the given parameter id and
   * channel. The formula is: <code>rval=adjustFP(param,value)+random(-gain/2,+gain/2)</code>, where
   * random(min,max) is a uniformly distributed random number between min and max.
   *
  ...
  public int adjust(final int param, final int value) {
    ...
    if (gains.containsKey(param) && offsets.containsKey(param)) {
      ...
      final double offset = gains.get(param);
      final double randomOffset = (Math.random() - 0.5) * gain;
      rval = (int) Math.round(gain * value + offset + randomOffset);
    }
    return rval;
  }

  /**
   * Returns the exact floating point gain-adjusted value for the given parameter id and channel.
   * The formula is: <code>rval=gain*value+offset</code>
   *
   * @param param parameter id
   * @param value channel datum
   * @return exact gain-adjusted value
  ...
  /**
   * Reads in a gain file. The expected format is a text file where every line is lists a parameter
   * id, a gain coefficient and an offset value. Alternately, the offset column may be ommited.
   *
   * @param name of the gain file
   * @param autoSuppress <code>true</code> adjust to zero any value for which no gain coeffiecients
   *     exist, <code>false</code> to simply return the original value back to the user
  ...
  public void gainFile(final String name, final boolean autoSuppress) throws SortException {
    try {
      ...
      throw new SortException(
          "There was an error while reading the gain file, \"" + name + "\".", ioe);
    }
  }

  private int getNumberOfColumns(final InputStream input) throws IOException {
    int rval = 0;
    final LineNumberReader lnr = new LineNumberReader(new InputStreamReader(input));
  ...
  private void readGains(final InputStream input, final int rows, final int columns)
      throws IOException {
    final LineNumberReader lnr = new LineNumberReader(new InputStreamReader(input));
