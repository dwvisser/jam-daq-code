...
/**
 * Abstract class for sort routines which users extend with their own specific code. Defines
 * histograms, gates, scalers, monitors, and parameters. The user writes a sort class which must
 * have the following methods:
 *
 * <dl>
 *   <dt>initialize
 *   <dd>called when the sort process is initialized
 *   <dt>sort
 *   <dd>called for each event
 *   <dt>monitor
 *   <dd>called each time the monitors are updated
 * </dl>
 *
 * <p>There are two online modes now. One is hybrid VME/CAMAC acquisition,and the other is totally
 * VME bus. In the first case, the user must set a list of CAMAC commands. In the second, a list of
 * addresses and thresholds for the VME bus.
 *
 * @author Ken Swartz
 * @author Dale Visser
 * @version 1.4
 ...
 * @see jam.data.Gate
 */
public abstract class AbstractSortRoutine implements Sorter, Beginner, Ender, EventSizeModeClient {

  /** constant to define a 1d histogram type int */
  protected static final HistogramType HIST_1D = HistogramType.ONE_DIM_INT;

  /** constant to define a 1d histogram type double */
  protected static final HistogramType HIST_1D_DBL = HistogramType.ONE_D_DOUBLE;

  /** constant to define a 1d histogram type int */
  protected static final HistogramType HIST_1D_INT = HistogramType.ONE_DIM_INT;

  /** constant to define a 2d histogram type int */
  protected static final HistogramType HIST_2D = HistogramType.TWO_DIM_INT;

  /** constant to define a 1d histogram type double */
  protected static final HistogramType HIST_2D_DBL = HistogramType.TWO_D_DOUBLE;

  /** constant to define a 2d histogram type int */
  protected static final HistogramType HIST_2D_INT = HistogramType.TWO_DIM_INT;
  ...
  protected static HistInt1D createHist1D(final int numCh, final String name) {
    ...
    return (HistInt1D) Factory.createHistogram(sortGroup, new int[numCh], name);
  }
   ...
   * @return a newly allocated histogram
   */
  protected static HistInt1D createHist1D(final int numCh, final String name, final String title) {
    final Group sortGroup = SORT_GROUP_GETTER.getSortGroup();
    return (HistInt1D) Factory.createHistogram(sortGroup, new int[numCh], name, title);
  }
   ...
   * @return a newly allocated histogram
   */
  protected static HistInt1D createHist1D(
      final int numCh,
      final String name,
      final String title,
      final String labelX,
    ...
    final Group sortGroup = SORT_GROUP_GETTER.getSortGroup();
    return (HistInt1D)
        jam.data.Factory.createHistogram(sortGroup, new int[numCh], name, title, labelX, labelY);
  }
   ...
   * @return a newly allocated histogram
   */
  protected static HistInt2D createHist2D(final int chX, final int chY, final String name) {
    final Group sortGroup = SORT_GROUP_GETTER.getSortGroup();
    return (HistInt2D) Factory.createHistogram(sortGroup, new int[chX][chY], name);
  }
   ...
   * @return a newly allocated histogram
   */
  protected static HistInt2D createHist2D(
      final int chX, final int chY, final String name, final String title) {
    final Group sortGroup = SORT_GROUP_GETTER.getSortGroup();
    return (HistInt2D) Factory.createHistogram(sortGroup, new int[chX][chY], name, title);
  }
  ...
  protected static HistInt2D createHist2D(
    ...
    final Group sortGroup = SORT_GROUP_GETTER.getSortGroup();
    return (HistInt2D)
        jam.data.Factory.createHistogram(sortGroup, new int[chX][chY], name, title, labelX, labelY);
  }
  ...
  protected static HistInt2D createHist2D(final int chans, final String name) {
    final Group sortGroup = SORT_GROUP_GETTER.getSortGroup();
    return (HistInt2D) Factory.createHistogram(sortGroup, new int[chans][chans], name);
  }
   ...
   * @return a newly allocated histogram
   */
  protected static HistInt2D createHist2D(final int chans, final String name, final String title) {
    final Group sortGroup = SORT_GROUP_GETTER.getSortGroup();
    return (HistInt2D) Factory.createHistogram(sortGroup, new int[chans][chans], name, title);
  }
  ...
  protected static HistInt2D createHist2D(
    ...
    final Group sortGroup = SORT_GROUP_GETTER.getSortGroup();
    return (HistInt2D)
        jam.data.Factory.createHistogram(
            sortGroup, new int[chans][chans], name, title, labelX, labelY);
  }
  ...
  /**
   * Size of an event to be used for offline sorting. The event size is the maximum number of
   * paramters in an event.
   */
  ...
  protected final transient GainCalibration gains;
   ...
   * @throws IllegalStateException when there is no event size yet
   */
  ...
  /**
   * Required by the <code>Sorter</code> interface. As written always returns zero, and should be
   * overwritten whenever using monitors.
   *
  ...
  /**
   * Explicitly sets the size of a event for offline sorting. Used in the absence of CAMAC or VME
   * specification of parameters. The user has to know what detector each parameter ID represents.
   *
  ...
  /**
   * Sets how the event size is determined. Generally not called explicitly by subclasses.
   *
  ...
  public void setEventSizeMode(final EventSizeMode mode) throws SortException {
    ...
    if ((!evtSizeMode.equals(mode)) && (!evtSizeMode.equals(EventSizeMode.INIT))) {
    ...
    if (mode == EventSizeMode.CNAF
        || mode == EventSizeMode.VME_MAP
      ...
      throw new SortException(
          mess.append("Illegal value for event size mode: ").append(mode).toString());
    }
  }
  ...
  /**
   * Writes an event to the event output stream. Used by the <code>sort()</code> method, if so
   * desired.
   *
