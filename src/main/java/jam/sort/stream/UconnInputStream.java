...
/**
 * This class knows how to handle Uconn Be7. It extends EventInputStream, adding methods for reading
 * events and returning them as int arrays which the sorter can handle.
 *
...
public final class UconnInputStream extends AbstractEventInputStream {
    ...
    try {
      ...
      showMessage(
          "Block fullsize  "
              ...
              + blockNumEvnt);
      /* read in scalers */
      final List<Integer> scalerValues = new ArrayList<>(UconnStreamConstants.NUMBER_SCALERS);
      for (int i = 0; i < UconnStreamConstants.NUMBER_SCALERS; i++) {
        scalerValues.add(dataInput.readInt() & UconnStreamConstants.SCALER_MASK);
      }
        ...
        if (newBlock) {
          ...
          if (skipped < numSkip) {
            throw new EventException(
                "Tried to skip " + numSkip + "bytes. Was only able to skip " + skipped + ".");
          }
          newBlock = true;
  ...
  private void readEventHeader() throws EventException {
    try {
      ...
      throw new EventException(
          "Reading Event header," + ioe.getMessage() + " [UconnnInputStream]", ioe);
    }
  }
    ...
    for (int i = 0; i < eventNumWord; i++) {
      ...
      if ((dataWord & UconnStreamConstants.VSN_MARKER) == 0) { // data word
        ...
        if (vsn < UconnStreamConstants.VSN_TDC) {
          chan =
              (dataWord >> UconnStreamConstants.ADC_CHAN_SHFT) & UconnStreamConstants.ADC_CHAN_MASK;
          ...
          chan =
              (dataWord >> UconnStreamConstants.TDC_CHAN_SHFT) & UconnStreamConstants.TDC_CHAN_MASK;
