...
/**
 * This class knows how to handle the Kmax data format. It extends <code>EventInputStream</code>,
 * adding methods for reading events and returning them as int arrays which the sorter can handle.
 *
...
public final class Kmax6InputStream extends AbstractEventInputStream {
  ...
  // event sizes
  private final transient List<Short> eventsze = new ArrayList<>(5);
  ...
  /**
   * Check for end of run word, may not be valid in the Kmax case, which relies solely on the
  ...
  /**
   * Reads a block from the input stream Expects the stream to be at the beginning of a block It is
   * up to user to ensure this
   *
   * @return whether read is successful or not
   * @throws EventException thrown for errors in event stream
   */
  private boolean readBlockHeader() throws EventException {
    ...
    try {
      ...
      throw new EventException(
          "Reading Block header," + ioe.getMessage() + " [KmaxInputStream]", ioe);
    }
    return rval;
  }
      ...
      try {
        if (newBlock) { // if a new block read in block header
          ...
          throw new IllegalStateException(
              getClass().getName() + ": Block Event Type >5: " + blockEventType);
        }
  ...
  /**
   * Read in the header Format of KMaxo data Implemented <code>EventInputStream</code> abstract
   * method.
   *
  ...
  public boolean readHeader() throws EventException {
    try {
      ...
      throw new EventException(
          "Reading event header from IOException " + ioe.getMessage() + " [KmaxInputStream]", ioe);
    }
  }
}
