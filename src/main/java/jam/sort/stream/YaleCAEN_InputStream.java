...
/**
 * This class sorts the native data format of the CAEN V7x5 ADC's and TDC's. Parameters are mapped
 * from the slot number and unit channel # to indices in the int array that gets passed to sort
 * routines.
 *
 * @version 18 Nov 2001
 * @author Dale Visser
...
public class YaleCAEN_InputStream extends AbstractL002HeaderReader {
  ...
  private enum BufferStatus {
    /** Indicates the state where flushing of the remaining contents of the buffer is occuring. */
    FIFO_ENDRUN_FLUSH,
    ...
    /**
     * This is the state when the EventStream has characters in it indicating that acquisition has
     * been stopped or ended. In this situation, all data has been read out from the ADC's and been
     * sent to Jam. So the stream needs to empty out it's remaining contents to the sort routine.
     */
    FIFO_FLUSH,

    /**
     * In this state every new read from the stream requires that the oldest event in the the "FIFO"
     * buffer be pulled to be returned so as to make room for a new event counter.
     */
    FIFO_FULL,

    /** Indicates the state where we're reading through end-of-buffer padding characters. */
    PADDING,
  ...
  private static final int END_COMPARE = 0x4000000;

  private static final short ENDRUN = (short) (CAEN_StreamFields.END_PAD & 0xffff);

  private static final int HEAD_COMPARE = 0x2000000;

  private static final int PARM_COMPARE = 0x0000000;

  private static final int TYPE_MASK = 0x7000000;

  // event counter/event should be retrieved
  // from
  private final transient int[] eventNumbers = new int[CAEN_StreamFields.BUFFER_DEPTH];

  /** Keys are the event numbers, values are the array indices. */
  private final transient Map<Integer, Integer> eventNumMap =
      Collections.synchronizedMap(new HashMap<Integer, Integer>(CAEN_StreamFields.BUFFER_DEPTH));

  private final transient int[][] fifo =
      new int[CAEN_StreamFields.BUFFER_DEPTH][CAEN_StreamFields.NUM_CHANNELS];
  ...
  private final transient int[] tempData = new int[32];

  private final transient int[] tempParams = new int[32];

  /**
   * Make sure to issue a setConsole() after using this constructor. It is here to satisfy the
   * requirements of Class.newInstance()
   */
   ...
   */
  private void handleEndBlock(final int endblock, final int numParams) throws EventException {
  ...
  /**
   * Reads an event from the input stream Expects the stream position to be the beginning of an
   * event. It is up to the user to ensure this.
   *
  ...
  public EventInputStatus readEvent(final int[] data) throws EventException {
    synchronized (this) {
      ...
      try {
        ...
        LOGGER.warning(
            getClass().getName()
                + ".readEvent(): End of File reached...file may be corrupted, or run not ended"
                + " properly.");
      } catch (IOException ioe) { // we got to the end of a file or stream
        rval = EventInputStatus.UNKNOWN_WORD;
        LOGGER.warning(getClass().getName() + ".readEvent(): Problem reading integer from stream.");
      } catch (EventException e) {
        //                rval = EventInputStatus.UNKNOWN_WORD;
        throw new EventException(
            getClass().getName() + ".readEvent() parameter = " + lastParameterRead, e);
      }
      return rval;
    }
  }
   ...
   *
   * @param header the header word that was before the parameters
   * @return the last parameter word that was read
   * @throws IOException if there is a problem reading data
   * @throws EventException if an unexpected word is read
   */
  private int readEventParameters(final int header) throws IOException, EventException {
    ...
    while (keepGoing) {
      ...
      if (isParameter(lastParameterRead)) {
        ...
        throw new EventException(
            getClass().getName()
                + ".readEvent(): didn't get a Parameter or End-of-Block when expected, int datum ="
                + " 0x"
