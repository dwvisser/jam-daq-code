...
/**
 * This class knows how to handle Oak Ridge tape format (with special headers as used by Charles
 * Barton's group). It extends EventInputStream, adding methods for reading events and returning
 * them as int arrays which the sorter can handle.
 *
...
public final class L003InputStream extends AbstractEventInputStream {
  ...
  /**
   * Implementation of <code>EventInputStream</code> abstract method. Note, this reads a Vax byte
   * order version of L002 headers unlike L002HeaderRecord.
   *
  ...
  public boolean readHeader() throws EventException {
    ...
    try {
      ...
      throw new EventException("Reading event header -" + ioe.getMessage(), ioe);
    }
  }
    ...
    try {
      ...
      if (paramWord == L002Parameters.EVENT_END_MARKER) {
        ...
        throw new EventException(
            "L003InputStream parameter value: " + paramWord + " [L003InputStream]");
      } else {
  ...
  private boolean scalerRead() throws IOException {
    ...
    while ((nScalerBytes < SCALER_REC_SIZE) && bytesRead >= 0) {
      if (nScalerBytes < SCALER_REC_SIZE - SCALER_BUFF_SIZE) {
        ...
        bytesRead = dataInput.read(scalerDump, 0, SCALER_REC_SIZE - nScalerBytes);
      }
      if (bytesRead != SCALER_BUFF_SIZE) {
        showWarningMessage("scaler Not a full read, only read in " + bytesRead + " bytes");
      }
      if (bytesRead > 0) {
