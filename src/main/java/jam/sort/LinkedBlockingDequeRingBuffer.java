...
/**
 * <code>RingBuffer</code> is a list of buffers which starts repeating after the last buffer is
 * filled. It allows asynchronous inter-Thread communication. If a buffer is placed in a full ring,
...
public final class LinkedBlockingDequeRingBuffer implements RingBuffer {
  ...
  private final transient LinkedBlockingDeque<byte[]> poolStack =
      new LinkedBlockingDeque<>(NUMBER_BUFFERS);

  private final transient boolean hasRing;

  /**
   * Creates a new ring buffer with or without a backing deque.
   *
   * @param empty whether this is a no-capacity ring buffer
   */
  ...
  private void validateBuffer(final byte[] inbuffer) {
    ...
    if (inbuffer.length != BUFFER_SIZE) {
      throw new IllegalArgumentException("buffer capacity expected to be " + BUFFER_SIZE);
    }
  }
  ...
  /**
   * Passes back a copy of the current buffer in the given <code>byte</code> array. Blocks until the
  ...
  /**
   * Tells you if the ring buffer is empty. Used to check if you have read all the buffers in the
   * ring.
   *
  ...
  /**
   * Gets whether the ring buffer is close to filling, defined as approximately 97% full.
   *
