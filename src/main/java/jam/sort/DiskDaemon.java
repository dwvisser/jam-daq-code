...
public final class DiskDaemon extends AbstractStorageDaemon {
  ...
  private final transient Object rreLock = new Object();
  ...
  public void closeEventInputFile() throws SortException {
    if (inputFileOpen) {
      try {
        ...
        throw new SortException("Unable to close file [DiskDaemon]", ioe);
      }
    }
  }

  /** Close event input file that is from the list, if one from the list is open. */
  @Override
  public boolean closeEventInputListFile() {
    ...
    try {
      ...
      LOGGER.log(Level.SEVERE, "Unable to close file: " + inputFile.getPath(), ioe);
      rval = false;
    }
    return rval;
  }
    ...
    if (outputFileOpen) {
      try {
        ...
        throw new SortException(
            "Unable to close file EventException:" + ee.getMessage() + " [DiskDaemon]", ee);
      } catch (IOException ioe) {
        throw new SortException("Unable to close file [DiskDaemon]", ioe);
      }
    }
  ...
  public void openEventInputFile(final File file) throws SortException {
    if (file == null) {
      final SortException exception =
          new SortException(getClass().getName() + ": Cannot open input event file, null name.");
    ...
    try {
      ...
      throw new SortException("Unable to open file: " + file.getPath() + " [DiskDaemon]", ioe);
    }
  }
  ...
  public boolean openEventInputListFile() {
    ...
    try {
      ...
      if (rval) {
        ...
        LOGGER.severe("File does not have correct header. File: " + file.getAbsolutePath());
      }
  ...
  public void openEventOutputFile(final File file) throws SortException {
    if (file == null) {
      throw new SortException(
          getClass().getName() + ": Cannot open output event file, file name is null.");
    }
    try {
  ...
  public boolean readHeader() throws SortException {
    try {
      final BufferedInputStream headerInputStream =
          new BufferedInputStream(new FileInputStream(inputFile), eventInput.getHeaderSize());
  ...
  public void run() {
    try {
      if (mode == Mode.ONLINE) {
        ...
        throw new IllegalStateException("run() called when mode not ONLINE");
      }
    } catch (IOException ioe) {
      LOGGER.log(Level.SEVERE, "Error while writing data to file: " + ioe.getMessage(), ioe);
    }
  }
  ...
  public void writeHeader() throws SortException {
    try {
      ...
      throw new SortException("Could not write Header Record [DiskDaemon]", ioe);
    }
  }
  ...
  private void writeLoop() throws IOException {
    final NumberUtilities numberUtilities = GuiceInjector.getObjectInstance(NumberUtilities.class);
    ...
    while (checkState()) {
      ...
      final short last2bytes = numberUtilities.bytesToShort(buffer, offset, ByteOrder.BIG_ENDIAN);
