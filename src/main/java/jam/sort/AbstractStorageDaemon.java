...
/**
 * General class to read and write event data streams from/to storage. For online takes events from
 * a <code>RingBuffer</code> and writes them out to a storage device. For offline takes events from
 * a storage device and pipes them to an <code>EventInputStream</code>.
 *
 * @author Ken Swartz
 * @version 0.5
 * @since JDK 1.1
...
public abstract class AbstractStorageDaemon extends GoodThread {
  ...
  /**
   * Closes the current file in the list, does not throw exception so it can be easily put in loop.
   ...
   * @exception SortException thrown if an unrecoverable error occurs in a <code>jam.sort</code>
   *     class
   * @exception EventException thrown if an unrecoverable error occurs in an event stream class
   */
  public abstract InputStream getEventInputFileStream() throws SortException, EventException;
   ...
   * @exception EventException thrown if an unrecoverable error occurs in an event stream class
   */
  public abstract OutputStream getEventOutputFileStream() throws SortException, EventException;
  ...
  /**
   * Open the next file in the list, does not throw exception so it can be easily put in loop.
  ...
  public void setupOff(
      final AbstractEventInputStream eventInputStream,
