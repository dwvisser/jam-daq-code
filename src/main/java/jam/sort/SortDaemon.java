...
/**
 * The daemon (background thread) which sorts data. It takes an <code>EventInputStream</code>, and a
 * <code>Sorter</code> class. It reads events from the <code>EventInputStream</code> and gives these
 * to the <code>Sorter</code> method <code>sort(int [])</code>. Last modified 18 December 1999 to
 * use RingInputStream KBS.
 *
 * @author Ken Swartz
 * @author Dale Visser
 * @version 1.1
...
public class SortDaemon extends GoodThread {
  ...
  /** allocate only once to avoid GC having to collect abandoned references in sortOnline() loop */
  ...
  private final transient Object offlineSortLock = new Object();
  ...
  /**
   * Call to gracefully interrupt and end the current offline sort. This is used as one of the
   * conditions to read in the next buffer
   */
   ...
   *
   * @return <code>true</code> if there are no unsorted buffers in the ring buffer
   */
  ...
  private void handleStatusOffline() {
    if (eventInputStatus == EventInputStatus.END_BUFFER) {
      ...
      LOGGER.warning(getClass().getName() + ".sortOffline(): Unknown word in event stream.");
    } else {
      updateCounters();
      endSort = true;
      if (!offlineSortingCanceled()) {
        throw new IllegalStateException("Illegal post-readEvent() status = " + eventInputStatus);
      }
    }
  }
  ...
  private void handleStatusOnline() throws SortException {
    if (eventInputStatus == EventInputStatus.END_BUFFER) {
      ...
      throw new SortException("Sorter stopped due to unknown status: " + eventInputStatus);
    }
  }
  ...
  private void increaseSortInterval() {
    synchronized (this) {
      sortInterval++;
      LOGGER.warning(
          "Sorting ring buffer half-full." + " Sort interval increased to " + sortInterval + ".");
    }
  }
  ...
  public void run() {
    ...
    try {
      if (GuiceInjector.getObjectInstance(JamStatus.class).isOnline()) {
        sortOnline();
      } else {
        sortOffline();
      }
   ...
   *
   * @param newSorter an object capable of sorting event data
   */
  ...
  /**
   * Sets the sort sample interval This is the frequeny of buffers sent to the sort routine. For
   * example if this is set to 2 only every second buffer is sent to the sort routine.
   *
   ...
   */
  public void setup(final AbstractEventInputStream eventInputStream, final int eventSize) {
  ...
  private void sortOffline() throws Exception { // NOPMD
    ...
    while (checkState()) {
      ...
      while (!this.offlineSortingCanceled() && offlineController.openNextFile()) {
        /*
  ...
  public void sortOnline() throws Exception { // NOPMD
    ...
    final int[] eventData = new int[eventSize];
    final byte[] buffer = GuiceInjector.getObjectInstance(RingBufferFactory.class).freshBuffer();
  ...
  /**
   * Invokes begin() in the user's sort routine if it implements the <code>Beginner</code>
   * interface.
   *
  ...
  /**
   * Invokes end() in the user's sort routine if it implements the <code>Ender</code> interface.
   *
   * @see jam.sort.Ender
   */
  public void userEnd() {
