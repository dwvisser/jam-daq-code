...
/**
 * Represents a single channel of data that the acquisition electronics will read out. Used by
 * VME_Map to store its info.
 *
 * @author <a href=mailto:dwvisser@users.sourceforge.net>Dale Visser</a>
 */
public final class VME_Channel {

  private final transient int slot, baseAddress, channel, threshold;

  private final transient Type type;

  private static final String S_INVALID = ".VMEChannel(): Invalid";

  /** Encapsulates whether a parameter is an event parameter or a scaler parameter. */
  enum Type {
  ...
  /**
   * Creates an event parameter. V775/V785 TDC's/ADC's can have base addresses from
   * 0x20000000-0xe0ff0000.
   *
   ...
   */
  VME_Channel(final int slot, final int baseAddress, final int channel, final int threshold)
      throws SortException {
    super();
    if (channel >= 0 && channel < 32) {
      this.channel = channel;
    } else {
      throw new SortException(getClass().getName() + S_INVALID + " channel = " + channel);
    }
    if (slot >= 2 && slot <= 20) { // valid slots for ADC's and TDC's
      this.slot = slot;
    } else {
      throw new SortException(getClass().getName() + S_INVALID + " slot = " + slot);
    }
    ...
    if (threshold >= 0 && threshold < 4096) {
      ...
      throw new SortException(getClass().getName() + S_INVALID + " threshold = " + threshold);
    }
    type = Type.EVENT;
  }

  /**
   * Gets the index in the event stream that this parameter is associated with.
   *
  ...
  /**
   * Get the threshold channel defined for the event parameter. The threshold channel is a lower
   * value threshold that the ADC or TDC requires for storing a parameter.
   *
