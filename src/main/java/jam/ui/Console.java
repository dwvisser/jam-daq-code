...
public class Console extends JPanel {
  ...
  private final transient LinkedList<String> cmdStack = new LinkedList<>(); // NOPMD
  ...
  private final transient List<CommandListener> listenerList =
      Collections.synchronizedList(new ArrayList<CommandListener>());

  private final transient JTextField textIn = new JTextField();

  /**
   * Constructs a JamConsole which has an text area for output a text field for input.
   *
   * @param consoleLog log of console activity
   * @param finder finds commands
   * @param listener listens to commands
   */
  @Inject
  public Console(
  ...
  private void notifyListeners(final String cmd, final String[] params) {
    ...
    if (!found) {
      ...
      final Collection<String> offer = commandFinder.getSimilar(cmd, true);
      if (!offer.isEmpty()) {
  ...
  private void parseCommand(final String _inString) {
    ...
    if (numWords > 0) { // check at least something was entered
      ...
      if (parameters.length > 0) {
        System.arraycopy(
            cmdTokens.toArray(new String[0]), initIndex, parameters, 0, numWords - initIndex);
      }
      /* perform command */
