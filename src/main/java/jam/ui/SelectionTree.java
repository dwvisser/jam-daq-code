...
/**
 * Implements a <code>JTree</code> for selecting <code>Histogram</code>'s and <code>Gate</code>'s to
 * display.
 *
 * @author Ken Swartz
 */
public final class SelectionTree extends JPanel implements PropertyChangeListener {
  ...
  private static final Logger LOGGER = Logger.getLogger(SelectionTree.class.getPackage().getName());
  ...
  private final transient TreeSelectionListener listener =
   ...
   * @param runStateBox run state widget
   * @param broadcaster broadcasts state changes
   * @param treeCellRender renders the tree elements
   */
  @Inject
  public SelectionTree(
      final JamStatus status,
      final RunStateBox runStateBox,
  ...
  private void addHistNodes(final DefaultMutableTreeNode groupNode, final AbstractHistogram hist) {
  ...
  private AbstractHistogram getAssociatedHist(final TreePath path) {
    ...
    return (AbstractHistogram)
        ((DefaultMutableTreeNode) path.getPathComponent(path.getPathCount() - 2)).getUserObject();
  }

  private boolean isGate(final TreePath path) {
    return ((DefaultMutableTreeNode) path.getLastPathComponent()).getUserObject() instanceof Gate;
  }
  ...
  private TreePath pathForDataObject(final Object dataObject) {
    ...
    final Enumeration<?> nodeEnum =
        ((DefaultMutableTreeNode) treeModel.getRoot()).breadthFirstEnumeration();
    while (nodeEnum.hasMoreElements()) {
      final DefaultMutableTreeNode loopNode = (DefaultMutableTreeNode) nodeEnum.nextElement();
   ...
   * non-javadoc
   */
  private void refreshGateSelection(final Gate gate, final TreePath histTreePath) {
    /* Iterate over all nodes below histogram node. */
    final Enumeration<?> nodeEnum =
        ((DefaultMutableTreeNode) histTreePath.getLastPathComponent()).breadthFirstEnumeration();
    while (nodeEnum.hasMoreElements()) {
      final DefaultMutableTreeNode currentNode = (DefaultMutableTreeNode) nodeEnum.nextElement();
  ...
  private void refreshSelection() {
    ...
    final List<AbstractHist1D> overlayHists =
        AbstractHistogram.getHistogramList(status.getOverlayHistograms(), AbstractHist1D.class);
    ...
    synchronized (this) {
      ...
      if (!isSyncEvent()) {
        ...
        final DefaultMutableTreeNode firstNode =
            ((DefaultMutableTreeNode) prime.getLastPathComponent());
  ...
  /**
   * A gate has been selected. Tell all appropriate classes, like Display and JamStatus.
   *
   * @param gate to select
   * @see jam.data.Gate
   */
