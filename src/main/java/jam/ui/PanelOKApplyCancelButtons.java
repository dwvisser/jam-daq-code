...
public final class PanelOKApplyCancelButtons {
  ...
  private final transient JPanel panel = new JPanel(new FlowLayout(FlowLayout.CENTER));
  ...
  /**
   * Handler for OK, apply and cancel methods which are called when their associated buttons are
   * pressed.
   *
  ...
  /**
   * Default implementation of <code>Listener</code> has <code>doOK()</code> execute <code>apply()
   * </code>, then <code>cancel</code>, which makes the given <code>Window</code> invisible.
   *
   * @author <a href="mailto:dwvisser@users.sourceforge.net">Dale W Visser</a>
   */
  public abstract static class AbstractListener implements Listener {
     ...
     *
     * @param window the ultimate container we wish to make disappear on cancel and OK
     */
