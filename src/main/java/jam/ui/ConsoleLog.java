...
public final class ConsoleLog implements MessageHandler {
  private static final transient SimpleAttributeSet ATR_WARN, ATR_ERR;
  ...
  private final transient JTextPane textLog = new JTextPane();

  private final transient Document doc = textLog.getStyledDocument();

  private final transient JScrollPane jsp =
      new JScrollPane(
          textLog,
  ...
  public ConsoleLog() {
    super();
    textLog.setToolTipText("After setup, this log is (usually) written to a file, too.");
    textLog.setEditable(false);
    final int lineHeight = textLog.getFontMetrics(textLog.getFont()).getHeight();
  ...
  /**
   * Outputs the string as a message to the console, which has more than one part, so message can
  ...
  public void messageOut(final String message, final int part) {
    synchronized (this) {
      ...
      if (part == NEW) {
        ...
        try {
          doc.insertString(doc.getLength(), mbuff.toString(), SimpleAttributeSet.EMPTY);
        } catch (BadLocationException e) {
          JOptionPane.showMessageDialog(
              textLog, e.getMessage(), getClass().getName(), JOptionPane.ERROR_MESSAGE);
        }
  ...
  /**
   * Create a file for the log to be saved to. The method appends a number (starting at 1) to the
   * file name if the file already exists.
   *
  ...
  public String setLogFileName(final String name) throws JamException {
    ...
    try {
      ...
      throw new JamException("Problem opening log file " + file.getAbsolutePath(), ioe);
    }
    return newName;
  }
