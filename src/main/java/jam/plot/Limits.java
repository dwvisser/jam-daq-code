 ...
 *
 * <ul>
 *   <li>x channel display limits
 *   <li>y channel display limits
 *   <li>count range display limits
 *   <li>whether scale is linear or log
 * </ul>
 *
 * There is a separate instance of <code>Limits</code> for every <code>Histogram</code>. The class
 * contains a <code>static
 * Hashtable</code> referring to all the <code>Limits</code> objects by the associated <code>
 * Histogram</code> object.
 *
 * @author Ken Swartz
 * @author Dale Visser
 * @version 1.4
...
final class Limits {
  ...
  private static final Map<String, Limits> TABLE =
      Collections.synchronizedMap(new HashMap<String, Limits>());
  ...
  private final transient String histName;

  private final transient BoundedRangeModel rangeModelX = new DefaultBoundedRangeModel();

  private final transient BoundedRangeModel rangeModelY = new DefaultBoundedRangeModel();

  private int minimumX, maximumX;
  private int minimumY, maximumY;
  private int minimumCounts, maximumCounts;

  private final transient int sizeX; // translate to rangemodel min, max

  private final transient int sizeY; // translate to rangemodel min, max
  ...
  /**
   * Creates the display limits for the specified histogram, specifying whether to ignore first
   * and/or last channels for auto-scaling.
   *
   ...
   */
  private Limits(final AbstractHistogram hist, final boolean ignoreZero, final boolean ignoreFull) {
    super();
    if (hist == null) {
  ...
  /**
   * Determines initial limit values, X and Y limits set to extremes for the given histogram, and
   * the counts scale is auto-scaled.
   *
   * @param ignoreZero true if the zero channel is ignored for auto-scaling
   * @param ignoreFull true if the last channel is ignored for auto-scaling
   */
  ...
  private int getMaxCounts(final int chminX, final int chmaxX, final int chminY, final int chmaxY) {
    int maxCounts;
    ...
    if (histogram.getDimensionality() == 1) {
      maxCounts = getMaxCounts1D((AbstractHist1D) histogram, chminX, chmaxX);
    } else { // dim==2
  ...
  private int getMaxCounts1D(final AbstractHist1D hist, final int chminX, final int chmaxX) {
  ...
  private int getMaxCounts2D(
      final AbstractHist2D hist,
      final int chminX,
      final int chmaxX,
      final int chminY,
      final int chmaxY) {
  ...
  protected static Limits getLimits(final AbstractHistogram hist) {
    ...
    if (hist == null) {
      ...
      if (object == null) {
        ...
        final boolean ignoreZero = prefs.getBoolean(PlotPreferences.AUTO_IGNORE_ZERO, true);
        final boolean ignoreFull = prefs.getBoolean(PlotPreferences.AUTO_IGNORE_FULL, true);
   ...
   *
   * @param newScale one of <code>Limits.LINEAR</code> or <code>
   * Limits.LOG</code>
   */
  ...
  public String toString() {
    String temp =
        "Limits of \"" + histName + "\": " + "MinX: " + getMinimumX() + ", MaxX: " + getMaximumX();
    return temp;
  }
}
