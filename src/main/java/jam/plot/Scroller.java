...
class Scroller extends JPanel implements AdjustmentListener, Limitable {
  ...
  private final transient AbstractPlot plot;

  private final transient boolean isPlot2d;
  ...
  private final transient MouseAdapter mouseListener =
      new MouseAdapter() {
  ...
  /**
   * Set the Limits that the scroll bars are connected to the models in Limits.
   *
   ...
   *
   * @param event adjustment event message
   */
    ...
    synchronized (LOCK) {
      if (plot.hasHistogram()) {
        ...
        if (source.equals(scrollCount)) {
          // scale scroll bar
          if ((scrollValue != lastScrollValue) || (scrollVisible != lastScrollVisible)) {
  ...
  /** called to reset the counts scroll bar used because mouse release is not alwayed call */
  public void update() {
    synchronized (LOCK) {
      if (plot.hasHistogram()) {
  ...
  private void countChange(final int scrollValue, final int scrollVisible) {
    synchronized (LOCK) {
      ...
      // are decreasing
      int newMax = (int) Math.round(lastCountMax * Math.pow(scaleChange, sign));
