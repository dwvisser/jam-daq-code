...
final class Plot2d extends AbstractPlot {
  ...
  private final transient Rectangle areaMark = new Rectangle();

  // only used in one method, but don't want to keep creating
  private final transient Rectangle clipBounds = new Rectangle(); // NOPMD

  private transient double[][] counts2d = EMPTY;

  /** last pixel point added to gate list */
  private final transient Point lastGatePoint = new Point();

  private final transient Object monitor = new Object();

  /** clip to use when repainting for mouse movement, in graphics coordinates */
  private final transient Polygon mouseMoveClip = new Polygon();

  private final transient PlotColorMap plotColorMap = PlotColorMap.getInstance();
  ...
  Plot2d(final PlotSelection plotSelection) {
    ...
    setSmoothColorScale(PlotPreferences.PREFS.getBoolean(ColorPrefs.SMOOTH_SCALE, true));
  }

  /**
   * Add to the selection clip region, using the two given graphics-coordinates points to indicate
   * the corners of a rectangular region of channels that needs to be included.
   *
  ...
  protected void copyCounts(final AbstractHistogram hist) {
    ...
    if (type == HistogramType.TWO_DIM_INT) {
      ...
      for (int i = 0; i < hist.getSizeX(); i++) {
        System.arraycopy(counts2dDble[i], 0, counts2d[i], 0, hist.getSizeY());
      }
    }
  }
   ...
   *
   * @param mode one of GATE_NEW, GATE_CONTINUE, GATE_REMOVE, GATE_SAVE or GATE_CANCEL
   * @param pChannel the channel coordinates of the point
   * @param pPixel the plot coordinates of the point
   */
  @Override
  protected void displaySetGate(final GateSetMode mode, final Bin pChannel, final Point pPixel) {
    if (mode == GateSetMode.GATE_NEW) {
      ...
      if (pointsGate.npoints > 0) {
        ...
        if ((pointsGate.npoints > 0)) { // go back a point
          ...
          final Bin lpoint = Bin.create(pointsGate.xpoints[last], pointsGate.ypoints[last]);
          /* update variables */
   ...
   *
   * @return the maximum counts in the region of currently displayed 2d Histogram
   */
  @Override
   ...
   *
   * @return the minimum counts in the region of currently displayed 2d Histogram
   */
  @Override
  ...
  /**
   * Given a shape, return the bounding rectangle which includes all pixels in the rectangle
   * bounding the given shape, plus some extra space given by the plot channels just outside the
   * edge of this rectangle.
   *
   * @param clipShape the shape we want to cover with a rectangular clip region
   * @param chanCoords if <code>true</code>, the given shape is assumed given in channel
   *     coordinates, otherwise it is assumed given in graphics coordinates
   * @return a bounding rectangle in the graphics coordinates
   */
  private Rectangle getClipBounds(final Shape clipShape, final boolean chanCoords) {
    ...
    if (chanCoords) { // shape is in channel coordinates
      ...
      final Bin bin2 = Bin.create(bin1.getX() + box.width, bin1.getY() + box.height);
      /* now do conversion */
      ...
      final Bin bin2 = painter.toData(new Point(box.x + box.width, box.y + box.height));
   ...
   *
   * @param point the channel to get counts for
   ...
   *
   * @param bin1 a corner of the rectangle in plot coordinates
   * @param bin2 a corner of the rectangle in plot coordinates
   */
  @Override
  ...
  /**
   * Mouse moved so update drawing gate or marking area as appropriate For gate undo last line draw,
   * and draw a new line.
   *
   * @param event created when the mouse pointer moves while in the plot
   */
  @Override
  public void mouseMoved(final MouseEvent event) {
    if (panel.isSettingGate()) {
      ...
      synchronized (lastMovePoint) {
        ...
        lastMovePoint.setLocation(painter.toData(event.getPoint()).getPoint());
        addToSelectClip(plotSelection.start, Bin.create(lastMovePoint));
      }
      panel.setMouseMoved(true);
      synchronized (plotSelection.areaClip) {
   ...
   *
   * @param context the graphics context to paint on
   */
  @Override
  ...
  /**
   * Called to draw a 2d histogram, including title, border, tickmarks, tickmark labels and last but
   * not least update the scrollbars.
   *
   * @param context the graphics context to paint to
   */
  @Override
  public void paintHistogram(final Graphics context) {
    ...
    painter.drawAxisLabel(
        axisLabelX == null ? X_LABEL_2D : axisLabelX, javax.swing.SwingConstants.BOTTOM);
    final String axisLabelY = plotHist.getLabelY();
    painter.drawAxisLabel(
        axisLabelY == null ? Y_LABEL_2D : axisLabelY, javax.swing.SwingConstants.LEFT);
    context.setPaintMode();
    context.setColor(plotColorMap.getForeground());
  }
  ...
  public void paintMarkArea(final Graphics context) {
    ...
    context2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f));
  ...
  protected void paintSelectingArea(final Graphics context) {
    ...
    synchronized (lastMovePoint) {
      painter.markArea2dOutline(plotSelection.start, Bin.create(lastMovePoint));
    }
  ...
  protected void paintSettingGate(final Graphics context) {
    ...
    context2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.8f));
    synchronized (lastMovePoint) {
      context2d.drawLine(lastGatePoint.x, lastGatePoint.y, lastMovePoint.x, lastMovePoint.y);
    }
    panel.setMouseMoved(false);
    mouseMoveClip.reset();
  }
