...
/**
 * Class of a library of methods to draw things for a graph. First the update method is called to
 * set the limits for the plot and to give it a handle to the graphics object.
 *
 * <p>The screen origin is upper right hand corner while the data point origin is lower left hand
 * corner. There are private methods that map screen pixels to data co-ordinates and other public
 * methods that class to map data points to screen pixels.
 *
 * <p>The border size is the area around the plot the outline the plot is part of the plot, the
 * outline is part not part of the border. Therefore channels with zero counts will therefore draw
 * along the lower border, and the left and right border will be part of a channel.
 *
 * <p>The method getSize() for <code>Component</code> returns the number of pixels which are labeled
 * starting with 0, to getSize()-1
 *
...
final class Painter {
  ...
  private final transient Tickmarks tickmarks;
  ...
  private final transient int plotDimensions;
  ...
  private final transient Point middle; // middle of plot area

  private final transient PlotInternalView view = new PlotInternalView();

  /**
   * Full constructor, all contructors eventually call this one. Other constructors have defaults.
   *
  ...
  protected void setLayout(final GraphicsLayout.Type type) {
    ...
    border =
        new Insets(
            graphLayout.border.top,
            graphLayout.border.left,
            graphLayout.border.bottom,
            graphLayout.border.right);
    final Font screenFont =
        new Font(Painter.FONT_CLASS, Font.BOLD, (int) GraphicsLayout.SCREEN_FONT_SIZE);
  ...
  protected void setView(final PageFormat format) {
    synchronized (monitor) {
      ...
      if (format != null) {
        viewSize =
            new Dimension((int) format.getImageableWidth(), (int) format.getImageableHeight());
      }
    }
  }

  private final transient Object limitsLock = new Object();

  private static final String FONT_CLASS = "Serif";

  private static final float TITLE_SCREEN_SIZE = GraphicsLayout.SCREEN_FONT_SIZE + 2;

  private static final transient int MARK_OFFSET = 3;

  // stuff for channel marker
  private static final transient int MARK_MIN_LENGTH = 20;

  private int getMinimumY() {
    synchronized (limitsLock) {
      return plotDimensions == 1 ? plotLimits.getMinimumCounts() : plotLimits.getMinimumY();
    }
  }

  private int getMaximumY() {
    synchronized (limitsLock) {
      return plotDimensions == 1 ? plotLimits.getMaximumCounts() : plotLimits.getMaximumY();
    }
  }

  /**
   * updates the current display parameters the most basic update this one must always be called
   *
   * @param graph the Graphics object
   * @param newViewSize the viewable size of the canvas in pixels
   * @param limits the limits of the plot
   * @since Version 0.5
   */
  protected void update(final Graphics graph, final Dimension newViewSize, final Limits limits) {
  ...
  /**
   * Draws the border around the plot As plotSize() returns the the size of the plot including the
   * borders we have to subtract one from the width and height. as drawRect(x,y,dx,dy) draws at x, y
   * and at x+dx, y+dy.
   *
   * @since Version 0.5
   */
  protected void drawBorder() {
    graphics2d.drawRect(border.left, border.top, view.getWidth() - 1, view.getHeight() - 1);
  }
  ...
  private void ticksBottom(final int lowerLimit, final int upperLimit) {
    ...
    final int[] ticks = tickmarks.getTicks(lowerLimit, upperLimit, scale, Tickmarks.Type.MINOR);
    for (int tick : ticks) {
      final int xOrigin = toViewHorzLin(tick);
      int bottom = view.getBottom();
      graphics2d.drawLine(xOrigin, bottom, xOrigin, bottom - graphLayout.tick.minor);
      bottom = border.top;
      graphics2d.drawLine(xOrigin, bottom, xOrigin, bottom + graphLayout.tick.minor);
    }
    final int[] ticksMajor =
        tickmarks.getTicks(lowerLimit, upperLimit, scale, Tickmarks.Type.MAJOR);
    for (int aTicksMajor : ticksMajor) {
      final int xOrigin = toViewHorzLin(aTicksMajor);
      int bottom = view.getBottom();
      graphics2d.drawLine(xOrigin, bottom, xOrigin, bottom - graphLayout.tick.major);
      bottom = border.top;
      graphics2d.drawLine(xOrigin, bottom, xOrigin, bottom + graphLayout.tick.major);
    }
  }
   ...
   * @since Version 0.5
   */
  private void ticksLeft(final int lowerLimit, final int upperLimit, final Scale scale) {
    int xCoordinate;
    int yCoordinate;
    ...
    for (int tick : ticks) {
      ...
      graphics2d.drawLine(
          xCoordinate, yCoordinate, xCoordinate + graphLayout.tick.minor, yCoordinate);
    ...
    for (int aTicksMajor : ticksMajor) {
      ...
      xCoordinate = border.left;
      graphics2d.drawLine(
          xCoordinate, yCoordinate, xCoordinate + graphLayout.tick.major, yCoordinate);
      xCoordinate = view.getRight();
      graphics2d.drawLine(
          xCoordinate, yCoordinate, xCoordinate - graphLayout.tick.major, yCoordinate);
    }
  ...
  private void labelsBottom(final int lowerLimit, final int upperLimit) {
    ...
    for (int aTicksMajor : ticksMajor) {
      ...
      final int yCoordinate =
          view.getBottom() + metrics.getAscent() + graphLayout.labelOffsets.bottom;
   ...
   * @since Version 0.5
   */
  private void labelsLeft(final int lowerLimit, final int upperLimit, final Scale scale) {
    final int[] ticksMajor =
        tickmarks.getTicks(lowerLimit, upperLimit, scale, Tickmarks.Type.MAJOR);
    for (int aTicksMajor : ticksMajor) {
      final String label = Integer.toString(aTicksMajor);
      ...
      final int xCoordinate = border.left - offset - graphLayout.labelOffsets.left;
      graphics2d.drawString(label, xCoordinate, yCoordinate);
    }
  }
  ...
  private void axisLabelBottom(final String label) {
    ...
    final int yCoordinate =
        view.getBottom() + metrics.getAscent() + graphLayout.axisLabelOffsets.bottom;
    graphics2d.drawString(label, xCoordinate, yCoordinate);
  }
   ...
   * @since Version 0.5
   */
  private void drawHist(final double[] counts, final double binWidth, final boolean log) {
    ...
    if (counts == null) {
      ...
      synchronized (limitsLock) {
        ...
        firstBin = (int) Math.floor(plotLimits.getMinimumX() / binWidth);
      ...
      for (int i = firstBin; i <= lastBin; i++) {
        ...
        yCoordinate = log ? toViewVertLog(drawCounts[i]) : toViewVertLinCk(drawCounts[i]);
  ...
  private void drawLineLinear(final double[] channel, final double[] counts) {
    ...
    synchronized (limitsLock) {
      ...
      upperX = Math.min(plotLimits.getMaximumX(), (int) channel[channel.length - 1]);
    }
    ...
    for (int k = 0; k < numberColors; k++) {
      ...
      graphics2d.fillRect(
          ...
          view.getBottom()
              ...
              - k * graphLayout.colorScale.size, // vertical
          graphLayout.colorScale.size,
          graphLayout.colorScale.size); // size
    }
  }

  private void drawScaleKey(
      final int minCount,
      final int[] colorThresholds,
    ...
    for (int j = minChanY; j <= maxChanY; j++) {
      for (int i = minChanX; i <= maxChanX; i++) {
        ...
        if (count > minCount) {
          ...
          /* inline for speed */
          graphics2d.fillRect(xValue, yValue - channelHeight + 1, channelWidth, channelHeight);
        }
      }
    }
  }
   ...
   * @since Version 0.5
   */
  protected void drawGate1d(final int lowerLimit, final int upperLimit, final boolean noFillMode) {
    clipPlot();
  ...
  protected void settingGate1d(final Polygon gatePoints) {
    ...
    if (gatePoints.npoints > 0) {
      ...
      if (gatePoints.npoints > 1) {
        markAreaOutline1d(
            toDataHorz(gatePoints.xpoints[gatePoints.npoints - 2]), toDataHorz(xValue));
      } else {
        graphics2d.drawLine(xValue, view.getBottom(), xValue, border.top);
      }
    }
  }
   ...
   *
   * @param xValue1 a point in plot coordinates
   * @param xValue2 a point in plot coordinates
   * @return a rectangle in graphics coordinates that will highlight the channels indicated
   */
  protected Rectangle getRectangleOutline1d(final int xValue1, final int xValue2) {
  ...
  /**
   * Given a rectangle in plot coordinates, return the bounding rectangle in graphics coordinates.
   *
  ...
  protected Rectangle getRectangleOutline2d(final Rectangle channels) {
    ...
    return getRectangleOutline2d(Bin.create(channels.getLocation()), Bin.create(highX, highY));
  }

  /**
   * Returns the rectangle that includes the box of plot coordinates indicated by the given plot
   * coordinates.
   *
   * @param bin1 in plot coordinates
   * @param bin2 in plot coordinates
   * @return in graphics coordinates
   */
  ...
  protected void markChannel1d(final int channel, final double count) {
    ...
    if (xValue1 >= border.left && xValue1 <= view.getRight()) {
      ...
      graphics2d.drawString(label, xValue2, yValue2 - Painter.MARK_OFFSET);
    }
  }
  ...
  protected void drawPeakLabels(final double[][] peaks) {
    ...
    for (int i = 0; i < peaks[0].length; i++) {
      ...
      if (xValue1 >= border.left && xValue1 <= view.getRight()) {
        ...
        final String label = Integer.toString((int) Math.round(peaks[1][i]));
  ...
  protected void markChannel2d(final Bin bin) {
    ...
    graphics2d.drawString(
        label, (int) rectangle.getMaxX() + Painter.MARK_OFFSET, rectangle.y - Painter.MARK_OFFSET);
  }
   ...
   */
  protected void markArea1d(final int lowChan, final int highChan, final double[] counts) {
    int minChan, maxChan;
    boolean log;
    ...
    for (int i = minChan; i <= maxChan; i++) {
      yValue = log ? toViewVertLog(counts[i]) : toViewVertLinCk(counts[i]);
  ...
  private int addToFillX(
      final Polygon fill, final int lastx, final int lasty, final int xValue, final int yValue) {
    int result = lastx;
  ...
  private int addToFillY(final Polygon fill, final int lasty, final int xValue, final int yValue) {
    int result = lasty;
    if (yValue != lasty) {
      fill.addPoint(xValue, yValue);
      result = yValue;
   ...
   *
   * @param rectangle rectangle in graphics coordinates
   */
   ...
   * @since Version 0.5
   */
  protected void drawDataLine(
      final int xValue1, final int yValue1, final int xValue2, final int yValue2) {
    graphics2d.drawLine(
        toViewHorzLin(xValue1),
        toViewVertLin(yValue1),
        toViewHorzLin(xValue2),
        toViewVertLin(yValue2));
  }
    ...
    synchronized (monitor) {
      ...
      synchronized (limitsLock) {
        if (xView < border.left) {
          ...
          data = (int) (plotLimits.getMinimumX() + (xView - border.left) / conversion.getX());
        }
      }
      return data;
    }
  ...
  protected int toDataVert(final int yCoordinate) {
    synchronized (monitor) {
      ...
      if (yCoordinate < border.top) {
        ...
        data = (int) (getMinimumY() + (view.getBottom() - yCoordinate) / conversion.getY());
      }
      return data;
    }
  }
  ...
  protected Point toViewLin(final Bin dataPoint) {
    final Point viewPoint =
        new Point(toViewHorzLin(dataPoint.getX()), toViewVertLin(dataPoint.getY()));
    return viewPoint;
  }
  ...
  protected void clipPlot() {
    graphics2d.clipRect(border.left, border.top, view.getWidth() + 1, view.getHeight() + 1);
  }
  ...
  private int toViewHorzLin(final double data) {
    synchronized (limitsLock) {
      return (int) (border.left + (conversion.getX() * (data - plotLimits.getMinimumX())));
    }
  }
  ...
  private int toViewVertLog(final double data) {
    ...
    if (dataLog > takeLog(getMaximumY())) {
      ...
      rval = view.getBottom() - (int) (conversion.getYLog() * (dataLog - minYLog));
    }
    return rval;
  }
