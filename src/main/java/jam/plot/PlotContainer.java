...
public final class PlotContainer implements PlotContainerSelectListener {
   ...
   */
  private final transient Object plotLock = new Object();

  private final transient CardLayout cardLayout;

  private final transient Plot1d plot1d;

  private final transient Plot2d plot2d;

  private final transient Scroller scroller1d, scroller2d;

  private transient boolean hasData;

  private final transient PlotSelectListener selectListener;
  ...
  private final transient JPanel panel = new JPanel();

  /**
   * @param selectListener place to send selection messages
   * @param plot1d the 1d plot
   * @param plot2d the 2d plot
   */
  @Inject
  protected PlotContainer(
      final PlotSelectListener selectListener, final Plot1d plot1d, final Plot2d plot2d) {
  ...
  protected void select(final boolean selectedState) {
    final Border border =
        ((layoutType == LayoutType.LABELS_BORDER) || (layoutType == LayoutType.NO_LABELS_BORDER))
            ? (selectedState
  ...
  protected void setLayoutType(final LayoutType type) {
    ...
    } else if ((type == LayoutType.NO_LABELS_BORDER) || (type == LayoutType.NO_LABELS)) {
      plot1d.setLayout(GraphicsLayout.Type.WO_LABELS);
      plot2d.setLayout(GraphicsLayout.Type.WO_LABELS);
    }
    // Plot border
    final int borderWidth =
        (type == LayoutType.LABELS_BORDER || type == LayoutType.NO_LABELS_BORDER) ? 2 : 0;
    final Border border =
   ...
   * Display a histogram,
   *
   * @param hist histogram to display
   */
   ...
   * Overlay a histogram.
   *
   * @param num the number of the histogram to overlay
   */
  ...
  /**
   * @param signals the fit curve, minus background
   * @param background the fit background
   * @param residuals the fit residuals
   ...
   *
   * @param channel bin to grab counts from
   ...
   * @param size x-range to stay within
   * @param useMax whether to use 0 or xMax if out of range
   * @return input value if in range, 0 or xMax if not
   */
  private int constrainValue(final int value, final int size, final boolean useMax) {
  ...
  protected void setRenderForPrinting(final boolean printing, final PageFormat format) {
