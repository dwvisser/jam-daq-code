...
/**
 * Class the does the actions on plots. Receives commands from buttons and command line. Performs
 * action by performing command on plot, plot1d and plot2d. Commands available:
 *
 * <dl>
 *   <dt>update
 *   <dd>update a plot with current data
 *   <dt>expand
 *   <dd>expand channels to view
 *   <dt>zoomin, zoomout
 *   <dt>full
 *   <dd>view all channels
 *   <dt>linear, log
 *   <dt>range
 *   <dd>change the count range
 *   <dt>auto
 *   <dd>auto scale the count range
 *   <dt>area
 *   <dd>get an area
 *   <dt>cancel
 * </dl>
...
public final class Action {
  ...
  private static final Map<String, Method> NO_ARG_MAP = new HashMap<>();
  ...
  private static boolean existsAndIsCalibrated(final AbstractHistogram hist) {
    return (hist != null && hist instanceof AbstractHist1D)
        && ((AbstractHist1D) hist).isCalibrated();
  }
   ...
   *
   * @param console Jam's console component
   */
  @Inject
  Action(
  ...
  /**
   * Calculate the area and centroid for a region. Maybe we should copy inquire methods to this
   * class?
   */
  ...
  private void area() { // NOPMD
    if (commandPresent) {
      ...
      init();
      final String name =
          ((AbstractHistogram) SelectionTree.getCurrentHistogram()).getFullName().trim();
  ...
  /**
   * @param hist the first element of which is the number of the hist to display
   */
  ...
  public final transient Commandable commandable =
      new Commandable() {
        ...
        public void doCommand(
            final String inCommand, final List<Double> inParams, final boolean typedAtConsole) {
          synchronized (this) {
  ...
  private void getCalibratedPeakStatistics(
      final PlotContainer currentPlot,
      final AbstractHistogram hist,
      final double[] fwhm,
      final double[] centroidError,
  ...
  private void getNetArea(
    ...
    double distance;
    final int[] bgdX1 = {
      clicks.get(0).getX(), clicks.get(1).getX(), clicks.get(2).getX(), clicks.get(3).getX()
    };
    Arrays.sort(bgdX1);
  ...
  private void go() { // NOPMD
    ...
    if (commandPresent) {
      ...
      if (currentPlot.getDimensionality() == 1 && existsAndIsCalibrated(hist)) {
        final String mess = intro + cal + space + ENERGY + leftParen + space;
        textOut.messageOut(mess, MessageHandler.NEW);
      } else {
        final String mess = intro + CHANNEL + leftParen + space;
        textOut.messageOut(mess, MessageHandler.NEW);
      }
    }
  }
      ...
      if (existsAndIsCalibrated(hist)) {
        if (currentPlot.getDimensionality() == 1) {
          output
              .append(ENERGY)
              .append(equal)
              .append(currentPlot.getEnergy(xCoord))
              .append(sep)
              .append(CHANNEL)
              .append(equal)
              .append(xCoord);
  ...
  private void handleClick6(final PlotContainer currentPlot, final AbstractHistogram hist) {
    ...
    final int[] bgdPts = {
      getClick(0).getX(), getClick(1).getX(), getClick(2).getX(), getClick(3).getX()
    };
    Arrays.sort(bgdPts);
  ...
  private void help() { // NOPMD
    ...
    buffer.append(
        "Commands:\tli - Linear Scale\tlo - Log Scale\ta  - Auto Scale\tra - Range\tex - Expand\tf "
            + " - Full view\t zi - Zoom In\tzo - Zoom Out\td  - Display\to  - Overlay\tu  - Update"
            + "\tg  - GoTo\tar - Area\tn  - Net Area\tre - Rebin\tc  - Bin\t");
  ...
  /** Set the current plot, i.e., the one we will do actions on. Reset the current state. */
  ...
  private final transient PlotMouseListener mouseListener =
      new PlotMouseListener() {
        /**
  ...
  private void rebin(final List<Double> parameters) {
    ...
    if (!parameters.isEmpty()) {
      ...
      if (binWidth >= 1.0 && binWidth < hist.getSizeX()) {
        ...
        textOut.messageOut(String.valueOf(binWidth), MessageHandler.END);
        LOGGER.warning(
            "Rebin command ignored. Bin value must be \u2265 1.0 and smaller than the histogram.");
        done();
      }
    }
  }
  ...
  /**
   * Call <code>update()</code> on the current plot, reset the command-line, and broadcast a
   * histogram selection message to force the rest of the GUI to be consistent with the currently
   * selected histogram.
   *
