...
public final class ColorSettingsFrame extends JDialog implements ChangeListener {
  ...
  private static final ColorSettingsFrame CSF = new ColorSettingsFrame();
   ...
   */
  public static ColorSettingsFrame getInstance() {
    return CSF;
  }

  private final transient RainbowPanel rainbowPanel = new RainbowPanel();

  private final transient JTextField x0RField, x0GField, x0BField, aRField, aGField, aBField;
  private final transient JSlider x0RSlider, x0GSlider, x0BSlider, aRSlider, aGSlider, aBSlider;

  private static final String RAINBOW = "Rainbow";

  private ColorSettingsFrame() {
    super(GuiceInjector.getObjectInstance(JFrame.class), "Color Scale Settings");
    final Container contents = getContentPane();
    contents.setLayout(new GridBagLayout());
    final GridBagConstraints gbc = new GridBagConstraints();
    final PanelOKApplyCancelButtons buttons = createStandardButtonsPanel();
    final int x0R =
        (int) Math.round(100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.X0R.toString(), .80));
    final int x0G =
        (int) Math.round(100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.X0G.toString(), .60));
    final int x0B =
        (int) Math.round(100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.X0B.toString(), .20));
    final int aRed =
        (int) Math.round(100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.ARED.toString(), .50));
    final int aGreen =
        (int) Math.round(100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.AGREEN.toString(), .40));
    final int aBlue =
        (int) Math.round(100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.ABLUE.toString(), .30));
    ...
    final JComponent csLabel = new JLabel("<html>Choose a<p>color scheme :</html>");
    colorPanel.add(csLabel, gbc);
    setGrid(gbc, 0, 1, 1);
    final JComboBox<String> csChooser = defineColorSchemeChooser();
        ...
        itemEvent -> {
          ...
          if ("Greyscale".equals(selection)) {
            ...
            final int x0red =
                (int)
                    Math.round(
                        100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.X0R.toString(), .80));
            final int x0green =
                (int)
                    Math.round(
                        100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.X0G.toString(), .60));
            final int x0blue =
                (int)
                    Math.round(
                        100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.X0B.toString(), .20));
            final int aRedD =
                (int)
                    Math.round(
                        100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.ARED.toString(), .50));
            final int aGreenD =
                (int)
                    Math.round(
                        100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.AGREEN.toString(), .40));
            final int aBlueD =
                (int)
                    Math.round(
                        100 * COLOR_PREFS.getDouble(GradientSpecFieldsRGB.ABLUE.toString(), .30));
  ...
  private PanelOKApplyCancelButtons createStandardButtonsPanel() {
    final PanelOKApplyCancelButtons buttons =
        new PanelOKApplyCancelButtons(
            new PanelOKApplyCancelButtons.AbstractListener(this) {
              public void apply() {
                ...
                COLOR_PREFS.putDouble(GradientSpecFieldsRGB.ABLUE.toString(), aBlueD);
                COLOR_PREFS.putDouble(GradientSpecFieldsRGB.AGREEN.toString(), aGreenD);
                COLOR_PREFS.putDouble(GradientSpecFieldsRGB.ARED.toString(), aRedD);
                COLOR_PREFS.putDouble(GradientSpecFieldsRGB.X0B.toString(), x0blue);
                COLOR_PREFS.putDouble(GradientSpecFieldsRGB.X0G.toString(), x0green);
                COLOR_PREFS.putDouble(GradientSpecFieldsRGB.X0R.toString(), x0red);
              }
            });
    return buttons;
  }
  ...
  private void setGrid(
      final GridBagConstraints gbc, final int gridx, final int gridy, final int width) {
    gbc.gridx = gridx;
    gbc.gridy = gridy;
    gbc.gridwidth = width;
  }
