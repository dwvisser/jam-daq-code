...
/**
 * A color scale where ranges in counts are represented by individual, discrete colors.
 *
...
public final class DiscreteColorScale implements ColorScale {
  private static final Color[] B_ON_W = {
    new Color(0, 0, 127), // 0
    new Color(0, 0, 255), // 1
  ...
  private static final Color[] W_ON_B = {
    new Color(0, 0, 127), // 0
    new Color(0, 0, 255), // 1
    new Color(128, 0, 255), // 2
    new Color(255, 0, 255), // 3
  ...
  private final transient Scale scale;
  ...
  private static final DiscreteColorScale LOG = new DiscreteColorScale(Scale.LOG);

  private static final DiscreteColorScale LINEAR = new DiscreteColorScale(Scale.LINEAR);

  /**
   * Given the type of counts scale, return an appropriate discrete color scale.
   *
   * @param scale the counts scale
  ...
  private static Color[] colors = B_ON_W;

  /**
   * Sets the color thresholds.
   *
   * @param lowerLimit bottom of scale
   * @param upperLimit top of scale
   */
  public void setRange(final int lowerLimit, final int upperLimit) {
    synchronized (this) {
      thresholds =
          scale == Scale.LINEAR
              ? colorThresholdsLin(lowerLimit, upperLimit)
              : colorThresholdsLog(lowerLimit, upperLimit);
    }
  }

  /**
   * Returns the counts thresholds for the various colors. Used to paing the key.
   *
  ...
  private int thresholdStep(final int lowerLimit, final int upperLimit) {
    ...
    final ScaleCalculator calculator = new ScaleCalculator(range, colors.length);
