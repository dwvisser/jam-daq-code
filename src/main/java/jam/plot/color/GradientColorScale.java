...
public final class GradientColorScale implements ColorScale {
  ...
  private static final GradientColorScale LINEAR = new GradientColorScale(0, 100, Scale.LINEAR);

  private static final GradientColorScale LOG = new GradientColorScale(0, 100, Scale.LOG);

  protected static Color getRGB(
      final double level,
      final double x0R,
      final double sigR,
      final double x0G,
  ...
  private static float gaussExp(final double level, final double center, final double sig) {
   ...
   */
  public static GradientColorScale getScale(final Scale scale) {
    synchronized (GradientColorScale.class) {
      return scale == Scale.LINEAR ? LINEAR : LOG;
    }
  }

  private transient double blueCenter =
      COLOR_PREFS.getDouble(GradientSpecFieldsRGB.X0B.toString(), 0.2);

  private transient double blueSpread =
      COLOR_PREFS.getDouble(GradientSpecFieldsRGB.ABLUE.toString(), 0.09);

  private transient double greenCenter =
      COLOR_PREFS.getDouble(GradientSpecFieldsRGB.X0G.toString(), 0.6);

  private transient double greenSpread =
      COLOR_PREFS.getDouble(GradientSpecFieldsRGB.AGREEN.toString(), 0.16);
  ...
  private transient double redCenter =
      COLOR_PREFS.getDouble(GradientSpecFieldsRGB.X0R.toString(), 0.8);

  private transient double redSpread =
      COLOR_PREFS.getDouble(GradientSpecFieldsRGB.ARED.toString(), 0.25);

  { // NOPMD
    COLOR_PREFS.addPreferenceChangeListener(
        pce -> {
          final String key = pce.getKey();
          if (!key.equals(ColorPrefs.SMOOTH_SCALE)) {
            final double newValue = Double.parseDouble(pce.getNewValue());
            if (GradientSpecFieldsRGB.ABLUE.toString().equals(key)) {
              blueSpread = newValue;
            } else if (GradientSpecFieldsRGB.AGREEN.toString().equals(key)) {
              greenSpread = newValue;
            } else if (GradientSpecFieldsRGB.ARED.toString().equals(key)) {
              redSpread = newValue;
   ...
   */
  private GradientColorScale(final double min, final double max, final Scale scale) {
    super();
    if (min > max) {
      setMaxCounts(min);
      setMinCounts(max);
