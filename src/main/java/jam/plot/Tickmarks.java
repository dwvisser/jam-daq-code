...
/**
 * Class to calculate tickmarks and scale. Need to clean up especially log part error in log for 2d.
 *
 * @version 0.5
 * @author Ken Swartz
 */
final class Tickmarks {
   ...
   * either one or 2 d
   */
  int[] getTicks(final int lowerLimit, final int upperLimit, final Scale scale, final Type type) {
    int[] ticks = new int[0];
  ...
  private int tickSpace(final int lowerLimit, final int upperLimit) {
    ...
    final ScaleCalculator calculator = new ScaleCalculator(range, MIN_NUMBER_TICKS);
  ...
  private int[] ticksLog(final int lowerLimit, final int upperLimit) {
    ...
    for (int i = startTick; i <= upperLimit; i += decadeIncrement) {
      numberTicks++;
      decadeIncrement = nextDecadeIfValueIsMultipleOfCurrent(decadeIncrement, i);
    }
    // load tick values
  ...
  private int nextDecadeIfValueIsMultipleOfCurrent(final int decade, final int value) {
    int result = decade;
