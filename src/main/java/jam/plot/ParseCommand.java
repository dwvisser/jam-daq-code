...
final class ParseCommand implements CommandListener {
  ...
  private final transient CurrentPlotAccessor plotAccessor;
  ...
  private final transient Commandable commandable;

  ParseCommand(final Commandable commandable, final CurrentPlotAccessor plotAccessor) {
    super();
   ...
   *
   * @param string string rep of the number
   * @return the number
  ...
  private double convertNumber(final String string) throws NumberFormatException {
    return (string.indexOf('.') >= 0) ? Double.parseDouble(string) : Integer.parseInt(string);
  }
   ...
   */
  private void cursorChannel(final List<Double> parameters, final boolean vertical) {
      ...
      if (plotAccessor.getPlotContainer().getDimensionality() == 1) {
        ...
        if (vertical) { // use 0,counts
          ...
          for (int i = 0; i < numParam - 1; i = i + 2) {
            cursor.setChannel(parameters.get(i).intValue(), parameters.get(i + 1).intValue());
            commandable.setCursor(cursor);
            commandable.doCommand(PlotCommands.CURSOR, true);
          }
        }
   ...
   */
  public boolean performParseCommand(final String _command, final String[] cmdParams) {
    ...
    if (command.equals(Console.NUMBERS_ONLY)) {
      ...
      if (commandable.isCursorCommand()) {
        final boolean vertical = PlotCommands.RANGE.equals(commandable.getCurrentCommand());
