...
/**
 * This class is a display routine for plots. It is implemented by <code>Display</code>.
 *
 * <p>
 *
 ...
 */
@Singleton
public final class PlotDisplay extends JPanel
    implements PreferenceChangeListener,
        PropertyChangeListener,
        CurrentPlotAccessor,
        PlotSelectListener {

  private final transient Action action; // handles display events
  ...
  private final transient List<PlotContainer> plotContainers = new ArrayList<>();
   ...
   * @param action handles plot actions
   * @param broadcaster broadcasts state changes
   * @param toolbar the plot toolbar
   */
  @Inject
  public PlotDisplay(
    ...
    final String location = PREFS.get(Toolbar.LOCATION_KEY, defaultLocation);
    final int orientation =
        (BorderLayout.NORTH.equals(location) || BorderLayout.SOUTH.equals(location))
  ...
  /**
   * Adds a plot mouse listener, plot mouse is a mouse which is calibrated to the current display.
   *
  ...
  private void createPlots(final int numberPlots) {
    for (int i = plotContainers.size(); i < numberPlots; i++) {
      plotContainers.add(GuiceInjector.getObjectInstance(PlotContainer.class));
    }
  }
   ...
   * @param start channel where curves start
   */
  ...
  public void setPeakFindProperties(
      final double width, final double sensitivity, final boolean cal) {
  ...
  public void setView(final View viewIn) {
    ...
    final GridLayout gridLayout = new GridLayout(currentView.getRows(), currentView.getColumns());
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    ...
    if (command == Command.REFRESH) {
      ...
      getPlotContainer().displaySetGate(GateSetMode.GATE_CONTINUE, (Bin) event.getContent(), null);
  ...
  private void processCommand(final Command command) {
    if (command == Command.HISTOGRAM_NEW) {
      ...
      if (named instanceof AbstractHistogram) {
        ...
        final List<AbstractHist1D> overHists =
