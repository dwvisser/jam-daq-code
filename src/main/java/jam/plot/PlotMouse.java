...
/**
 * Converts screen pixel to data value for a mouse Click. You add listeners and remove listeners,
 * classes that implement PlotMouseListener. (method plotMousePressed); The PlotMouse class belongs
 * to a plot class. Only mousePressed event are implemented
 *
 * @author Ken Swartz.
 */
final class PlotMouse extends MouseAdapter {
  /* list of listeners for PlotMouse */
  private final transient List<PlotMouseListener> listeners = new ArrayList<>();
   ...
   */
  protected void setPlotContainerSelectListener(final PlotContainerSelectListener plotSelect) {
  ...
  /**
   * A mousePressed event has occurred, so call all listeners in the listeners list.
   *
  ...
  public void mousePressed(final MouseEvent event) {
    ...
    final CountsContainer countsContainer = (CountsContainer) event.getSource();
