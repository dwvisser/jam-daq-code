...
final class RemoteSnapshot {
  private static final Logger LOGGER =
      Logger.getLogger(RemoteSnapshot.class.getPackage().getName());
  ...
  public void takeSnapshot(final String url) throws JamException {
    ...
    try {
      ...
      throw new JamException("Remote look up could not find name " + url, nbe);
    }
    try {
      AbstractHistogram.setHistogramList(remoteData.getHistogramList());
      Gate.setGateList(remoteData.getGateList());
    } catch (RemoteException re) {
      throw new JamException("Remote getting histogram list [SetupRemote]", re);
    }
  }
}
