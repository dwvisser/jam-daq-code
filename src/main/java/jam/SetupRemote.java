...
/**
 * Class to make this process into a remote server for Jam or hook up to a remote online acquisition
 * that is a server.
 *
 * @author Ken Swartz
 */
public class SetupRemote extends JDialog implements ActionListener, ItemListener {
  ...
  private final transient RemoteSnapshot remote;

  private final transient JButton bapply, bok;
  private final transient JCheckBox checkLock, clink, cserver, csnap;

  private final transient boolean inApplet; // are we running in a applet
   ...
   * @param remote remote snapshot
   */
  @Inject
  public SetupRemote(final JFrame frame, final JamStatus status, final RemoteSnapshot remote) {
  ...
  /**
   * Locks up the Remote setup so the fields cannot be edited. and puts us in remote mode
   *
   ...
   *
   * @param name name to give the server process
  ...
  public void server(final String name) throws JamException {
    try {
      ...
      throw new JamException(
          "Creating remote server unknown host, name: " + name + " [SetupRemote]", unhe);
    } catch (RemoteException re) {
      throw new JamException("Creating remote server " + re.getMessage() + " [SetupRemote]", re);
    } catch (java.net.MalformedURLException mue) {
      throw new JamException("Creating remote Server, malformed URL.", mue);
    }
  }
