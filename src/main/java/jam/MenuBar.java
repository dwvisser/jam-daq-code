...
final class MenuBar implements PropertyChangeListener {
  ...
  private final transient JMenu fitting = new JMenu("Fitting");

  private final transient JMenuBar menus = new JMenuBar();
  ...
  protected MenuBar(
    ...
    menus.add(
        this.commandManager.createMenu(
            "Setup",
            CommandNames.SHOW_SETUP_ONLINE,
            CommandNames.SHOW_SETUP_OFF,
            CommandNames.SHOW_SETUP_REMOTE,
    ...
    menus.add(
        this.commandManager.createMenu(
            "Help", CommandNames.HELP_ABOUT, CommandNames.USER_GUIDE, CommandNames.HELP_LICENSE));
  }

  private JMenu createFileMenu(final ImportMenu importMenu) {
    final JMenu file =
        this.commandManager.createMenu("File", CommandNames.CLEAR, CommandNames.OPEN_HDF);
    file.add(
        this.commandManager.createMenu(
            "Open Special",
            CommandNames.OPEN_MULTIPLE_HDF,
            CommandNames.OPEN_ADD_HDF,
    ...
    final JMenuItem saveSpecial =
        this.commandManager.createMenu(
            "Save Special",
            CommandNames.SAVE_SORT,
            CommandNames.SAVE_GROUP,
            CommandNames.SAVE_HISTOGRAMS,
            CommandNames.SAVE_GATES);

    file.add(saveSpecial);
    file.addSeparator();

    final JMenuItem utilities =
        this.commandManager.createMenu(
            "Scaler Utilities", CommandNames.OPEN_SCALERS, CommandNames.SHOW_SCALER_SCAN);
    file.add(utilities);
    file.addSeparator();
    ...
    final JMenu expHist =
        this.commandManager.createMenu(
            "Export",
            CommandNames.EXPORT_TABLE,
            CommandNames.EXPORT_TEXT,
            CommandNames.EXPORT_SPE,
            CommandNames.EXPORT_DAMM,
  ...
  private JMenu createHistogramMenu() {
    ...
    group.add(this.commandManager.getMenuItem(CommandNames.SHOW_RENAME_GROUP));
    group.add(this.commandManager.getMenuItem(CommandNames.DELETE_GROUP));

    histogram.add(this.commandManager.getMenuItem(CommandNames.SHOW_NEW_HIST));
    histogram.add(this.commandManager.getMenuItem(CommandNames.SHOW_HIST_ZERO));
    histogram.add(this.commandManager.getMenuItem(CommandNames.DELETE_HISTOGRAM));
    histogram.add(this.commandManager.getMenuItem(CommandNames.SHOW_HIST_FIT));
    histogram.add(this.commandManager.getMenuItem(CommandNames.SHOW_HIST_PROJECT));
    histogram.add(this.commandManager.getMenuItem(CommandNames.SHOW_HIST_COMBINE));
    histogram.add(this.commandManager.getMenuItem(CommandNames.SHOW_GAIN_SHIFT));
  ...
  private JMenu createScalerMenu() {
    ...
    menus.add(scalers);
    scalers.add(this.commandManager.getMenuItem(CommandNames.DISPLAY_SCALERS));
    scalers.add(this.commandManager.getMenuItem(CommandNames.SHOW_ZERO_SCALERS));
    scalers.addSeparator();
    scalers.add(this.commandManager.getMenuItem(CommandNames.DISPLAY_MONITORS));
    scalers.add(this.commandManager.getMenuItem(CommandNames.DISPLAY_MON_CFG));
    return scalers;
  }
  ...
  private JMenu createPreferencesMenu() {
    ...
    mPrefer.add(this.commandManager.getMenuItem(CommandNames.SHOW_GRADIENT));
    mPrefer.add(this.commandManager.getMenuItem(PlotPreferences.ENABLE_SCROLLING));
    mPrefer.add(this.commandManager.getMenuItem(PlotPreferences.DISPLAY_LABELS));
    mPrefer.add(this.commandManager.getMenuItem(PlotPreferences.BLACK_BACKGROUND));
    mPrefer.addSeparator();
