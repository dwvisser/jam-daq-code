...
public final class StringUtilities {
  ...
  private static final Charset ASCII = Charset.availableCharsets().get("US-ASCII");

  private static final char ELEMENT_SEPARATOR = '/';

  private static final Logger LOGGER =
      Logger.getLogger(StringUtilities.class.getPackage().getName());
  ...
  /**
   * Truncates a <code>String</code> or pads the end with spaces to make it a certain length.
   *
  ...
  /**
   * Make a unique name out of the given name that differs from names in the given set.
   *
   ...
   * @return unique name
   */
  public String makeUniqueName(final String name, final Set<String> nameSet, final int nameLength) {
    ...
    while (!isUnique) {
      ...
      if (copyFound) {
        ...
        final String temp = makeLength(rval.toString(), nameLength - nameAddition.length());
        rval.setLength(0);
        rval.append(temp);
    ...
    if (warn) {
      LOGGER.warning("\"" + name + "\" truncated to produce new name \"" + rval + "\".");
    }
