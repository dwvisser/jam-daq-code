...
public class YaleCAENgetScalers {
  private static final Logger LOGGER =
      Logger.getLogger(YaleCAENgetScalers.class.getPackage().getName());
  ...
  /**
   * Constructs an object that can scan YaleCAEN event files for scaler blocks.
   *
   * @param frame application frame
   * @param executor provides threads for tasks
   */
  @Inject
  ...
  /**
   * Takes an event file, searches for scaler blocks in it and creates tab-delimited text listing
   * each scaler block on one row of text.
   *
  ...
  private boolean doIt(final File events, final StringBuilder strError) {
    ...
    pBstatus =
        new ProgressMonitor(
            frame,
            "Scanning " + events.getName() + " for scaler blocks",
            "Initializing",
            0,
            lengthMB);
  ...
  private int appendScalerValues(
      final StringBuffer strBuff,
      final DataInputStream dis,
      final int counter,
      final int numScalers)
      throws IOException {
    int rval = counter;
