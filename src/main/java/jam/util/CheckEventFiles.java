 ...
 * for the presence of the buggy behavior whereby the last buffer from the previous run gets put as
 * the first buffer as the next run. Give it a directory and it will print which event files have a
...
public final class CheckEventFiles {
  ...
  private static final String PACKAGENAME = CheckEventFiles.class.getPackage().getName();
  ...
  private void checkFirstBuffer(final File eventFile) {
    ...
    try {
      final DataInputStream instream =
          new DataInputStream(new BufferedInputStream(new FileInputStream(eventFile)));
      /* skip header and all but last word of first data buffer */
      final boolean skipSuccess = (bytesToSkip == instream.skip(bytesToSkip));
      if (skipSuccess) {
  ...
  private void makeScalerSummary(final File infile, final File outPath) {
    final File csvFile =
        new File(
            outPath,
            infile.getName().substring(0, infile.getName().lastIndexOf(".evn")) + "_scalers.csv");
    ...
    try {
      ...
      if (skipped != headerSize) {
        throw new IOException("Expected to skip " + headerSize + " bytes, only skipped " + skipped);
      }
  ...
  private void processAllScalerBlocks(final FileWriter csvStream, final DataInputStream fromStream)
      throws IOException {
    ...
    while (true) {
      ...
      if (readVal == SCALER_HEADER) {
        ...
        processScalerBlock(csvStream, fromStream, blockNum, lastVal, val, numScalers);
        csvStream.write("\n");
      }
    }
  }

  private void processScalerBlock(
      final FileWriter csvStream,
      final DataInputStream fromStream,
      final int blockNum,
      final List<Integer> lastVal,
    ...
    for (int i = 0; i < numScalers; i++) {
      ...
      if (value < lastVal.get(i)) {
        LOGGER.info("Scaler " + i + " out of sequence, block " + blockNum);
      }
      lastVal.set(i, value);
    }
  }
  ...
  private void initializeLists(
      final List<Integer> lastVal, final List<Integer> val, final int numScalers) {
    val.clear();
    lastVal.clear();
    for (int i = 0; i < numScalers; i++) {
      lastVal.add(-1);
