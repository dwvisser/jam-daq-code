...
public final class FileUtilities {
  ...
  /**
   * Takes a file and extension and appends the extension to the file name. If the file has no
   * extension, the extension is appended. If it does have an extension, it is replaced. By
   * extension, we mean the usual <code>DOS/Unix</code> filenaming convention:
   *
   * @param fileIn the file to add extension to.
   * @param extension file extension; everything up to any decimal is thrown away
   * @param mode <code>FORCE</code>,<code>APPEND_ONLY</code> or <code>NO_CHANGE</code>
   * @return a filename with the desired extension
   * @throws IllegalArgumentException if an unrecognized mode is given
   */
  public File changeExtension(final File fileIn, final String extension, final int mode) {
    final String path = fileIn.getParent();
    final String fileName = changeExtension(fileIn.getName(), extension, mode);
    final String fileFullName = path + File.separator + fileName;
    return new File(fileFullName);
  }

  /**
   * Takes a file name and extension and appends the extension to the file name. If the filename has
   * no extension, the extension is appended. If it does have an extension, it is replaced. By
   * extension, we mean the usual <code>DOS/Unix</code> filenaming convention:
   *
   * <p><code><i>filename</i>.ext</code><br>
   ...
   * @see #NO_CHANGE
   */
  public String changeExtension(final String fileName, final String extension, final int mode) {
  ...
  /**
   * Check if a file already exist and display dialog to confirm overwritting of the file
   *
   ...
   * @return given file if a directory, parent directory otherwise, null if given file doesn't exist
   */
