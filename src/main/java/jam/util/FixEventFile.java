...
/**
 * This program takes the first data block of a specified event file, removes it, and appends it to
 * the end of a second specified event file. The first file is typically run number "x" and the
 * second file is for run number "x-1".
 *
 * @author <a href="mailto:dwvisser@users.sourceforge.net">Dale W. Visser</a>
 */
public class FixEventFile {

  private static final String PACKAGENAME = FixEventFile.class.getPackage().getName();
  ...
  FixEventFile(final File inputFile) {
    ...
    try {
      ...
      final FileUtilities fileUtil = GuiceInjector.getObjectInstance(FileUtilities.class);
  ...
  private void copyFile(final File src, final File dest) {
    ...
    try {
      ...
      final BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(dest));
  ...
  private void processCurrentRun(final int currentRun) {
    ...
    final File currentSourceFile = new File(directory, expName + currentRun + evn);
    ...
    try {
      ...
      final BufferedInputStream fromStream = new BufferedInputStream(fromFile);
      final BufferedOutputStream modStream = new BufferedOutputStream(modFile);
      final BufferedOutputStream appendStream = new BufferedOutputStream(appendFile);
      ...
      if (numRead < header.length) {
        LOGGER.severe("Expected " + header.length + " bytes, only got" + numRead + ".");
      }
      ...
      if (numRead < dataBlock.length) {
        LOGGER.severe("Expected " + dataBlock.length + " bytes, only got" + numRead + ".");
      }
