...
public final class PeakFinder {
   ...
   */
  public static PeakFinder getInstance() {
    return instance;
  }
  ...
  /** Maximum separation in sigma between peaks to count them as being in the same multiplet. */
  private static final double MAX_SEP = 1.3;

  /**
   * Given a spectrum and search parameters, performs a digital filter peak search as specified in
   * V. Hnatowicz et al in Comp Phys Comm 60 (1990) 111-125. Setting the sensitivity to a typical
   * value of 3 gives a 3% chance for any peak found to be false.
   *
  ...
  public List<Double> getCentroids(
      final double[] data, final double _sensitivity, final double _width) {
    spectrum = data.clone();
  ...
  private List<Multiplet> peakFind() {
    ...
    for (int i = filterLimit + 1; i < spectrum.length - filterLimit - 1; i++) {
      if (sum1[i] > sensitivity * Math.sqrt(sum2[i])
          && sum1[i] > sum1[i - 1]
          && sum1[i] > sum1[i + 1]) { // conditions
        // met,
        // calculate
        // centroid
        final double posn =
            (sum1[i - 1] * (i - 1) + sum1[i] * i + sum1[i + 1] * (i + 1))
  ...
  private List<Multiplet> breakUp(final Multiplet peaks) {
    ...
    for (int i = 0; i < peaks.size(); i++) {
      if (i == 0
          || (peaks.get(i).getPosition() - peaks.get(i - 1).getPosition()) > (MAX_SEP * width)) {
  ...
  private void renormalize(final List<Multiplet> multiplets) {
    for (Multiplet multiplet : multiplets) {
      ...
      for (int ch = (int) Math.round(multiplet.get(0).getPosition() - width * MAX_SEP);
          ch
              < (int)
                  Math.round(multiplet.get(multiplet.size() - 1).getPosition() + width * MAX_SEP);
          ch++) {
  ...
  private Peak correctPeak(final Multiplet src, final int index) {
    ...
    if (index == 0) {
      ...
      final double dLast = thisPeak.getPosition() - lastPeak.getPosition();
      final double kLast = getK(dLast);
      correction = (nextPeak.getArea() * kNext + lastPeak.getArea() * kLast) / thisPeak.getArea();
    }
  ...
  private double getK(final double diff) {
    ...
    final double kval =
        diff * Math.exp(-diffSq / (4.0 * sigmaSq)) * (1.0 - diffSq / (6.0 * sigmaSq));
    return kval;
  }
