...
public final class Group implements Nameable {
  ...
  private static final NameValueCollection<Group> COLLECTION = Warehouse.getGroupCollection();

  /** The histograms in this group. */
  public final transient NameValueCollection<AbstractHistogram> histograms =
      Factory.createHistogramCollection();
  ...
  private final transient List<Scaler> scalerList = new ArrayList<>();
   ...
   */
  protected Group(final String groupName, final Type type, final String fileName) {
    super();
   ...
   *
   * @param name the new name of a group
  ...
  protected HistDouble2D createHistDouble2D(
      ...
      final String labelY,
      final double[][] array) {
    final HistDouble2D result = new HistDouble2D(title, labelX, labelY, array);
  ...
  protected HistDouble1D createHistDouble1D(
      ...
      final String labelY,
      final double[] array) {
    final HistDouble1D result = new HistDouble1D(title, labelX, labelY, array);
    addGroupInfoToHist(result, name);
    return result;
  }
