...
public final class Factory {
  ...
  /**
   * Set a group as the current group, create the group if it does not already exist
   *
  ...
  public static Group createGroup(
      final String groupName, final String fileName, final Group.Type type) {
   ...
   */
  public static AbstractHistogram createHistogram(
      final Group group,
      final Object array,
      final String name,
      final String title,
    ...
    final jam.data.HistogramType hType = jam.data.HistogramType.getArrayType(array);
    if (hType == jam.data.HistogramType.ONE_DIM_INT) {
      rval = group.createHistInt1D(name, title, labelX, labelY, (int[]) array);
    } else if (hType == jam.data.HistogramType.ONE_D_DOUBLE) {
      rval = group.createHistDouble1D(name, title, labelX, labelY, (double[]) array);
    } else if (hType == jam.data.HistogramType.TWO_DIM_INT) {
      rval = group.createHistInt2D(name, title, labelX, labelY, (int[][]) array);
    } else { // TWO_D_DOUBLE
      rval = group.createHistDouble2D(name, title, labelX, labelY, (double[][]) array);
    }
    return rval;
  }
   ...
   * @param nameIn desired name
   * @param idNum desired id number
   * @return the scaler object
   */
  public static Scaler createScaler(final Group group, final String nameIn, final int idNum) {
    ...
    final String name = stringUtil.makeUniqueName(nameIn, scalerNames, Scaler.NAME_LENGTH);
   ...
   */
  public static AbstractHistogram createHistogram(
      final Group group, final Object array, final String name) {
   ...
   */
  public static AbstractHistogram createHistogram(
      final Group group, final Object array, final String name, final String title) {
