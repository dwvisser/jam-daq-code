...
public final class Scaler implements DataElement {
  ...
  private static final Broadcaster BROADCASTER = GuiceInjector.getObjectInstance(Broadcaster.class);

  private static final List<Scaler> LIST = Collections.synchronizedList(new ArrayList<Scaler>());
  ...
  /**
   * Update all the scaler values. The value indices refer to the scaler number.
   *
  ...
  private final transient String name; // name of scaler

  private final transient int number; // number in list
   ...
   *
   * @param nameIn name of the scaler, which must be <=16 characters
   * @param idNum number of scaler, most often the same as the register number in a CAMAC scaler
   *     unit
