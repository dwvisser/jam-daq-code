...
public final class GateSet extends AbstractControl {
  ...
  private final transient JButton addP, removeP, unset, save, cancel;

  private final transient JComboBox<Object> cgate;

  private transient Gate currentGate;

  private transient Nameable currentHistogram = UnNamed.getSingletonInstance();

  private final transient List<Bin> gatePoints = new ArrayList<>();

  private final transient JLabel lLower, lUpper;

  private transient boolean newGate = false; // a gate has been chosen

  private final transient JTextField textLower, textUpper;
   ...
   * @param gateListRender renders the gate list elements
   */
  @Inject
  public GateSet(
      final Frame frame, final Broadcaster broadcaster, final GateListCellRenderer gateListRender) {
        ...
        if (currentHistogram instanceof AbstractHist1D) {
          synchronized (gatePoints) {
            if (gatePoints.isEmpty()) {
              ...
              LOGGER.severe(
                  getClass().getName() + ".addPoint(): setting 1 d gate should not be here.");
            }
          }
  ...
  private final transient Canceller canceller =
      new Canceller() {
        public void cancel() {
          checkHistogram();
  ...
  /**
   * Check that plot's current histogram has not changed. If so, cancel and make the plot's current
   * histogram our current histogram.
   *
   * @author Ken Swartz
   */
  ...
  private void printPoints(final Polygon poly) {
    ...
    for (int i = 0; i < poly.npoints; i++) {
      msg.append('[').append(xcoord[i]).append(',').append(ycoord[i]).append("] ");
    }
  ...
  private void saveGate() {
    if (currentHistogram instanceof AbstractHist1D) {
      ...
      LOGGER.info("Gate Set " + currentGate.getName() + " Limits=" + lim1 + "," + lim2);
    } else if (currentHistogram instanceof AbstractHist2D) {
