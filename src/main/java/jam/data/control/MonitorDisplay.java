...
public class MonitorDisplay extends AbstractControl {
  ...
  private final transient JToggleButton checkAudio;

  private final transient JPanel pBars;

  private final transient AcquisitionStatus status;

  /**
   * Constructs a new monitor display dialog.
   *
   * @param frame application frame
   * @param status acquisition status
   * @param broadcaster broadcasts state changes
   */
  @Inject
  public MonitorDisplay(
      final Frame frame, final AcquisitionStatus status, final Broadcaster broadcaster) {
  ...
  private JPanel createPanel(final Monitor monitor) {
    ...
    pBars.add(pMonitors);
    final JLabel labelDisp = new JLabel(monitor.getName(), SwingConstants.RIGHT);
  ...
  public void doSetup() {
    ...
    if (numberMonitors > 0) {
      final Dimension dialogDim =
          calculateScrollDialogSize(this, monitorPanel, BORDER_HEIGHT, numberMonitors);
      setSize(dialogDim);
    }
  }
