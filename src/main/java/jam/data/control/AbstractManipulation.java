...
/**
 * Base class for histogram manipulations has methods to create combo box of histograms
 *
 * @author Ken Swartz
 */
abstract class AbstractManipulation extends AbstractControl {
  ...
  protected static final String NEW_HIST = "NEW: ";

  /** String to append to new histogram group in combo box */
  private static final String WILD_CARD = "/.";

  private static final NameValueCollection<Group> GROUPS = jam.data.Warehouse.getGroupCollection();
   ...
   * @param broadcaster broadcast status updates
   */
  public AbstractManipulation(
      final Frame frame, final String title, final boolean modal, final Broadcaster broadcaster) {
   ...
   */
  protected void loadAllHists(
      final JComboBox<Object> comboBox, final boolean addNew, final int histDim) {
    ...
    if (addNew) {
      ...
      for (Group group : GROUPS.getList()) {
        if (group.getType() != Group.Type.SORT && !Group.WORKING_NAME.equals(group.getName())) {
  ...
  protected String parseGroupName(final String name) {
    ...
    final String groupName =
        builder.substring(NEW_HIST.length(), name.length() - WILD_CARD.length());
    return groupName;
  }
   ...
   *
   * @param groupName name of group for hist
   * @param histName name of histogram
   * @param size number of channels in histogram
