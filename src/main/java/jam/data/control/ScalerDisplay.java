...
public final class ScalerDisplay extends AbstractControl {
  ...
  private final transient JButton bupdate = new JButton("Read");

  private final transient JButton bzero = new JButton("Zero");

  private final transient JCheckBox checkDisabled = new JCheckBox("Disable Zero", true);

  private final transient Object monitor = new Object();

  private final transient JPanel pScalers;

  private final transient JamStatus status;

  private final transient List<JTextField> textScaler = new ArrayList<>();

  private final transient BroadcastUtilities broadcast;

  /**
   * Creates the dialog box for reading and zeroing scalers.
   *
   * @param frame application frame
   * @param broadcast for broadcasting scaler commands
  ...
  public ScalerDisplay(
      final Frame frame,
      final BroadcastUtilities broadcast,
    ...
    final Border borderScalers = new EmptyBorder(BORDER_HEIGHT, 10, BORDER_HEIGHT, 10);
  ...
  private void addScalerControlPanel(final Container cddisp) {
    final JPanel plower = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 10));
  ...
  /**
   * @param currentScaler to creat label for
  ...
  /**
   * @param currentScaler scaler to use value of
   */
  ...
  public void displayScalers() {
    synchronized (monitor) {
      ...
      if (DataBase.getInstance().isValid(nameable)) {
        ...
        final List<? extends DataElement> scalerList = currentGroup.getScalerList();
        ...
        for (DataElement currentScaler : currentGroup.getScalerList()) {
          ...
          text.setText(String.valueOf((int) currentScaler.getCount()));
        }
      }
    }
  }
      ...
      if (DataBase.getInstance().isValid(nameable)) {
        ...
        if (numberScalers > 0) {
          final Dimension dialogDim =
              calculateScrollDialogSize(this, panelS, BORDER_HEIGHT, numberScalers);
          setSize(dialogDim);
        }
        displayScalers();
      }
  ...
  /**
   * Read the scaler values send out command to read scalers, which should be received by
   * VMECommunication. VME should then send a command to CAMAC to read the scalers, when VME
   * receives back the scaler values it calls distribute event which will call our update method.
   */
  public void read() {
