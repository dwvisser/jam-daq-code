...
public class GroupRename extends AbstractControl {
   ...
   */
  @Inject
  public GroupRename(final Frame frame, final JamStatus status, final Broadcaster broadcaster) {
    ...
    if (show) {
      ...
      if (DataBase.getInstance().isValid(nameable)) {
        ...
        if (currentGroup.getType() == Group.Type.SORT) {
          LOGGER.severe(
              "Cannot rename sort groups, selected group " + currentGroup.getName() + ".");
        } else {
