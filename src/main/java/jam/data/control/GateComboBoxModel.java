...
final class GateComboBoxModel extends DefaultComboBoxModel<Object> {
  ...
  /**
   * Class representing the possible modes of GateComboBoxModel's. Only two modes exist now, which
   * are accessible as static fields of this class.
   *
   * @version 1.4.2 (RC3)
   * @author <a href="mailto:dwvisser@users.sourceforge.net">Dale Visser</a>
   */
  public enum Mode {
    /** The mode for which all gates of the same dimensionality of the displayed histogram. */
    ALL,

    /** The mode for which only gates belonging to the displayed histogram are listed. */
    DISPLAYED_HIST
  }

  private final transient List<Object> lastValue = Collections.synchronizedList(new ArrayList<>());

  private final transient Mode mode;

  private transient Object selection = null;

  /** Create the default model that shows gates for the currently displayed histogram. */
  GateComboBoxModel() {
   ...
   *
   * @param listmode whether just gates for the current histogram or all histograms of the same
   *     dimensionality should be shown
   */
  ...
  public Object getElementAt(final int index) {
    ...
    if (numGates() > 0) {
      if (index == 0) {
        ...
        final AbstractHistogram his = (AbstractHistogram) SelectionTree.getCurrentHistogram();
