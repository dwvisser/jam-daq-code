...
public final class Projections extends AbstractManipulation {
  ...
  private final transient JComboBox<Object> cfrom, cto, cchan;

  private final transient JCheckBox cdown;

  private final transient JTextField tlim1, tlim2, ttextto;
  ...
  public Projections(
    ...
    cfrom = new JComboBox<>(new HistogramComboBoxModel(HistogramComboBoxModel.Mode.TWO_D));
    ...
    pEntries.add(pradio);
    /* Channels panel */
    final JPanel pchannel = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 0));
  ...
  /**
   * Loads the name of the current histogram and the list of gates for that histogram. For a 1d
   * histogram set co-ordinates as x y if 2d or lower and upper if 1 d
   */
  @Override
  public void doSetup() {
    cfrom.setSelectedIndex(0);
  ...
  private void project() throws DataException {
    ...
    final AbstractHistogram hfrom = AbstractHistogram.getHistogram(hfromname);
    counts2d =
        (hfrom.getType() == HistogramType.TWO_D_DOUBLE)
            ? ((HistDouble2D) hfrom).getCounts()
            : this.numberUtilities.intToDouble2DArray(((HistInt2D) hfrom).getCounts());
  ...
  private void internalProject(
      final double[][] counts2d,
      final AbstractHistogram hfrom,
      final int[] limits,
      final boolean gateSelected,
      final Gate gate)
      throws DataException {
    double[] countsDouble;
    final StringBuffer typeProj = new StringBuffer();
    if (cdown.isSelected()) {
      countsDouble = calculateXprojection(counts2d, limits, gateSelected, gate, typeProj);
    } else { // cacross is true
      countsDouble = calculateYprojection(counts2d, limits, gateSelected, gate, typeProj);
    }
    setProjectionCounts(countsDouble);
    LOGGER.info(
        "Project " + hfrom.getFullName().trim() + " to " + hto.getFullName() + " " + typeProj);
  }

  private double[] calculateYprojection(
      final double[][] counts2d,
      final int[] limits,
      final boolean gateSelected,
      final Gate gate,
    ...
    if (gateSelected) {
      ...
      typeProj
          .append("counts between X channels ")
          .append(limits[0])
          .append(" and ")
          .append(limits[1]);
      countsDouble = projectY(counts2d, hto.getSizeX(), limits[0], limits[1]);
    }
    return countsDouble;
  }

  private double[] calculateXprojection(
      final double[][] counts2d,
      final int[] limits,
      final boolean gateSelected,
      final Gate gate,
    ...
    if (gateSelected) {
      ...
      typeProj
          .append("counts between Y channels ")
          .append(limits[0])
          .append(" and ")
          .append(limits[1]);
      countsDouble = projectX(counts2d, hto.getSizeX(), limits[0], limits[1]);
    }
    return countsDouble;
  }

  private void setProjectionCounts(final double[] countsDouble) throws DataException {
  ...
  private int[] setLimits(final double[][] counts2d, final boolean between, final boolean full)
      throws DataException {
  ...
  private void getDestinationHistogram(final AbstractHistogram hfrom, final String name) {
    if (isNewHistogram(name)) {
  ...
  protected double[] projectX(
      final double[][] inArray, final int outLength, final int _ll, final int _ul) {
  ...
  private double[] projectX(final double[][] inArray, final int outLength, final Gate gate) {
    double[] out = new double[outLength];
  ...
  private double[] projectY(final double[][] inArray, final int outLength, final Gate gate) {
    double[] out = new double[outLength];
    for (int i = 0; i < inArray.length; i++) {
      for (int j = 0; j < inArray[0].length; j++) {
