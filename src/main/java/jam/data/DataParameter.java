 ...
 * box for the user to enter values for these parameters. In a sort routine you use
 * parameter.getValue() to get the value entered into the dialog box.
 *
 * @author Ken Swartz
 * @version 0.9
...
public class DataParameter implements Nameable {
  ...
  private static final Map<String, DataParameter> TABLE =
      Collections.synchronizedMap(new HashMap<String, DataParameter>());
  private static final List<DataParameter> LIST =
      Collections.synchronizedList(new ArrayList<DataParameter>());

  /** Limit on name length. */
  public static final int NAME_LENGTH = 16;

  private final transient String name; // parameter name
