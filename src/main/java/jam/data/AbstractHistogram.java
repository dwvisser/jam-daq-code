...
public abstract class AbstractHistogram implements DataElement {
  ...
  private static final List<List<AbstractHistogram>> DIM_LIST = new ArrayList<>(2);

  private static final String EMPTY_STRING = "";

  /* histogramList is ordered by the creation of the histograms */
  private static final List<AbstractHistogram> LIST = new ArrayList<>();
  ...
  private final transient GateCollection gates;
  ...
  public static List<AbstractHistogram> getHistogramList(final int dim) {
    if (dim < 1 || dim > 2) {
      throw new IllegalArgumentException("Expect 1 or 2, the possible numbers of dimensions.");
    }
    return Collections.unmodifiableList(DIM_LIST.get(dim - 1));
  }

  /**
   * @param names of histograms we want
   * @param <T> most general type of histogram to include in the list
   * @param type how the type gets specified
  ...
  private final transient int sizeX; // size of histogram, for 1d size for

  private final transient int sizeY; // size used for 2d histograms y size

  /** title of histogram */
  private transient String title;

  private final transient jam.data.HistogramType type; // one or two dimension
  ...
  protected AbstractHistogram(
      final jam.data.HistogramType type, final int sizeX, final int sizeY, final String title) {
    super();
    this.type = type;
    this.sizeX = sizeX;
    this.sizeY = sizeY;
   ...
   */
  protected AbstractHistogram(
      final jam.data.HistogramType type,
      final int sizeX,
      final int sizeY,
      final String title,
   ...
   */
  protected AbstractHistogram(
      final jam.data.HistogramType type, final int size, final String title) {
   ...
   *
   * @param countsIn 1d or 2d array of <code>int</code>'s or <code>double</code> 's, according to
   *     this histogram's type
  ...
  /**
   * Returns the histogram full name that resolves it. (could change if multiple histograms have the
   * same name)
   *
   ...
   *
   * <ul>
   *   <li><code>ONE_DIM_INT</code>
   *   <li><code>TWO_DIM_INT</code>
   *   <li><code>ONE_DIM_DOUBLE</code>
   *   <li><code>TWO_DIM_DOUBLE</code>
   * </ul>
   *
   * @return the type
   * @see jam.data.HistogramType
  ...
  /**
   * Set the counts array using the given <code>int</code> or <code>double</code> array.
   *
   * @param countsIn 1d or 2d array of <code>int</code>'s or <code>double</code> 's
   * @throws IllegalArgumentException if countsIn is the wrong type.
   */
   ...
   * @throws IllegalStateException if x-axis label has already been explicitly set
   */
  public final void setLabelX(final String label) {
    if (labelX.length() > 0) {
      throw new IllegalStateException("Please call setLabelX() only once per histogram.");
    }
    labelX = label;
  }
   ...
   * @throws IllegalStateException if y-axis label has already been explicitly set
   */
  public final void setLabelY(final String label) {
    if (labelY.length() > 0) {
      throw new IllegalStateException("Please call setLabelY() only once per histogram.");
    }
    labelY = label;
  }

  /**
   * Sets the number of this histogram. May have the side effect of bumping another histogram with
   * the desired number to a new number.
   *
   * @param num the desired number for the histogram
   */
  ...
  protected final void updateNames(final Nameable group) {
    ...
    uniqueName =
        GuiceInjector.getObjectInstance(StringUtilities.class).makeFullName(groupName, name);
    NAME_MAP.put(uniqueName, this);
  }

  /** Clear this histogram's data and delete it from all lists that refer to it. */
  public void delete() {
    clearInfo();
    LIST.remove(this);
