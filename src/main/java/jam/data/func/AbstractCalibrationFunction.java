 ...
 * calibrations of spectra. But could also do time of flight an rho for a magnetic spectrometer.
 *
 * @author Ken Swartz
 * @version 1.0
 */
public abstract class AbstractCalibrationFunction implements Function {
  ...
  AbstractCalibrationFunction(final String name, final int numberTerms) {
    ...
    if (numberTerms < MAX_TERMS) {
      ...
      throw new IllegalArgumentException(
          getClass().getName() + "--Maximum terms: " + MAX_TERMS + ", asked for: " + numberTerms);
    }
  }
  ...
  /**
   * @param numFormat the number format to use
   ...
   */
  public void setPoints(final double[] ptsChannelIn, final double[] ptsEnergyIn) {
    setIsFitPoints(true);
  ...
  /**
   * Set the coefficients of the calibration function using the contents of the passed <code>Array
   * </code>. If passed a larger than necessary array, the first elements of the array will be used.
   *
  ...
  /**
   * Gets the channel for the given energy. Don't always have a inverse function so by default
   ...
   *
   * @throws CalibrationFitException if the fit fails
   */
  ...
  protected void loadIcon(final String iconFile) {
    ...
    if (urlIcon == null) {
      JOptionPane.showMessageDialog(
          null, "Can't load resource for calibration function icon " + iconFile);
    } else {
      CalibrationFunctionCollection.setIcon(this.getName(), new ImageIcon(urlIcon));
    }
  }
}
