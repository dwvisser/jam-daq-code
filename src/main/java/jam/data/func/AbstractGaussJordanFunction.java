 ...
 * @author Dale Visser
 */
public abstract class AbstractGaussJordanFunction extends AbstractCalibrationFunction {
  /**
   * @param name name
   * @param numberTerms number of terms
   */
  ...
  /**
   * do a least squares fit of data points y=a+bx returns to fit a, and b values in an array
   *
   ...
   */
  protected double[] polynomialFit(final double[] xVal, final double[] yVal, final int order)
      throws CalibrationFitException {
    final int numTerms = order + 1;
    // Check data
    if (xVal.length < numTerms) {
      throw new IllegalArgumentException("Need more positions than order for polynomial fit");
    }
    if (xVal.length != yVal.length) {
      throw new IllegalArgumentException("Need same number of x and y points for polynomial fit");
    }
      ...
      for (int j = 0; j < alphaLength; j++) {
        if (pivotIndices[j] != 1) {
          for (int k = 0; k < alphaLength; k++) {
            if (pivotIndices[k] == 0 && Math.abs(alpha[j][k]) >= big) {
  ...
  private void pivot(double[][] alpha, final int[] columnIndices, final int[] rowIndices) {
    final int alphaLength = alpha.length;
  ...
  private void normalizeToPivotAndSubtract(
      double[][] alpha, final double[][] beta, final int column) {
  ...
  private void subtractRows(double[][] array, final int row1, final int row2, final double factor) {
