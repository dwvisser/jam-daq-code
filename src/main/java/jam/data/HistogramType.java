...
public final class HistogramType {
  private static final int[] DIM = {1, 2, 1, 2};

  private static final boolean[] INT = {true, true, false, false};
  ...
  private static final String[] STRING = {"1D int", "2D int", "1D double", "2D double"};
  ...
  private final transient int typeNum;
  ...
  public Object getSampleArray(final int sizeX, final int sizeY) {
    final Object rval;
    if (sizeY == 0) {
      rval = isInteger() ? new int[sizeX] : new double[sizeX];
    } else {
      rval = isInteger() ? new int[sizeX][sizeY] : new double[sizeX][sizeY];
    }
    return rval;
  }
