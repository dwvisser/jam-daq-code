...
public final class Gate implements DataElement {
  ...
  private static final List<Gate> LIST = Collections.synchronizedList(new ArrayList<Gate>());
   ...
   *
   * @param fullName name of the desired gate
  ...
  private final transient Polygon bananaGate = new Polygon(); // polygon of 2d
  ...
  private final transient String histUniqueName; // histogram gate belongs to
  ...
  private final transient String name; // name of gate

  private final transient int sizeX; // size of gate for 1d and x of 2d

  private final transient int sizeY; // size used for 2d histograms only

  private final transient String uniqueName; // unique name of group
   ...
   *
   * @param nameIn name of the gate which will be put on the chooser in the display
  ...
  public Polygon getBananaGate() {
    if (dimensions != 2) {
      throw new UnsupportedOperationException("getBananaGate can only be called for 2D gates.");
    }
    return bananaGate;
  }
  ...
  /**
   * Returns the dimensionality of this <code>Gate</code>, which is the same as its associated
   * <code>Histogram</code>.
   *
  ...
  public int[] getLimits1d() {
    ...
    if (dimensions != 1) {
      throw new UnsupportedOperationException("getLimits1d(): can only be called for 1D gates.");
    }
    bounds[0] = lowerLimit;
    bounds[1] = upperLimit;
    return bounds;
  ...
  public boolean[][] getLimits2d() {
    if (dimensions != 2) {
      throw new UnsupportedOperationException("getLimits2d(): can only be called for 2D gates.");
    }
  ...
  /**
   * Returns whether channel is inside 1-d gate. If the channel is equal to wither limiting channel
   * or between them, it is inside the gate.
   *
  ...
  public boolean inGate(final int channelX, final int channelY) {
    ...
    if (dimensions != 2) {
      throw new UnsupportedOperationException("inGate(int,int): can only be called for 2D gates");
    }
    if (isSet && (channelX >= 0) && (channelX < sizeX) && (channelY >= 0) && (channelY < sizeY)) {
  ...
  /**
   * Gets number of individual points in gate. For 1-d, this is always 2. For 2-d banana gates the
   * number is variable. This is part of the class for <code>HistogramIO</code> to be able to write
   * HDF files more efficiently.
   *
  ...
  /**
   * "Removes" the gate's limits, so that it will return false for all calls to <code>inGate()
   * </code>.
   */
