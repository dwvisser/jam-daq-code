...
/**
 * Generates the "splash" window that displays while Jam is launching. The window disappears after a
 * specified timeout period or when the user clicks on it.
 */
class SplashWindow extends JWindow {

  private final transient URL urlNukeIcon;

  private final transient URL urlOSIGif;

  private static final Logger LOGGER = Logger.getLogger(SplashWindow.class.getPackage().getName());

  /**
   * Creates the splash window which will exist for as long as the specified wait time in
   * milliseconds.
   *
    ...
    final Runnable waitRunner =
        () -> {
          try {
            ...
            LOGGER.log(Level.SEVERE, "Exception occured trying to display splash window.", e);
  ...
  private void drawWindow() {
    ...
    final int sizexy = 80;
    nukeicon.setImage(nukeicon.getImage().getScaledInstance(sizexy, sizexy, Image.SCALE_SMOOTH));
    ...
    final JLabel versionLabel =
        new JLabel(
            "v. " + GuiceInjector.getObjectInstance(Version.class).getName(), SwingConstants.RIGHT);
    panelSouth.add(versionLabel);
    panelSouth.setBackground(Color.CYAN);
    panelSouth.setBorder(BorderFactory.createMatteBorder(0, 1, 1, 1, Color.BLACK));
