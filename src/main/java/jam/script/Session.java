...
/**
 * Class which exposes an API for scripting offline sorting sessions. Using this class, you can
 * write and compile a .java file which, when executed, will
 *
 * <ol>
 *   <li>Launch Jam in the background.
 *   <li>Setup offline sorting.
 *   <li>Load an HDF file (i.e., load gates and parameters).
 *   <li>Zero histograms (often needed after the previous step).
 *   <li>Define the list of event files to sort.
 *   <li>For sort routines that invoke <code>writeEvent()</code>, specify the event output file.
 *   <li>(Optional) Show the Jam window to observe sort progress.
 *   <li><em>Perform the sort.</em>
 *   <li>Save the results in an HDF file.
 *   <li>(Optional) Add histograms in stored HDF files together.
 * </ol>
 *
 * <p>The last step may be desirable in the case where you would like to execute portions of the
 * sorting task on different machines at the same time, and combine their results in a "merge
 * script".
 *
...
public final class Session implements PropertyChangeListener {
  ...
  private static final Logger LOGGER = Logger.getLogger(Session.class.getPackage().getName());

  private static final String OUTSTREAM = "\tout: ";

  private final transient File base;
  ...
  private final transient RunControl runControl;

  private final transient SetupSortOff setupSortOffline;

  private final transient SetupSortOn setupSortOnline;

  private final transient SortControl sortControl;

  private transient RunState state = RunState.NO_ACQ;

  private final transient JFrame frame;

  private final transient HistogramZero histogramZero;

  private final transient CommandListener listener;

  /**
   * Creates an instance, of which the user then invokes the methods to script an offline sorting
   * session. A non-trivial side-effect of invoking this constructor is that an instance of Jam is
   * started up in the background.
   *
   * @param frame parent frame
   * @param sortOffline sort offline setup dialog
   * @param sortOnline sort online setup dialog
   * @param sortControl offline sort control
   * @param runControl online run control
   * @param hdfio HDF I/O
  ...
  protected Session(
      final JFrame frame,
      final SetupSortOff sortOffline,
      final SetupSortOn sortOnline,
      final SortControl sortControl,
  ...
  /**
   * Add an event file to the list of event files to sort. If the file reference passed represents a
   * directory, then all files with names ending in <code>.evn</code> in the directory are added to
   * the list. Files whose names don't meet this requirement are ignored and not added to the list.
   * This method does <em>not</em> recurse into the directory tree.
   *
   * @param fileOrDir <code>.evn</code> file or folder containing such files
   * @throws IllegalStateException if <code>setupOffline()</code> hasn't been called yet
   */
  public void addEventFile(final File fileOrDir) {
    ...
    if (fileOrDir.isFile()) {
      LOGGER.log(Level.INFO, "Added event file to sort: " + fileOrDir.getName());
    }
    if (fileOrDir.isDirectory()) {
      LOGGER.log(Level.INFO, "Added event files in folder: " + fileOrDir.getName());
    }
    if (numFiles > 0) {
      filesGiven = true;
    } else {
      LOGGER.log(Level.WARNING, fileOrDir.getName() + " didn't contain any usable files.");
    }
  }

  /**
   * Add the histogram counts in the given HDF file, or all HDF files in the given directory, into
   * the histograms in memory. You must have already invoked <code>setupSort()</code>.
   *
  ...
  public void addHDF(final File hdf) {
    ...
    if (!isSetup) {
      throw new IllegalStateException("You may not call loadHDF() before calling setupOffline().");
    }
    if (hdf.isDirectory()) {
   ...
   *
   * @throws IllegalStateException if <code>setupOffline()</code> hasn't been called yet, or if no
   *     event files have been specified
   */
  public void beginSort() {
    ...
    try {
      ...
      LOGGER.log(Level.SEVERE, "Interrupted while waiting for sort to finish.", e);
    } catch (Exception e) {
  ...
  /**
   * Utility method for defining a file reference relative to the directory that java was launched
   * from. Here's an example for Linux and MacOS X systems: If the JVM was launched from <code>
   * /home/user/sortscript</code>, and this method is called with the argument <code>
   * "../thesisRuns/run34.evn"</code>, then a File object representing <code>
   * /home/user/thesisRuns/run34.evn</code> is returned, <em>whether or not that file really exists
   * yet</em>.
   *
   * @param fname relative path reference to the file of interest
   * @return File object resolving <code>fname</code> against the base path given by the system
  ...
  /**
   * Reads in the given text file, interpreting each line as a filename, which gets added to the
   * list of event files to sort. Such files are typically created using the <code>Save List</code>
   * button in the <code>Control|Sort...</code> dialog in Jam.
   *
  ...
  public void loadFileList(final File list) {
    ...
    if (numFiles > 0) {
      ...
      LOGGER.log(Level.WARNING, list.getName() + " didn't contain any usable filenames.");
    }
  }

  /**
   * Load the given HDF file into memory. You must have already invoked <code>setupOffline()</code>.
   *
  ...
  public void loadHDF(final File hdf) {
    ...
    hdfio.readFile(FileOpenMode.RELOAD, hdf, Warehouse.getSortGroupGetter().getSortGroup());
    LOGGER.log(Level.INFO, "Loaded HDF file: " + hdf);
  }

  /** Used to unload the offline sort setup, including emptying the list of files to sort. */
  ...
  /**
   * Set the file to output events to when the user's sort routine invokes <code>writeEvent()</code>
   *
  ...
  public void setEventOutput(final File eventsOut) {
    ...
    LOGGER.log(Level.INFO, "Set file for pre-sorted events" + eventsOut.getAbsolutePath());
  }

  /**
   * Completes the task equivalent of specifying the settings in Jam's dialog for setting up offline
   * sorting. That is, calling this defines the classpath to sort routines, the fully qualified name
   * of the <code>AbstractSortRoutine</code> to use for sorting, and references to the <code>
   * EventInputStream</code> and <code>EventOutputStream</code> to use.
   *
   * @param classPath the path that sort routines get loaded from
   * @param sortName fully qualified with all package names in the standard java "dot" notation,
   *     e.g., <code>"sort.Calorimeter"</code> for the file <code>sort/Calorimeter.class</code>
   *     relative to <code>classPath</code>
  ...
  /**
   * Show Jam's graphical interface. Once shown, the user may interact with Jam like normal,
   * including interrupting the scripted process.
   */
  public void showJam() {
  ...
  public class Online {
    ...
    public void setup(
        ...
        final Class<? extends AbstractEventInputStream> inStream,
        final Class<? extends AbstractEventOutputStream> outStream) {
      Session.this.setupSortOnline.setupSort(classPath, sortName, inStream, outStream);
    ...
    public void setup(
        ...
        final Class<? extends AbstractEventInputStream> inStream,
        final Class<? extends AbstractEventOutputStream> outStream) {
      Session.this.setupSortOnline.setupSort(sortName, inStream, outStream);
      Session.LOGGER.log(Level.INFO, "Setup online sorting:");
