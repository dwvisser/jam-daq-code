...
/**
 * Class to communicate with VME crate using UDP packets Two UDP sockets can be setup with a daemon
 * that receives packets and knows what to do with them The first int of a packet indicates what
...
final class VMECommunication extends GoodThread implements VmeSender {
  ...
  private static final String ANOTHER_JAM = " " + "(Is another copy of Jam running online?)";

  private static final String STOPPED_WARNING =
      "Network " + "receive daemon stopped, need to restart Online.";
  ...
  /**
   * Creates the instance of this class for handling IP communications with the VME front end
   * computer.
   *
   * @param broadcaster broadcasts counter updates
   * @param packetBuilder constructs message packets
   * @param stringUtilities converts strings to byte arrays with ASCII characters
   */
  @Inject
  protected VMECommunication(
  ...
  /**
   * @param localInetAddress address of the machine this instance of Jam is running on
   * @param portSend port for sending
   * @param portRecv port for receiving
   * @param vmeInetAddress address of VME front end computer
   * @throws CommunicationsException if there is a problem communicating with the front end
   */
  protected void bindSocketsAndSetActive(
      final String localInetAddress,
      final int portSend,
      final int portRecv,
      final String vmeInetAddress)
    ...
    if (!active) {
      ...
      try { // create a ports to send and receive
        ...
        throw new CommunicationsException("Unknown local host: " + localInetAddress, ue);
      }
      try {
        addressVME = InetAddress.getByName(vmeInetAddress);
      } catch (UnknownHostException ue) {
        throw new CommunicationsException("Unknown VME host: " + vmeInetAddress, ue);
      }
      try {
        socketSend = new DatagramSocket(portSend, addressLocal);
      } catch (BindException be) {
        throw new CommunicationsException("Problem binding send socket." + ANOTHER_JAM, be);
      } catch (SocketException se) {
        throw new CommunicationsException("Problem creating send socket.", se);
      }
      try {
        socketReceive = new DatagramSocket(portRecv, addressLocal);
      } catch (BindException be) {
        throw new CommunicationsException("Problem binding receive socket." + ANOTHER_JAM, be);
      } catch (SocketException se) {
        throw new CommunicationsException("Problem creating receive socket.", se);
      }
  ...
  /**
   * Thread method: is a daemon for receiving packets from the VME crate. The first int of the
   * packet is the status word. It determines how the packet is to be handled.
   */
  @Override
  public void run() {
    ...
    try {
      ...
      LOGGER.log(Level.SEVERE, "Unable to read datagram status word.", ioe);
      LOGGER.warning(STOPPED_WARNING);
    }
  }

  private void unpackBuffer(
      final List<Integer> unpackedValues, final ByteBuffer byteBuffer, final int status) {
    if (status == PacketTypes.OK_MESSAGE.intValue()) {
      LOGGER.info(getClass().getName() + ": " + unPackMessage(byteBuffer));
      ...
      unPackCounters(byteBuffer, unpackedValues);
      broadcaster.broadcast(BroadcastEvent.Command.COUNTERS_UPDATE, unpackedValues);
    } else if (status == PacketTypes.ERROR.intValue()) {
      LOGGER.severe(getClass().getName() + ": " + unPackMessage(byteBuffer));
    } else {
      LOGGER.severe(getClass().getName() + ": packet with unknown message type received");
    }
  }

  /**
   * Method to send a CNAF list to the VME crate packet structure 16 bytes name 4 bytes number of
   * CNAF's 4 bytes for each CNAF.
   *
   * @param listName name of CNAF list to send
   * @param cnafList the CAMAC commands
   * @throws IOException if there's a problem
   */
  protected void sendCNAFList(final String listName, final List<CNAF> cnafList) throws IOException {
    GoodThread.LOGGER.entering(VMECommunication.class.getName(), "sendCNAFList", listName);
    ...
    final byte[] byteMessage =
        new byte[intSize + commandSize + intSize + cnafList.size() * cnafSize + 1];
    ...
    final DatagramPacket packetMessage =
        new DatagramPacket(byteMessage, byteMessage.length, addressVME, vmePort);
    if (socketSend == null) {
  ...
  protected void sendToVME(final PacketTypes status, final String message) {
    ...
    final DatagramPacket packetMessage =
        packetBuilder.message(status, message, addressVME, vmePort);
   ...
   * </dl>
   *
   * @param buffer message in readable form
   */
  private void unPackCounters(final ByteBuffer buffer, final List<Integer> destination) {
    synchronized (LOCK) {
  ...
  /**
   * Unpack a datagram with a message. Message packets have an ASCII character array terminated with
   * \0.
   *
   * @param buffer packet contents passed in readable form
