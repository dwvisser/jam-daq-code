 ...
 * @author Dale Visser
 */
public final class FrontEndVMECommunicator
    implements FrontEndCommunication, PreferenceChangeListener {

  private static final Object LOCK = new Object();

  private final transient VMECommunication vme;
  ...
  /**
   * New version uploads CAMAC CNAF commands with udp pakets, and sets up the camac crate.
   *
   * @param camacCommands object containing CAMAC CNAF commands
   * @throws IOException if there's a problem
   */
  public void setupCamac(final CamacCommands camacCommands) throws IOException {
  ...
  /**
   * Tells the front end to stop acquisition and end the run, which flushes out the data buffer with
   * an appended end-run marker.
   */
  public void end() {
  ...
  /**
   * Tells the VME whether to send verbose verbose status messages, which Jam automatically prints
   * on the console.
   *
  ...
  /**
   * Sets up networking. Two UDP sockets are created: one for receiving and one for sending. A
   * daemon is also set up for receiving.
   *
   * @throws CommunicationsException if something goes wrong
   */
  public void setupAcquisition() throws CommunicationsException {
    synchronized (LOCK) {
      final String LOCAL_IP = JamProperties.getPropString(PropertyKeys.HOST_IP);
      final int portSend = JamProperties.getPropInt(PropertyKeys.HOST_PORT_SEND);
      final int portRecv = JamProperties.getPropInt(PropertyKeys.HOST_PORT_RECV);
      final String VME_IP = JamProperties.getPropString(PropertyKeys.TARGET_IP);
      this.vme.bindSocketsAndSetActive(LOCAL_IP, portSend, portRecv, VME_IP);
  ...
  public void setupVMEmap(final VME_Map vmeMap) {
    ...
    for (VME_Channel channel : eventParams) {
      temp.append(channel.getSlot())
          .append(space)
          .append(hex)
          .append(Integer.toHexString(channel.getBaseAddress()))
          .append(space)
          .append(channel.getChannel())
          .append(space)
          .append(channel.getThreshold())
          .append(endl);
    }
    ...
    if (numRanges > 0) {
      for (Map.Entry<Integer, Byte> entry : hRanges.entrySet()) {
        ...
        temp.append(hex)
            .append(Integer.toHexString(base))
            .append(space)
