...
public class ScalerVMECommunicator implements ScalerCommunication, PropertyChangeListener {
  ...
  @Inject
  ScalerVMECommunicator(final VMECommunication vme, final Broadcaster broadcaster) {
    this.vme = vme;
    broadcaster.addPropertyChangeListener(this);
  }

  /**
   * Tell the VME to read the scalers, and send back two packets: the packet with the scaler values,
