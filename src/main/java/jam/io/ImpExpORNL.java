...
/**
 * Imports and exports Oak Ridge (Milner) formatted files, as used by <code>DAMM</code> and <code>
 * SORT</code>. A set of histograms consists of 2 files:
 *
 * <UL>
 *   <LI>Data file-- <code><i>filename</i>.his</code>
 *   <li>Directory file-- <code><i>filename</i>.drr</code>
 * </ul>
 *
...
public class ImpExpORNL extends AbstractImpExp { // NOPMD
  ...
  private static final ExtensionFileFilter FILTER = new ExtensionFileFilter(EXTS, "Oak Ridge DAMM");
  ...
  @Inject
  public ImpExpORNL(
      final Frame frame, final FileUtilities fileUtilities, final NumberUtilities numberUtilities) {
    super(frame);
  ...
  /**
   * Open a file which was written using ORNL .drr and .his format. Can't do the same as other
   * ImpExp routines, since 2 files are needed.
   *
   * @exception ImpExpException all exceptions given to <code>ImpExpException</code> display on the
   *     MessageHandler
   */
  @Override
   ...
   */
  private void readDrr(final InputStream buffin) throws IOException, ImpExpException {
    ...
    if (numRead != bsignature.length) {
      throw new ImpExpException("Unable to read in signature. Wrong # of characters: " + numRead);
    }
    final String signature = String.valueOf(bsignature);
    if (!(signature.equals(SIGNATURE))) {
      throw new ImpExpException(
          "Incorrect header, expected '" + SIGNATURE + "', but got '" + signature + "'.");
    }
  ...
  private void readDrrInfo(
      final byte[] parLabelb, final byte[] titleb, final DataInputStream disDrr)
      throws IOException {
  ...
  private void getCorrectByteOrder(final byte[] numHistByte) {
    if (!isCorrectByteOrder(numberUtilities.bytesToInt(numHistByte, 0, byteOrder))) {
   ...
   */
  private void readHist(final RandomAccessFile fileHis, final int index) throws IOException {
  ...
  private void read1dHistogram(
    ...
    if (numRead < bytesToRead) {
      throw new IllegalStateException(
          "Expected " + bytesToRead + " bytes, but only got " + numRead);
    }
    if (wordCh == 2) { // four byte data
      int offset = 0;
      for (int i = 0; i < sizeX; i++) {
        counts[i] = numberUtilities.bytesToInt(inBuffer, offset, byteOrder);
        offset += 4;
      }
    } else if (wordCh == 1) { // two byte data
      int offset = 0;
      for (int i = 0; i < sizeX; i++) {
        counts[i] = numberUtilities.bytesToShort(inBuffer, offset, byteOrder);
        offset += 2;
      }
    } else { // unable to handle data type
      throw new IOException("File uses " + wordCh + " words/channel, which can't be read.");
    }
    final AbstractHistogram hist = Factory.createHistogram(importGroup, counts, name);
    hist.setNumber(number);
  }

  private void read2dHistogram(
      final RandomAccessFile fileHis,
      final int index,
      final String name,
      final int number,
    ...
    if (wordCh == 2) { // four byte data
      ...
      for (int j = 0; j < sizeY; j++) {
        for (int i = 0; i < sizeX; i++) {
          counts2d[i][j] = numberUtilities.bytesToInt(inBuffer, offset, byteOrder);
          offset += 4;
        }
      }
    } else if (wordCh == 1) { // two byte data
      ...
      for (int j = 0; j < sizeY; j++) {
        for (int i = 0; i < sizeX; i++) {
          counts2d[i][j] = numberUtilities.bytesToShort(inBuffer, offset, byteOrder);
          offset += 2;
        }
      }
    ...
    final AbstractHistogram hist = Factory.createHistogram(importGroup, counts2d, name);
    hist.setNumber(number);
  }
  ...
  public void writeHist(final OutputStream ignored, final AbstractHistogram hist)
    ...
    try {
      final String fileNameHis =
          this.fileUtilities.changeExtension(getLastFile().getName(), ".his", FileUtilities.FORCE);
      final String fileNameDRR =
          this.fileUtilities.changeExtension(getLastFile().getName(), ".drr", FileUtilities.FORCE);
  ...
  private void writeDrr(final OutputStream buffout) throws IOException {
    ...
    final StringUtilities util = GuiceInjector.getObjectInstance(StringUtilities.class);
  ...
  private void writeHist1dInt(final DataOutputStream dosHis, final HistInt1D hist)
      throws IOException {
  ...
  private void writeHist1dDouble(final DataOutputStream dosHis, final HistDouble1D hist)
      throws IOException {
  ...
  private void writeHist2dInt(final DataOutputStream dosHis, final HistInt2D hist)
      throws IOException {
  ...
  private void writeHist2dDouble(final DataOutputStream dosHis, final HistDouble2D hist)
      throws IOException {
  ...
  /**
   * Opens a file with a specified dialog box title bar and file extension. It is usually called by
   * <code>openFile</code> in subclasses of <code>ImpExp</code>.
   *
  ...
  protected boolean openFile(final File file, final String msg) {
    ...
    try {
      ...
      LOGGER.log(
          Level.SEVERE,
          "Problem handling file \"" + inFile.getPath() + "\": " + ioe.getMessage(),
          ioe);
    } catch (ImpExpException iee) {
      LOGGER.log(Level.SEVERE, "Problem while importing or exporting: " + iee.getMessage(), iee);
    }
    return rval;
  }
}
