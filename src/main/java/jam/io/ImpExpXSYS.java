...
public class ImpExpXSYS extends AbstractImpExp { // NOPMD
  ...
  private static final ExtensionFileFilter FILTER = new ExtensionFileFilter("dat", "TUNL's XSYS");
  ...
  @Inject
  public ImpExpXSYS(
      final Frame frame, final Broadcaster broadcaster, final NumberUtilities numberUtilities) {
    super(frame);
    ...
    try {
      ...
      while (unPackHeaderXSYS(buffin)) {
        ...
        if (areaDataType == XSYS1DI4) {
          ...
          final int[] counts = unPackData1d(dis, areaSizeX, areaLengthPage);
          final AbstractHist1D hist =
              (AbstractHist1D) Factory.createHistogram(importGroup, counts, areaName, areaTitle);
          ...
          final int[][] counts2d = unPackData2d(dis, areaSizeX, areaSizeY, areaLengthPage);
          final AbstractHistogram hist =
              Factory.createHistogram(importGroup, counts2d, areaName, areaTitle);
          hist.setNumber(areaNumber);
          specRead++;
  ...
  private boolean unPackHeaderXSYS(final InputStream buffin) throws IOException, ImpExpException {
    ...
    if (!endOfFile) {
      ...
      if (firstHeader) {
        ...
        for (int i = 0; i < NUMBER_SCALERS; i++) {
          scalerTitles[i] =
              bufferToString(buffer, P_SCALER_TITLES + L_SCALER_TITLES * i, L_SCALER_TITLES);
  ...
  private int[][] unPackData2d(
    ...
    for (int channelX = 0; channelX < lengthX; channelX++) {
      for (int channelY = 0; channelY < lengthY; channelY++) {
        ...
        result[channelX][channelY] =
            this.numberUtilities.bytesToInt(tempBuff, index * L_INT, ByteOrder.LITTLE_ENDIAN);
        index++;
      }
    }
    return result;
   ...
   */
  private String bufferToString(final int[] inByteArray, final int position, final int length) {
