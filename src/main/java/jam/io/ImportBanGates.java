...
public final class ImportBanGates extends AbstractImpExp {
  ...
  private static final ExtensionFileFilter FILTER =
      new ExtensionFileFilter("ban", "ORNL Banana Gates");
  ...
  private void readGate(final StreamTokenizer parser) throws IOException {
    ...
    if (his == null) {
      LOGGER.warning("Couldn't load gate " + name + ". Expected histogram # " + hisNum + ".");
    } else {
  ...
  /**
   * @see jam.io.AbstractImpExp#writeHist(java.io.OutputStream, jam.data.AbstractHistogram)
   */
  @Override
