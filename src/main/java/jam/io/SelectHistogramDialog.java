...
class SelectHistogramDialog {
  ...
  private final transient JList<AbstractHistogram> histList;
  ...
  private void addToSelection() {
    final List<AbstractHistogram> selected = histList.getSelectedValuesList();
    ...
    lstHists.setListData(histFullSet.toArray(new AbstractHistogram[histFullSet.size()]));
  }

  protected void show() {
    final Set<AbstractHistogram> histSet = new HashSet<>();
    CollectionsUtil.getSingletonInstance()
        .addConditional(AbstractHistogram.getHistogramList(), histSet, BatchExport.HIST_COND_1D);
    histList.setListData(histSet.toArray(new AbstractHistogram[histSet.size()]));
