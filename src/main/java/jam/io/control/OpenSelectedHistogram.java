...
/**
 * Reads and displays list of the histograms for a user select hdf file. Then loads the user
 * selected histograms into the data base and appends the file indicator to the histogram name
 *
 * @author Ken
 */
public final class OpenSelectedHistogram implements HDFIO.AsyncListener {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(OpenSelectedHistogram.class.getPackage().getName());

  private final transient JDialog dialog;

  private final transient JTextField txtFile;

  private final transient JList<String> histList;

  private final transient DefaultListModel<String> histListData;
  ...
  /**
   * Constructs an object which uses a dialog to open a selected histogram out of an HDF file.
   *
   * @param frame parent frame
   * @param hdfio for getting HDF input
   * @param status application status
   * @param broadcaster broadcasts state changes
   */
  @Inject
  public OpenSelectedHistogram(
      final Frame frame, final HDFIO hdfio, final JamStatus status, final Broadcaster broadcaster) {
    this.frame = frame;
    this.hdfio = hdfio;
    this.status = status;
  ...
  private void loadHistograms() {
    ...
    if (selected.isEmpty()) {
      ...
      for (String histFullName : selected) {
        histAttrList.add(HistogramAttributes.getHistogramAttribute(histFullName));
      }
      /* Read in histograms */
  ...
  private boolean chooseFile() {
    ...
    final boolean openF =
        (option == JFileChooser.APPROVE_OPTION && jfile.getSelectedFile() != null);
    if (openF) {
      synchronized (this) {
  ...
  private void notifyApp() {
    ...
    if (firstGroup != null) {
      ...
      if (firstGroup.histograms.getList().size() > 0) {
        final AbstractHistogram firstHist = firstGroup.histograms.getList().get(0);
