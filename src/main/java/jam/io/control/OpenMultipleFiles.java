...
public final class OpenMultipleFiles implements HDFIO.AsyncListener {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(OpenMultipleFiles.class.getPackage().getName());
  ...
  private final transient JCheckBox chkBoxAdd;

  private final transient MultipleFileChooser multiChooser;
   ...
   * @param hdfio for opening HDF files
  ...
  public OpenMultipleFiles(
    ...
    final jam.ui.PanelOKApplyCancelButtons okApply =
        new jam.ui.PanelOKApplyCancelButtons(
  ...
  private void loadFiles() {
    ...
    if (chkBoxAdd.isSelected()) {
      hdfio.readFile(jam.io.FileOpenMode.ADD_OPEN_ONE, files, null, selectAttrib);
  ...
  private List<HistogramAttributes> createSelectedHistogramNamesList() {
    ...
    for (String histFullName : selected) {
      ...
      final HistogramAttributes histAttrib =
          HistogramAttributes.getHistogramAttribute(histFullName);
  ...
  private void notifyApp() {
    ...
    final List<jam.data.Group> groupList = jam.data.Warehouse.getGroupCollection().getList();
    if (!groupList.isEmpty()) {
      final jam.data.Group firstGroup = groupList.get(0);
      status.setCurrentGroup(firstGroup);
      final List<jam.data.AbstractHistogram> list = firstGroup.histograms.getList();
      if (!list.isEmpty()) {
