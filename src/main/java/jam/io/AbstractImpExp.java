...
/**
 * Abstract class for importing and exporting histograms. Gives you methods to open files and
 * returns either an input stream or an output stream. You write a method <code>openFile</code>,
 * which calls <code>openFile(String msg,String ext)</code> which in turn calls your implementation
 * of <code>readHist()</code>.
 *
 * @author Ken Swartz
 * @version 0.50
 * @see #openFile(File)
...
public abstract class AbstractImpExp {
  ...
  protected static final Logger LOGGER =
      Logger.getLogger(AbstractImpExp.class.getPackage().getName());

  /** size of read/write buffers */
  protected static final int BUFFER_SIZE = 256 * 256 * 4;
  ...
  protected final transient Frame frame;

  private static final Object LASTFILE_MON = new Object();

  private final transient String lastFileKey;
  ...
  /**
   * Default constructor so that it may be launched dynamically for batch exports.
   *
  ...
  public AbstractImpExp(final Frame frame) {
    ...
    lastFile = new File(PREFS.get(lastFileKey, System.getProperty("user.dir")));
    this.frame = frame;
  }
  ...
  /**
   * Opens a file for reading. Subclasses generally should call <code>openFile(msg,ext)</code> which
   * is already implemented in <code>ImpExp</code>.
   *
  ...
  /**
   * Saves a histogram or all histograms. Typically, the implementation calls the inherited <code>
   * ImpExp</code> method <code>saveFile(String,String,Histogram)</code> with appropriate arguments.
   *
  ...
  /**
   * Reads data from the passed <code>InputStream</code>. The specific implementations
   * <strong>may</strong> use this to read specific formats.
   *
   * @param inStream the stream to read the histogram from
   * @exception ImpExpException all exceptions given to <code>ImpExpException</code> go to the
   *     msgHandler
   */
  protected abstract void readData(InputStream inStream) throws ImpExpException;

  /**
   * Writes the passed <code>Histogram</code> to the passed <code>OutputStream</code>. The specific
   * implementations <strong>may</strong> use this to write specific formats.
   *
   ...
   *     msgHandler
   */
  protected abstract void writeHist(OutputStream outStream, AbstractHistogram hist)
    ...
    try {
      ...
      if (inFile != null) { // if Open file was not canceled
        ...
        final String groupName =
            GuiceInjector.getObjectInstance(FileUtilities.class)
                .removeExtensionFileName(inFile.getName());
        importGroup = jam.data.Factory.createGroup(groupName, Group.Type.FILE);
        setLastFile(inFile);
      ...
      LOGGER.log(Level.SEVERE, "Problem handling file: " + ioe.getMessage(), ioe);
      ...
      if (inBuffStream != null) {
        try {
          ...
          LOGGER.log(Level.SEVERE, "Problem closing file: " + ioe.getMessage(), ioe);
        }
      }
    }
    return rval;
  ...
  /**
   * Save a specific histogram using a file dialog box. The programmer supplies a file dialog box
   * title, a file extension, and the histogram to save. Implementers should call this from <code>
   * saveFile(Histogram)</code> .
   *
   * @param msg text to go on title bar of dialog box
   ...
   *     msgHandler
   */
  protected void saveFile(final String msg, final AbstractHistogram hist) throws ImpExpException {
    ...
    try { // open file dialog
      if (outFile != null) { // if Save file dialog was not canceled
        ...
        final BufferedOutputStream buffStream = new BufferedOutputStream(outStream, BUFFER_SIZE);
        if (!silent) {
   ...
   */
  public void saveFile(final File outFile, final AbstractHistogram hist) throws ImpExpException {
    try {
      final FileOutputStream outStream = new FileOutputStream(outFile);
      final BufferedOutputStream buffStream = new BufferedOutputStream(outStream, BUFFER_SIZE);
      writeHist(buffStream, hist);
  ...
  /**
   * Get a file from a file dialog box. See Java 1.1 AWT Reference (J.Zukowski) page 245 and
  ...
  protected File getFile(final String msg, final int state) {
    ...
    if (state == LOAD) {
      ...
      throw new IllegalArgumentException(
          getClass().getName() + "getFile() called with state = " + state);
    }
  ...
  /**
   * Returns whether this class is appropriate to use for the batch export dialog.
   *
