...
public abstract class AbstractData {
  ...
  protected static final Logger LOGGER =
      Logger.getLogger(AbstractData.class.getPackage().getName());

  private static final byte[] CLEARBYTES = new byte[0];

  /** List of Data Objects in the file. */
  private static List<AbstractData> objectList =
      Collections.synchronizedList(new ArrayList<AbstractData>());

  /** Count for references, to make sure references are unique Reference count, starts at 1 */
  private static short refCount = 1;

  /** Map of Data Objects in file with tag/ref as key */
  private static Map<Integer, AbstractData> tagRefMap =
      Collections.synchronizedMap(new HashMap<Integer, AbstractData>());
  ...
  protected static final StringUtilities STRING_UTIL =
  ...
  /**
   * Adds the data object to the list of objects. The reference number is expected to have been
   * previously assigned, but is checked to be unique for the object tag.
   *
   ...
   */
  private static <T extends AbstractData> int calculateKey(final Class<T> tag, final short ref) {
  ...
  protected static final <T extends AbstractData> T create(
      final byte[] bytes, final Class<T> tag, final short ref) throws HDFException {
  ...
  protected static <T extends AbstractData> T create(
      final Class<T> tag, final short ref, final int offset, final int length) throws HDFException {
    T dataObject = createDataObject(tag);
    if (dataObject != null) { // Only create necessary objects
  ...
  private static <T extends AbstractData> T createDataObject(final Class<T> tag)
      throws HDFException {
    T rval = null;
    if (TYPES.containsValue(tag)) {
      try {
        ...
        throw new HDFException("Couldn't create " + tag.getName() + " instance.", ie);
      }
    }
    return rval;
  }
   ...
   */
  protected static <T extends AbstractData> short createUniqueRef(final Class<T> tag) {
    if (tag == null) {
   ...
   */
  public static <T extends AbstractData> T getObject(final Class<T> tag, final short ref) {
    T match = null;
   ...
   */
  public static <T extends AbstractData> List<T> ofType(final Class<T> tagType) {
    return ofType(getDataObjectList(), tagType);
  }

  /**
   * @return a subset of the given list of <code>DataObject</code>'s of the specified type
  ...
  /**
   * Unique reference number, in case several data elements with the same tag exist. Only makes
   * sense in the context of an HDF file.
   */
  protected short ref;
  ...
  /**
   * Creates a new HDF DataObject, belonging to the specified <code>HDFile</code>. My approach is to
   * have a separate HDFile object for each physical HDF file on disk. Each <code>HDFile</code>
   * object handles the bookkeeping of the HDF File Header (see NCSA HDF: Specifications and
  ...
  /**
   * Returns a 2-byte representation of the reference number, which is unique for any given tag type
   * in an HDF file. In my code, it is unique, period, but the HDF standard does not expect or
   * require this.
   *
  ...
  /**
   * Utility method for getting an ASCII string out of the data representation.
   *
  ...
  protected void init(final byte[] data, final short reference) {
    if (data == null) {
      throw new IllegalArgumentException("Null data reference. Need data for initializiation.");
    }
    if (data.length == 0) {
      throw new IllegalArgumentException("Data array empty. Need data for initialization.");
    }
   ...
   */
  protected void init(final int byteOffset, final int len, final short reference) {
  ...
  /**
   * Utility method for inserting a String as an ASCII array into the data representation.
   *
  ...
  /**
   * Refreshes the byte array for each object, Should be called before find size or writing out.
