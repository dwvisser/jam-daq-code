...
final class ScientificData extends AbstractData {
  ...
  private enum InputMode {
    /**
     * indicates normal mode of operation, where <code>bytes</code> is read in when instantizing,
     * which can be a memory hog and cause problems
     */
    STORE,
    /** only creates the internal array when the data is requested, and then dereferences it */
    WAIT_TO_READ
  }
  ...
  /**
   * The size of the dimensions. //PROBLEM I have assumed identical x- and y- dimensions for 2-d
   * spectra. In the process of fixing this.
   */
  ...
  protected ByteBuffer getBytes() {
    ...
    switch (rank) {
      ...
      case 2:
        ...
        break;
      default:
        throw new IllegalStateException("SD_" + tag + ", bad value for rank: " + rank);
    }
    return bytes;
  }
   ...
   */
  protected int[] getData1d(final HDFile infile, final int size) throws HDFException {
  ...
  protected double[] getData1dD(final HDFile infile, final int size) throws HDFException {
    double[] output;
    if (numberType != NumberType.DOUBLE || rank != 1) {
      throw new HDFException("SD ref#" + getRef() + ": getData1dD() called on wrong type");
    }
    final byte[] localBytes = getLocalBytes(infile);
  ...
  protected int[][] getData2d(final HDFile infile, final int xlen, final int ylen)
      throws HDFException {
  ...
  protected double[][] getData2dD(final HDFile infile, final int xlen, final int ylen)
      throws HDFException {
