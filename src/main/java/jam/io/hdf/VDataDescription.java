...
public final class VDataDescription extends AbstractData {
  ...
  private static final short INTERLACE = 0;
  ...
  static final short DFNT_INT16 = 22; // NOPMD

  /** Type for <code>int</code>. */
  static final short DFNT_INT32 = 24; // NOPMD

  /** Type for <code>char</code>. */
  static final short DFNT_CHAR8 = 4; // NOPMD

  /** Type for <code>float</code>. */
  static final short DFNT_FLT32 = 5; // NOPMD

  /** Type for <code>double</code>. */
  static final short DFNT_DBL64 = 6; // NOPMD
  ...
  private static final short VH_VERSION = 3;
  ...
  VDataDescription(
    ...
    if (names.length == 0 || (names.length != types.length) || (names.length != orders.length)) {
      throw new IllegalArgumentException("All array parameters must have same length != 0.");
    }
    interlace = INTERLACE;
    ...
    int byteLength = 23 + 10 * nfields + name.length() + dataTypeName.length();
   ...
   * for storage
   */
  protected static short getColumnByteLength(final short type, final short order) {
    final short rval;
    switch (type) {
      case DFNT_INT16:
        rval = (short) (order * 2);
      ...
      case DFNT_DBL64:
        ...
        break;
      default:
        throw new IllegalArgumentException("Unknown Vdata field type: " + type);
    }
    return rval;
  }
  ...
  /**
   * Returns the <code>VdataDescription</code> with the name specified. Should only be called when
   * the name is expected to be unique.
   *
   * @param list should contain only VdataDescription objects
   * @param which type string showing what kind of info is contained
   * @return the data description with the given name
   */
  public static VDataDescription ofName(final List<VDataDescription> list, final String which) {
  ...
  public String toString() {
    final char colon = ':';
    final String rval = dataTypeName + colon + name + colon + order.length + " columns";
    return rval;
  }
}
