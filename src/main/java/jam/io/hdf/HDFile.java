...
public final class HDFile extends RandomAccessFile {
  ...
  private static final Logger LOGGER = Logger.getLogger(HDFile.class.getPackage().getName());
  ...
  private final transient AsyncProgressMonitor monitor;

  private final transient int stepsToTake;
   ...
   */
  public HDFile(final File file, final String mode) throws FileNotFoundException {
   ...
   * @exception FileNotFoundException if given file not found
   */
  HDFile(final File file, final String mode, final AsyncProgressMonitor progMon, final int steps)
   ...
   */
  protected void debugDumpDD(final short tag, final short ref, final int offset, final int length)
      throws IOException {
    mark();
    seek(offset);
    LOGGER.info("Read Tag " + tag + REF + ref + " offset " + offset + " length " + length);
    if (tag == Constants.DFTAG_NDG) {
      ...
      for (int i = 0; i < numItems; i++) {
        LOGGER.info("  VG  num " + i + " tag " + tags[i] + REF + refs[i]);
      }
    }
    reset();
  }

  private int getNumberObjctProgressStep(final int numObjects, final float timeFraction) {
    int rval = 1;
    if (stepsToTake > 0) {
   ...
   */
  protected byte[] lazyReadData(final AbstractData dataObject) throws HDFException {
    final int numObjSteps = getNumberObjctProgressStep(lazyLoadNum, FractionTime.READ_LAZY_HISTS);
  ...
  private byte[] readBytes(final int offset, final int length) throws IOException {
    ...
    if (numRead < length) {
      throw new IllegalStateException(
          "Tried to read " + length + " bytes from file. Only got " + numRead + ".");
    }
    reset();
    return rval;
  }
  ...
  private void loadDataObject(final short tag, final short ref, final int offset, final int length)
  ...
  private int getNumberOfSteps() throws IOException {
    int numObjSteps;
    if (lazyLoadData) {
      numObjSteps = getNumberObjctProgressStep(countHDFOjects(), FractionTime.READ_NOT_HIST);
    } else {
      numObjSteps = getNumberObjctProgressStep(countHDFOjects(), FractionTime.READ_ALL);
    }
    return numObjSteps;
  }
  ...
  /**
   * Looks at the internal index of data elements and sets the offset fields of the <code>DataObject
   * </code>'s. To be run when all data elements have been defined.
   */
  ...
  private void writeAllObjects() throws HDFException {
    ...
    int countObjct = 0;
    final int numObjSteps = getNumberObjctProgressStep(objectList.size(), FractionTime.WRITE_ALL);
    writeLoop:
