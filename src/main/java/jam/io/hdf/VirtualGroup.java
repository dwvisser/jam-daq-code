...
final class VirtualGroup extends AbstractData {
  ...
  private final transient List<AbstractData> elements =
  ...
  private static final short EXTAG = 0; // purpose?

  private static final short EXREF = 0; // purpose?

  private static final short VERSION = 3; // version of DFTAG_VG info

  private static final short MORE = 0; // unused but must add
  ...
  protected void refreshBytes() {
    ...
    final int numBytes = 7 * 2 + 4 * numItems + name.length() + type.length();
  ...
  /**
   * Returns a List of <code>VirtualGroup</code>'s of the type specified by <code>groupType</code>.
   *
   ...
   */
  protected static List<VirtualGroup> ofType(
      final List<VirtualGroup> list, final String groupType) {
    final List<VirtualGroup> output = new ArrayList<>();
  ...
  protected static VirtualGroup ofClass(final String groupType) {
    ...
    if (error) {
      throw new IllegalStateException("More than one VirtualGroup found with class " + groupType);
    }
    return rval;
  }

  /**
   * Returns a VirtualGroup of <code>VirtualGroup</code>'s with the name specified. Should only be
   * called when the name is expected to be unique.
   *
  ...
  protected static VirtualGroup ofName(final String groupName) {
    ...
    if (error) {
      throw new IllegalStateException("More than one VirtualGroup found with class " + groupName);
    }
    return output;
  }
   ...
   * @return the group with the given name
   */
  protected static VirtualGroup ofName(final List<VirtualGroup> list, final String groupName) {
    VirtualGroup output = null;
    for (VirtualGroup group : list) {
  ...
  public String toString() {
    return "(VG: type=\"" + type + "\", name=\"" + name + "\", ref=" + getRef() + ")";
  }
}
