...
public class ScanForScalers {
  private static final Logger LOGGER =
      Logger.getLogger(ScanForScalers.class.getPackage().getName());

  private static final char TAB = '\t';

  private final transient ProgressUpdater updater;
  ...
  private String[] getScalerNames() {
    String[] sname = null;
    final VDataDescription dataDesc =
        VDataDescription.ofName(AbstractData.ofType(VDataDescription.class), SCALER_SECT);
    // only the "scalers" VH (only one element) in the file
    if (dataDesc == null) {
      LOGGER.warning("No Scalers section in HDF file.");
    } else {
      final VData data = AbstractData.getObject(VData.class, dataDesc.getRef());
