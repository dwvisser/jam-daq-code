...
final class ConvertJamObjToHDFObj {
  ...
  /**
   * Add a text note to the file, which includes the state of <code>JamProperties</code>.
   *
  ...
  protected void addFileNote() {
    final String noteAddition =
        "\n\n"
            ...
            + "to dimension-1.  Two-dimensional data are properly displayed with increasing"
            + " channel\n"
            + "number from the lower left to the lower right for, and from the lower left to the"
            + " upper\n"
  ...
  protected VirtualGroup convertGate(final Gate gate) {
    ...
    final short[] types = {VDataDescription.DFNT_INT32, VDataDescription.DFNT_INT32};
  ...
  protected VDataDescription convertScalers(final List<Scaler> scalers) {
    ...
    final VDataDescription desc =
        new VDataDescription(name, scalerType, size, names, types, orders);
    final VData data = new VData(desc);
    for (int i = 0; i < size; i++) {
      final Scaler scaler = scalers.get(i);
      data.addInteger(0, i, scaler.getNumber());
      data.addChars(1, i, stringUtilities.makeLength(scaler.getName(), orders[1]));
  ...
  protected VDataDescription convertParameters(final List<DataParameter> parameters) {
    ...
    final VDataDescription desc =
        new VDataDescription(
            JamFileFields.PARAMETERS,
            JamFileFields.PAR_TYPE,
            size,
            JamFileFields.PARAM_COLS,
            types,
            orders);
    final VData data = new VData(desc);
    for (int i = 0; i < size; i++) {
      final DataParameter param = parameters.get(i);
      data.addChars(0, i, this.stringUtilities.makeLength(param.getName(), orders[0]));
