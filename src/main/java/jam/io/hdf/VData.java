...
public final class VData extends AbstractData {
  ...
  protected void addDouble(final int column, final int row, final double indata) {
  ...
  private byte[] getBytes(final int row, final int col) {
    final ByteBuffer out =
        ByteBuffer.allocate(VDataDescription.getColumnByteLength(types[col], order[col]));
    switch (types[col]) {
      ...
      default:
        throw new IllegalStateException(
            "Vdata.getBytes(" + row + "," + col + ") data type not good.");
    }
    return out.array();
  }
  ...
  /**
   * Get the <code>Integer</code> in the specified cell. Of course, there better actually be an
   * <code>Integer</code> there!
   *
   * @param row of cell
  ...
  /**
   * Get the <code>String</code> in the specified cell. Of course, there better actually be an
   * <code>String</code> there!
   *
   * @param row of cell
   * @param col column of cell
  ...
  /**
   * Once all the cells have been filled with objects, this should be called to set the byte
   * representation. The workhorse of this method is calls made to the <em>protected</em> method
   * <code>getBytes(row,col)</code>.
   */
  @Override
  ...
  private void setDoubleColumn(final int col, final boolean order1) {
    for (int row = 0; row < nvert; row++) {
      setCell(col, row, order1 ? getDouble(row, col) : new Double[1]); // NOPMD
    }
  }

  private void setFloatColumn(final int col, final boolean order1) {
    for (int row = 0; row < nvert; row++) {
      setCell(col, row, order1 ? getFloat(row, col) : new Float[1]); // NOPMD
    }
  }

  private void setIntColumn(final int col, final boolean order1) {
    for (int row = 0; row < nvert; row++) {
      setCell(col, row, order1 ? (Object) getInteger(row, col) : new Integer[1]); // NOPMD
    }
  }

  private void setShortColumn(final int col, final boolean order1) {
    for (int row = 0; row < nvert; row++) {
      setCell(col, row, order1 ? getShort(row, col) : new Short[1]); // NOPMD
    }
  }
