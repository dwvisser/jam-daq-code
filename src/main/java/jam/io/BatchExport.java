...
/**
 * Dialog for exporting lists of histograms. Searches <code>jam.io</code> for all classes extending
 * <code>jam.io.ImpExp</code>.
 *
 * @author <a href=mailto:dwvisser@users.sourceforge.net>Dale Visser </a>
 */
public final class BatchExport extends JDialog implements PropertyChangeListener {

  private static final Logger LOGGER = Logger.getLogger(BatchExport.class.getPackage().getName());

  /** Gives true if a histogram is 1 dimensional. */
  public static final CollectionsUtil.Condition<AbstractHistogram> HIST_COND_1D =
      hist -> hist.getDimensionality() == 1;

  private final transient JButton bExport = new JButton("Export");

  private final transient JComboBox<String> cbHist = new JComboBox<>();

  // NOPMD
  private final transient ActionListener cbHistListen =
      actionEvent -> {
        addSelectedHist();
        setExportEnable();
      };

  private final transient Map<AbstractButton, AbstractImpExp> exportMap =
      java.util.Collections.synchronizedMap(
          new java.util.HashMap<AbstractButton, AbstractImpExp>());

  private transient File lastListFile = null;

  private final transient JList<AbstractHistogram> lstHists = new JList<>(new DefaultListModel<>());

  private final transient SelectHistogramDialog selectHistDlg;

  private final transient JTextField txtDirectory =
      new JTextField(System.getProperty("user.home"), 40);

  private final transient JFrame frame;

  private final transient RuntimeSubclassIdentifier rtsi;
   ...
   * @param selectHistogram dialog for selecting histograms to export
   * @param broadcaster broadcasts state changes
   * @param rtsi for identifying export classes
  ...
  private void addAllHists() {
    ...
    CollectionsUtil.getSingletonInstance()
        .addConditional(AbstractHistogram.getHistogramList(), histSet, HIST_COND_1D);
  ...
  private void browseForDir() {
    final JFileChooser chooser = new JFileChooser(new File(txtDirectory.getText().trim()));
  ...
  private void buildGUI() {
    ...
    bBrowse.addActionListener(actionEvent -> browseForDir());
    ...
    bExport.addActionListener(actionEvent -> export());
    pButton.add(bExport);
  ...
  private List<AbstractImpExp> createExportList() {
    ...
    final Set<Class<? extends AbstractImpExp>> set =
        rtsi.find("jam.io", AbstractImpExp.class, false);
  ...
  private boolean createExportDir(final String exportDir) {
    ...
    if (!exportDirFile.exists()) { // NOPMD
      ...
      if (!exportDirFile.mkdirs()) {
        LOGGER.severe("Could not create the directory " + exportDirFile.getPath() + " .");
        status = false;
      }
      // Export dir exists, check it is a directory.
    } else if (!exportDirFile.isDirectory()) {
      LOGGER.severe("The path " + exportDirFile.getPath() + " is not a directory.");
      status = false;
    }
    return status;
  }
  ...
  private File createExportFile(
      final String dir, final String groupName, final String histName, final String extension) {
    final String fullFileName =
        this.fileUtilities.changeExtension(histName.trim(), extension, FileUtilities.APPEND_ONLY);
  ...
  private void export() {
    ...
    for (int i = 0; i < exportHistograms.length; i++) {
      ...
      exportFiles[i] =
          createExportFile(exportDir, groupName, histName, exportFormat.getDefaultExtension());
    ...
    if (status) {
      ...
      for (int i = 0; i < exportFiles.length; i++) {
        ...
        try {
          ...
          LOGGER.log(
              Level.SEVERE,
              "Exporting file: " + exportFiles[i].getPath() + " " + e.getMessage(),
              e);
        }
      }
  ...
  private AbstractButton getButton(final AbstractImpExp impExp) {
    ...
    rval.addActionListener(actionEvent -> setExportEnable());
  ...
  private Component getButtonPanel() {
    ...
    bAddHist.addActionListener(actionEvent -> selectHistDlg.show());
    ...
    bAddAllHist.addActionListener(actionEvent -> addAllHists());
  ...
  private void loadList() {
    ...
    chooser.setFileFilter(new ExtensionFileFilter(new String[] {"lst"}, "List Files (*.lst)"));
    final int option = chooser.showOpenDialog(this);
    // save current values
    if (option == JFileChooser.APPROVE_OPTION && chooser.getSelectedFile() != null) {
      ...
      try {
        final java.io.BufferedReader reader =
            new java.io.BufferedReader(new java.io.FileReader(lastListFile));
        do {
  ...
  private void removeSelectedHist() {
    final List<AbstractHistogram> removeList = lstHists.getSelectedValuesList();
    final ListModel<AbstractHistogram> model = lstHists.getModel();
  ...
  private void saveList() {
    ...
    if (option == JFileChooser.APPROVE_OPTION && chooser.getSelectedFile() != null) {
      ...
      try {
        final ListModel<AbstractHistogram> model = lstHists.getModel();
        final java.io.FileWriter saveStream = new java.io.FileWriter(chooser.getSelectedFile());
        for (int i = 0; i < model.getSize(); i++) {
  ...
  /**
   * Implementation of Observable interface listeners for broadcast events. broadcast events where
   * there are new histograms or histograms added.
   */
