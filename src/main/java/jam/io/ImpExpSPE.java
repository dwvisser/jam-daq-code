...
/**
 * Imports and Exports Spectra (Histograms) using the SPE format, this is used by RADWARE gf2.
 *
 * @version 0.5
 * @author Ken Swartz
 */
public final class ImpExpSPE extends AbstractImpExp { // NOPMD
  ...
  private static final ExtensionFileFilter FILTER = new ExtensionFileFilter("spe", "Radware gf3");
  ...
  /**
   * Reads in SPE file. We read in size channels but histogram gets defined with size channels as
   * jam has 0 to size channels.
   *
   * @exception ImpExpException all exceptions given to <code>ImpExpException</code> display on the
   *     MessageHandler
   */
    ...
    try {
      ...
      if (magicInt != MAGIC_WORD) { // magic word, int 24, hex 18, or ^X
        throw new ImpExpException(
            "Not a Spe File, incorrect magic word, word = " + magicInt + " [ImpExpSPE]");
      }
      /* read in name */
