...
/**
 * Sort routine for neutron counting with the aid of an anti-coincidence muon shield and a NaI
 * Annulus.
 *
 * @author R. Longland
 */
public class YaleCAENTestSortRoutine extends AbstractSortRoutine {
  ...
  private final transient int idNaITDC[] = new int[16]; // NaI timing
  private final transient int idNaIADC[] = new int[16]; // NaI energy
  private transient int idSciTAC; // Veto shield times
  private final transient int idSciADC[] = new int[5]; // Veto shield
  // energies
  ...
  private final transient DataParameter pcchNaI1[] = new DataParameter[16];
  private final transient DataParameter pcchNaI2[] = new DataParameter[16];
  ...
  private final transient HistInt1D hNaIADC[] = new HistInt1D[16];
  private final transient HistInt1D hNaITDC[] = new HistInt1D[16];
  private final transient HistInt1D hSciADC[] = new HistInt1D[5];
  ...
  private final transient Gate gNaITDCa[] = new Gate[16];

  // NaI ADC Gates
  private final transient Gate gNaIADC[] = new Gate[16];

  // Gate on summed NaI for 477 keV gamma
  private transient Gate gNaIsum;

  // Scintillator energy gates
  private final transient Gate gSciADC[] = new Gate[5];
  ...
  private void setupHistograms() {
    ...
    hnTDC = createHist1D(SPC_CHANNELS, "Capture Times", "Neutron Capture Peak");

    final int TWO_D_CHANNELS = 512;
    h2dnEvsPSD = createHist2D(TWO_D_CHANNELS, "E vs PSD", "Neutron E Singles vs PSD", "E", "PSD");
    h2dnEvsTDC =
        createHist2D(
            TWO_D_CHANNELS, "E vs Cap Time", "Neutron E Singles vs Capture Time", "E", "Time");
    eVsTdcPsd =
        createHist2D(
            TWO_D_CHANNELS,
            "E vs Cap Time, PSD",
            "Neutron E Singles vs Capture Time, Gated on PSD Gate a",
            "E",
            "Time");

    eNaIveto = createHist1D(SPC_CHANNELS, "E NaI Veto", "Neutron E Vetoed by NaI");
    eScintVetoEnergy =
        createHist1D(SPC_CHANNELS, "E Sci Veto E", "Neutron E Vetoed by Scintillator Energy");
    eScintVetoTime =
        createHist1D(SPC_CHANNELS, "E Sci Veto T", "Neutron E Vetoed by Scintillator Times");
    eScintNaIveto =
        createHist1D(
            SPC_CHANNELS, "E NaI and Sci Veto", "Neutron E Vetoed by NaI and Scintillators");

    ePsdGateA = createHist1D(SPC_CHANNELS, "E, PSD Gate a", "Neutron E Gated on PSD Gate a");
    ePsdGateB = createHist1D(SPC_CHANNELS, "E, PSD Gate b", "Neutron E Gated on PSD Gate b");

    eTdcGateA =
        createHist1D(SPC_CHANNELS, "E, Capture Gate a", "Neutron E Gated on Capture Gate a");
    eTdcGateB =
        createHist1D(SPC_CHANNELS, "E, Capture Gate b", "Neutron E Gated on Capture Gate b");
    eTdcGateCapture =
        createHist1D(SPC_CHANNELS, "E, 2D Capture Gate", "Neutron E Gated on 2D Capture Gate");

    eGateNaI =
        createHist1D(SPC_CHANNELS, "E, NaIE Gate", "Neutron E Gated on Summed Neutron Energy");

    hNaIsumE = createHist1D(SPC_CHANNELS, "NaI Summed Energy", "NaI Summed Energy");
    ...
    for (int i = 0; i < 5; i++) {
      hSciADC[i] = createHist1D(SPC_CHANNELS, "Sci " + i, "Scintillator E " + i);
    }
  }
  ...
  private void setupVmeMap() throws SortException {
    ...
    for (int i = 0; i < 5; i++) {
      idSciADC[i] = vmeMap.eventParameter(3, ADC_BASE_FIRST, i + 16, THRESHOLDS);
    }

    // NaI
    final int ADC_BASE_SECOND = 0xfa800000;
    for (int i = 0; i < 16; i++) {
      idNaIADC[i] = vmeMap.eventParameter(5, ADC_BASE_SECOND, i, THRESHOLDS);
  ...
  public void sort(final int[] data) {
    ...
    // Energy
    final boolean vetoSciE =
        (gSciADC[0].inGate(cSciADC[0])
            || gSciADC[1].inGate(cSciADC[1])
            || gSciADC[2].inGate(cSciADC[2])
            || gSciADC[3].inGate(cSciADC[3])
            || gSciADC[4].inGate(cSciADC[4]));
    // Time
  ...
  private double[] calculateCalibratedEnergies() {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      eNaIADC[i] =
          (diff12 / segmentDiff12) * cNaIADC[i] + (cenNaI1 - cchNaI1 * (diff12 / segmentDiff12));
    }
    return eNaIADC;
  }
  ...
  private boolean goodTimeNaITDCa() {
    ...
    final boolean goodTimeNaITDCa =
        gNaITDCa[0].inGate(cNaITDC[0])
  ...
  private void incrementWithin2Dgates(
      final int cnE, final int calibnE, final int calibnPSD, final int calibnTDC) {
  ...
  private void incrementVetoSpectra(
      final int cnE,
      final boolean goodTimeNaITDCa,
      final boolean vetoSciE,
