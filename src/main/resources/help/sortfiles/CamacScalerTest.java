 ...
 *
 * <p>Each event is 2 channel numbered 0 through 7 and for each event we increment all 2 histograms.
 * We do not do zero suppression so will would be a lot of zeros in the histograms convention for 2
 * d Histograms x first then y (x vs y)
 *
...
public class CamacScalerTest extends AbstractSortRoutine { // NOPMD
  ...
  public void initialize() throws SortException {
    ...
    rejectGeP1 =
        createHist1D(
            8192, "Ge #1 after reject:P", "Ge signal from detector #1 after rejection from prompt");
    rejectGeP2 =
        createHist1D(
            8192, "Ge #2 after reject:P", "Ge signal from detector #2 after rejection from prompt");
    rejectGeR1 =
        createHist1D(
            8192, "Ge #1 after reject:R", "Ge signal from detector #1 after rejection from random");
    rejectGeR2 =
        createHist1D(
            8192, "Ge #2 after reject:R", "Ge signal from detector #2 after rejection from random");
    hCh2 = createHist1D(8192, "Pulser #1", "Pulser signal through detector #1");
    hCh3 = createHist1D(8192, "Pulser #2", "Pulser signal through detector #2");
    ...
    ampGateP1 = createHist1D(8192, "AmpGatePrompt #1", "Ampl Gate Prompt from detector #1");
    apgGateP2 = createHist1D(8192, "AmpGatePrompt #2", "Ampl Gate Prompt from detector #2");
    ampGateR1 = createHist1D(8192, "AmpGateRandom #1", "Ampl Gate Random from detector #1");
    ampGateR2 = createHist1D(8192, "AmpGateRandom #2", "Ampl Gate Random from detector #2");
    gevsTAC1 = createHist2D(512, 128, "Ampl vs TAC #1", "Ampl vs TAC #1", "TAC #1", "Ampl");
    gevsTAC2 = createHist2D(512, 128, "Ampl vs TAC #2", "Ampl vs TAC #2", "TAC #2", "Ampl");
    ampGateSpectrum1 = createHist1D(8192, "TAC from Ge #1", "TAC signal from gate on Ge #1");
    ampGateSpectrum2 = createHist1D(8192, "TAC from Ge #2", "TAC signal from gate on Ge #2");
  ...
  private void incrementIfInGate(
      final Gate gate,
      final HistInt1D histogram,
      final int incrementChannel,
      final int checkChannel) {
