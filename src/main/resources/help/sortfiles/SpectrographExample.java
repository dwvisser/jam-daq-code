...
/**
 * Online sort routine for YLSA coincidence with Enge Spectrometer. This was modified from <CODE>
 * sort.coinc.SPplus3LEDA_v3</CODE>, which was used in the January 2001 test run. Changed 10 Aug
 * 2001 to calculate the scintillator event the right way; also added gate to cathAnde
 *
...
public final class SpectrographExample extends AbstractSortRoutine {
   ...
   * VME properties
   */
  private static final int[] ADC_BASE = {0x20000000};

  private static final int[] TDC_BASE = {0x30000000, 0x30020000};
   ...
   */
  private static final int TIME_THRESH = 30;

  /*
   * in nanoseconds
   */
  private static final int TIME_RANGE = 1200;

  /*
   * num of channels per ADC
   */
  private static final int ADC_CHANNELS = 4096;

  /*
   * 2D histograms
   */
  private static final int CHAN_2D = 256;

  /*
   * hi-res 2D histograms
   */
  private static final int TWO_D_HIRES = 2 * CHAN_2D;

  /*
   * 2D bits to shift >>
   */
  private static final int TWO_D_FACTOR =
      Math.round((float) (Math.log(ADC_CHANNELS / CHAN_2D) / Math.log(2.0)));

  /*
   * 2D hi-res bits to shift >>
   */
  private static final int HIRES_FACTOR =
      Math.round((float) (Math.log(ADC_CHANNELS / TWO_D_HIRES) / Math.log(2.0)));

  /*
   * ungated spectra
   */
  private final transient HistInt1D hCthd, hSntrSum, hFrntPsn;

  /*
   * Rear Wire Pulse Height
   */
  private final transient HistInt2D hFrntPH;

  /*
   * position x height y
   */
  private final transient HistInt2D hRearPH;

  private final transient HistInt2D hSntrCthd, hFrntCthd, hFrntSntr, hFrntPRearP;

  /*
   * gate by scintillator cathode
   */
  private final transient HistInt2D hFrntSntrGSC, hFrntCthdGSC;

  /*
   * gate by Front wire Cathode
   */
  private final transient HistInt2D hSntrCthdGFC, hFrntSntrGFC;
  private final transient HistInt2D hSntrCthdGFS, hFrntCthdGFS;

  /*
   * gate by Front wire Scintillator
   */
  private final transient HistInt1D hFrntGAll;

  /*
   * 2D gates
   */
  private final transient Gate gSntrCthd, gFrntSntr, gFrntCthd, gFrntRear;

  /*
   * number of FCLR's that went to ADC's
   */
  private final transient Monitor mEvntRaw, mEvntAccept;

  /*
   * id numbers for the signals;
   */
  private transient int idCthd, idScintR, idScintL, idFrntPsn, idRearPsn, idFrntHgh, idRearHgh;
  ...
  public SpectrographExample() {
    ...
    hSntrSum = createHist1D(ADC_CHANNELS, "ScintSum    ", "Scintillator Sum");
    hFrntPsn = createHist1D(ADC_CHANNELS, "FrontPosn    ", "Front Wire Position");
    ...
    hSntrCthd = createHist2D(CHAN_2D, "ScintCathode  ", "Cathode vs Scintillator", SCINT, CATH);
    hFrntCthd =
        createHist2D(CHAN_2D, "FrontCathode  ", "Cathode vs Front Position", FRONT_POS, CATH);
    hFrntSntr =
        createHist2D(CHAN_2D, "FrontScint ", "Scintillator vs Front Position", FRONT_POS, SCINT);
    hFrntPRearP =
        createHist2D(
            TWO_D_HIRES,
            "FrontRear  ",
    ...
    hSntrCthdGFC =
        createHist2D(
            CHAN_2D, "ScintCathodeGFC", "Cathode vs Scintillator - FwCa gate", SCINT, CATH);
    hSntrCthdGFS =
        createHist2D(
            CHAN_2D, "ScintCathodeGFS", "Cathode vs Scintillator - FwSc gate", SCINT, CATH);
    // FrontCathode Gated on other
    hFrntCthdGSC =
        createHist2D(
            CHAN_2D, "FrontCathodeGSC", "Cathode vs Front Position - ScCa gate", FRONT_POS, CATH);
    hFrntCthdGFS =
        createHist2D(
            CHAN_2D, "FrontCathodeGFS ", "Cathode vs Front Position - FwSc gate ", FRONT_POS, CATH);
    ...
    // gated on 4 gates
    hFrntGAll =
        createHist1D(ADC_CHANNELS, "FrontGAll    ", "Front Position - ScCa,FwCa,FwSc,FwRw gates");
    // gates 2d
