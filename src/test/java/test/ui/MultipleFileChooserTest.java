...
/**
 * Writing regression tests of some behavior of MultipleFileChooser, for bug fixing purposes.
 *
 * @author Dale
 */
public final class MultipleFileChooserTest {
  /**
   * Test the default behavior of MultipleFileChooser is to be in the user's home folder.
   *
   * @throws IOException if there is a file access problem
   */
  @Test
  public void testInitialFolderDefault() throws IOException {
    final File userDefault = (new JFileChooser()).getCurrentDirectory().getCanonicalFile();
   ...
   *
   * @throws IOException if file access has a problem
   */
  @Test
  public void testInitialFolderSpecified() throws IOException {
    final File specified = File.createTempFile("jam", null).getParentFile().getCanonicalFile();
