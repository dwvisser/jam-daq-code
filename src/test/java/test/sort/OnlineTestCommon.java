...
public final class OnlineTestCommon {
  ...
  public static Session session = GuiceInjector.getObjectInstance(Session.class);
  ...
  private static Future<?> submitSetupOnline(
      final String sortName,
      final Class<? extends AbstractEventInputStream> inputStream,
      final Class<? extends AbstractEventOutputStream> outputStream) {
    return EXECUTOR.submit(createSetupRunner(sortName, inputStream, outputStream));
  }

  private static Runnable createSetupRunner(
      final String sortName,
      final Class<? extends AbstractEventInputStream> inputStream,
      final Class<? extends AbstractEventOutputStream> outputStream) {
  ...
  /**
   * Setup online sorting within 20 seconds or fail, using YaleInputStream and YaleOutputStream.
   *
   * @param sortName name of the sort routine to set up.
   */
  public static void setupWithinTimeoutPeriod(final String sortName) {
    setupWithinTimeoutPeriod(sortName, YaleInputStream.class, YaleOutputStream.class);
  }
   ...
   *
   * @param sortName name of the sort routine to set up.
   * @param inputStream event input stream to use
   * @param outputStream event output stream to use
   */
  public static void setupWithinTimeoutPeriod(
      final String sortName,
      final Class<? extends AbstractEventInputStream> inputStream,
      final Class<? extends AbstractEventOutputStream> outputStream) {
    final Future<?> setupOnlineFuture = submitSetupOnline(sortName, inputStream, outputStream);
    try {
