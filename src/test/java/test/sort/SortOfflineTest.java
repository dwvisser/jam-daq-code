...
public class SortOfflineTest {
  ...
  private final transient Session session = GuiceInjector.getObjectInstance(Session.class);
  ...
  private void assertPostConditionsForYaleCAEN(final HistInt1D neutronE, final int expectedEvents) {
    assertEquals(
        expectedEvents, session.getEventsSorted(), "Events sorted wasn't the same as expected.");
    assertEquals(
  ...
  private HistInt1D setupYaleCAENOfflineSort() {
    session.setupOffline(
        "help.sortfiles.YaleCAENTestSortRoutine",
        YaleCAEN_InputStream.class,
  ...
  public void testYaleOfflineSort() {
    try {
      ...
      assertEquals(
          789.0, cathode.getArea(), 0.001, "Area in histogram wasn't the same as expected.");
    } finally {
      session.resetOfflineSorting();
    }
  }
  ...
  private HistInt1D setupYaleOfflineSort() {
    ...
    session.setupOffline(
        "help.sortfiles." + sortRoutineName, YaleInputStream.class, YaleOutputStream.class);
  ...
  public void testYaleOfflineSortTwoFiles() {
    try {
      ...
      assertEquals(
          2 * 789.0, cathode.getArea(), 0.001, "Area in histogram wasn't the same as expected.");
    } finally {
      session.resetOfflineSorting();
    }
  }
   ...
   *
   * @throws IOException if file access problem occurs
   */
  @Test
