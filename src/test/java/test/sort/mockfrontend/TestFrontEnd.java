...
public class TestFrontEnd extends JFrame {
  ...
  public TestFrontEnd() {
    ...
    center.add(fAddress);
    final NamedTextPanel jAddress = new NamedTextPanel("Jam Message Send Address", "");
    center.add(jAddress);
    final NamedTextPanel jdAddress = new NamedTextPanel("Jam Data Receive Address", "");
    center.add(jdAddress);
    final NamedTextPanel jmrAddress = new NamedTextPanel("Jam Message Receive Address", "");
    center.add(jmrAddress);
    final Console console = new Console();
    contents.add(console, BorderLayout.SOUTH);
    try {
      final String localhost = "localhost";
      final InetSocketAddress frontEnd = new InetSocketAddress(localhost, 5003);
      fAddress.setText(frontEnd.toString());
      final DatagramSocket frontEndSocket = new DatagramSocket(frontEnd);
      final InetSocketAddress jamSend = new InetSocketAddress(localhost, 5002);
      jAddress.setText(jamSend.toString());
      final InetSocketAddress jamData = new InetSocketAddress(localhost, 10205);
      jdAddress.setText(jamData.toString());
      final InetSocketAddress jamMessageReceive = new InetSocketAddress(localhost, 5006);
      jmrAddress.setText(jamMessageReceive.toString());
      final MessageSender sender =
          new MessageSender(
              eventsSent, buffersSent, console, frontEndSocket, jamData, jamMessageReceive);
      receiver = new MessageReceiver(this, console, frontEndSocket, sender);
