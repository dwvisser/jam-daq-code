...
public class MessageReceiver extends GoodThread {
   ...
   *
   * @param frame the GUI frame of this application
   * @param sender used to send messages and data
   */
  MessageReceiver(
      final Frame frame,
  ...
  /**
   * For OK_MESSAGE packets, always prints, and handles start and stop, generating YaleInputStream
   * event data. For VME_ADDRESS packets, prints out nicely formatted.
   */
  @Override
  public void run() {
    ...
    while (checkState()) {
      try {
        ...
        if (status == PacketTypes.OK_MESSAGE.intValue()) {
          eventGenerator = handleNormalMessage(eventGenerator, byteBuffer);
        ...
        JOptionPane.showMessageDialog(
            frame, e.getMessage(), getClass().getName(), JOptionPane.ERROR_MESSAGE);
      }
    }
  }

  private Future<?> handleNormalMessage(
      final Future<?> eventGenerator, final ByteBuffer byteBuffer) {
