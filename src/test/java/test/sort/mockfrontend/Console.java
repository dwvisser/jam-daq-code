...
/**
 * Class Console displays a output of commands and error messages and allows the input of commands
 * using the keyboard.
 *
 * @author Ken Swartz
 * @author Dale Visser
...
public class Console extends JPanel implements MessageHandler {
  ...
  private static final int DISPLAY_LINES = 25;
  private static final int NUMBER_LINES_LOG = 1000;

  /** Logs to a log file. */
  public static final Logger LOGGER = Logger.getLogger(Console.class.getPackage().getName());
  ...
  private final transient JTextPane textLog; // output text area
  private final transient Document doc;
  private final transient SimpleAttributeSet attr_normal, attr_warning, attr_error;
  ...
  /** Create a JamConsole which has an text area for output a text field for input. */
  public Console() {
    this(NUMBER_LINES_LOG);
  }

  /**
   * Constructor: Create a JamConsole which has an text area for output a text field for input.
   *
   * @param linesLog maximum number of lines in the log
   */
  ...
  private String buildConsoleMessage(final String message, final boolean addEndLine) {
