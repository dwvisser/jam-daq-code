...
public class MessageSender {
  ...
  private final transient SocketAddress jamMessageReceive;
  private final transient Console console;
  private final transient DatagramSocket localSocket;

  /**
   * Creates a new message sender.
   *
   * @param console for outputting messages to the screen
   */
  MessageSender(
      final Counter events,
      final Counter buffers,
      final Console console,
      final DatagramSocket localSocket,
      final SocketAddress jamData,
      final SocketAddress jamMessageReceive) {
    super();
  ...
  private final transient EventGenerator eventGenerator;
  ...
  class EventGenerator implements Runnable {
    ...
    private final transient byte[] parameter0 = {(byte) 0x80, 0x00};
    private final transient byte[] parameter1 = {(byte) 0x80, 0x01};
    ...
    EventGenerator(
        final Counter eventCounter, final Counter bufferCounter, final SocketAddress address) {
    ...
    private void fillBuffer() {
      final int eventLength = ((parameter0.length + 2) * 2) + eventEnd.length;
      ...
      for (int i = 0; i < count; i++) {
        ...
        System.arraycopy(eventEnd, 0, buffer, currentIndex, eventEnd.length);
        currentIndex += eventEnd.length;
        eventCounter.increment();
      }
      while (currentIndex < buffer.length) {
        System.arraycopy(bufferPad, 0, buffer, currentIndex, bufferPad.length);
    ...
    public void run() {
      final DatagramPacket packetMessage = constructPacket(this.jamDataSocketAddress, buffer);
  ...
  private DatagramPacket constructPacket(
      final SocketAddress socketAddress, final byte[] msgBuffer) {
    return new DatagramPacket(msgBuffer, msgBuffer.length, socketAddress);
  }
  ...
  public void sendScalerValues() {
     ...
     */
    final byte[] message = {
      0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, (byte) SCALER_VALUE
    };
    final DatagramPacket packet = this.constructPacket(this.jamMessageReceive, message);
    sendPacket(packet);
  }
